...
import availableImplementedLLMModelSpecificCategory from './engine_component/LLM_Model_Index'; // It may said it is erroring out but it isn't
...
import DDG from "duck-duck-scrape"; // might migrate later near the scraping subsystem
...
class ExternalLocalFileScraperBackgroundAgent {
	...
	public targetDirectoryList: any;
	public isRunning: any;
	public documentsLearned: any;
...
function initInteraction() {
	...
	ptyProcess.onData(async (res) => {
		...
		} else if (res.includes("\n>") || res.includes("\n> ") || res.includes("> ") || res.includes(" \n> ") || res.includes("\n\n> ") || res.includes("\n>\n")){ //ggml starts with > and ends with >, Gemma might be different
	...
	//since mainLLM will use LLaMa.cpp models we don't need to have additional attention to the mainLLM threads for other compatibility like mpt falcon and etc
	const paramArgs = `-m "${modelPath}" -n -1 --top_k ${params.top_k} --top_p ${params.top_p} -td ${threads} -tb ${threads} --temp ${params.temp} --rope-scaling yarn --repeat-penalty 1.5 --mirostat 2 -sm row --no-mmap -fa -cb -c 0 -s ${randSeed} ${basebinLLMBackendParamPassedDedicatedHardwareAccel}`; // This program require big context window set it to max common ctx window which is 4096 so additional context can be parsed stabily and not causes crashes
...
setInterval(async () => {
	...
	while (!ProcessingCoexistenceHold) {
		...
		*/
		...
		/*
		Example of LoRA training and merge adding
		...
		--separate-with-eos
		--separate-with-bos
		--opt-max-no-improvement 100
		--vocab-model ../models/ggml-vocab-llama.gguf
		--ctx 64 --embd 256 --head 8 --layer 16
		--checkpoint-in chk-shakespeare-256x16-LATEST.gguf
		--checkpoint-out chk-shakespeare-256x16-ITERATION.gguf
		--model-out ${availableImplementedLLMModelSpecificCategory.general_conversation.filename_selfRemorphedRetraining}.gguf
