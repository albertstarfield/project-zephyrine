...
const chatContainerWrapper = document.getElementById(
	"chat-container",
) as HTMLDivElement;
...
function addMessageToUI(msg: Message): string {
	...
	if (emptyState && !emptyState.classList.contains("hidden")) {
		...
		if (aboutContainer && !aboutContainer.classList.contains("hidden"))
			hideEls.push(aboutContainer);
		if (knowledgeContainer && !knowledgeContainer.classList.contains("hidden"))
			hideEls.push(knowledgeContainer);
			...
			try {
					...
					for (const line of chunk.split("\n")) {
						...
						try {
							...
							if (content) {
								...
								if (typingEl) {
									const bubble = typingEl.querySelector(
										".bubble",
									) as HTMLElement;
				...
				if (thinkState.rawBuf.length > 0) {
					if (thinkState.inThink) thinkState.thinkBuffer += thinkState.rawBuf;
						...
						thinkState.answerSegments[thinkState.answerSegments.length - 1] +=
							thinkState.rawBuf;
				}
				const typingEl = document.getElementById(typingId);
				if (typingEl) {
					...
					if (bubble) {
						if (!finalText.trim()) {
							...
							renderMarkdownToElement(
								bubble,
								"**Adelaide chose not to respond.** The model detected that continuing would likely produce a hallucinated or low-quality answer. Please rephrase or continue on the next prompt.",
							);
						} else {
		...
		if (res.ok) {
			...
			if (historySection) {
				const existingTopics =
					historySection.querySelectorAll(".history-topic");
          ...
          `;
					...
					dupBtn?.addEventListener("click", async (e) => {
						e.stopPropagation();
						const res = await fetch(`/api/sessions/${s.id}/duplicate`, {
							method: "POST",
						});
						if (res.ok) {
...
async function loadMessagesForSession(sessionId: number | null) {
	...
	try {
		...
		if (res.ok) {
			...
			if (msgs.length > 0) {
				if (emptyState && !emptyState.classList.contains("hidden"))
					hideEls.push(emptyState);
				if (chatContainerWrapper) showEls.push(chatContainerWrapper);
			} else {
				if (
					chatContainerWrapper &&
					!chatContainerWrapper.classList.contains("hidden")
				)
					hideEls.push(chatContainerWrapper);
...
function buildDisplayText(
	...
	const canonicalAnswer =
		[...answerSegments].reverse().find((s) => s.trim()) ?? "";
	...
	while (messageQueue.length > 0) {
		...
		try {
			...
			if (!reader) {
				...
				addMessageToUI({
					role: "assistant",
					content: "Error: No stream reader available.",
				});
				continue;
			}
				...
				for (const line of lines) {
					...
					try {
						...
						if (content) {
							...
							const typingEl = document.getElementById(
								typingId,
							) as HTMLElement | null;
							if (typingEl) {
								const bubble = typingEl.querySelector(
								...
								if (chatContainerWrapper) {
									chatContainerWrapper.scrollTop =
										chatContainerWrapper.scrollHeight;
								}
							}
						}
					} catch (e) {
			...
			addMessageToUI({
				role: "assistant",
				content: "Network error trying to reach the sidecar.",
			});
		}
	}
...
if (navSettings) {
	navSettings.addEventListener("click", (e) => {
		e.preventDefault();
		alert(
			"Not implemented yet but things are being settings automatically for now",
		);
	});
}
...
async function switchView(hideEls: HTMLElement[], showEls: HTMLElement[]) {
	...
	panels.forEach((p) => {
		if (
			!showEls.includes(p as HTMLElement) &&
			!p.classList.contains("hidden")
		) {
			const isInputContainer = p.classList.contains("input-container");
			const showingChatOrEmpty = showEls.some(
				(el) => el.id === "empty-state" || el.id === "chat-container",
			);
...
const searchInputK = document.getElementById(
...
const wrapperLiterature = document.getElementById(
	"knowledge-literature-wrapper",
);
const wrapperMemory = document.getElementById("knowledge-memory-wrapper");

const memSessionInput = document.getElementById(
	"memory-session-input",
) as HTMLInputElement;
const memTopicInput = document.getElementById(
	"memory-topic-input",
) as HTMLInputElement;
const memContentInput = document.getElementById(
	"memory-content-input",
) as HTMLTextAreaElement;
const memUploadBtn = document.getElementById("memory-upload-btn");
const memProgressContainer = document.getElementById(
...
if (navMain) {
	navMain.addEventListener("click", async (e) => {
		...
		if (!knowledgeContainer?.classList.contains("hidden"))
			hideEls.push(knowledgeContainer!);
		if (!chatContainerWrapper?.classList.contains("hidden"))
			hideEls.push(chatContainerWrapper!);
...
if (navTuning) {
	navTuning.addEventListener("click", async (e) => {
		...
		if (knowledgeContainer?.classList.contains("hidden")) {
			...
			if (
				wrapperLiterature &&
				!wrapperLiterature.classList.contains("hidden")
			) {
				loadKnowledgeFiles();
				loadGraph();
			} else {
							...
							if (line.trim()) {
								try {
									...
									if (data.progress !== undefined) {
										...
										if (progressText)
											progressText.textContent = `Embedding... ${data.progress}%`;
									}
...
if (memUploadBtn) {
	memUploadBtn.addEventListener("click", async () => {
		...
		try {
			...
			if (memProgressText)
				memProgressText.innerText = "Memory embedded successfully!";
...
if (searchBtnK && searchInputK) {
	searchBtnK.addEventListener("click", async () => {
		...
		if (searchResultsContainer)
			searchResultsContainer.innerHTML = "<p>Searching embeddings...</p>";

		try {
			const res = await fetch(
				`/api/knowledgestackfrontend/search?q=${encodeURIComponent(q)}`,
			);
			const data = await res.json();
...
async function loadGraph() {
	if (!cyContainer || wrapperLiterature?.classList.contains("hidden")) return;
	try {
		...
		if (!elements || elements.length <= 1 || !hasEdges) {
			cyContainer.innerHTML =
				'<p style="color:white; text-align:center; margin-top:20px;">No literature graph available yet. Upload documents to create one!</p>';
			return;
		}
...
async function loadMemoryGraph() {
	if (!cyMemoryContainer || wrapperMemory?.classList.contains("hidden")) return;
	try {
		...
		const hasEdges = elements.some((e: any) => e.data.source && e.data.target);
		if (!elements || elements.length <= 1 || !hasEdges) {
			cyMemoryContainer.innerHTML =
				'<p style="color:white; text-align:center; margin-top:20px;">No memory graph available yet. Chat with Zephy to create one!</p>';
			return;
		}
...
const mangoCanvas = document.getElementById(
	"mango-canvas",
) as HTMLCanvasElement;
const mangoJSCanvas = document.getElementById(
	"mango-js-canvas",
) as HTMLCanvasElement;
...
async function updateMangoHUD() {
	...
	try {
		const res = await fetch(
			`/api/adelaideenginestats?queue_len=${messageQueue.length}`,
		);
		if (res.ok) {
...
`;
			...
			const elp0Hist: { ts: number; val: number }[] =
				stats.WCET_ELP0_Hist || [];
			const elp1Hist: { ts: number; val: number }[] =
				stats.WCET_ELP1_Hist || [];
			const elp2Hist: { ts: number; val: number }[] =
				stats.WCET_ELP2_Hist || [];
			const mloopHist: { ts: number; val: number }[] =
				stats.WCET_mLoop_Hist || [];
			...
			const wcelHistory: { ts: number; val: number }[] =
				stats.WCEL_History_1m || [];
			...
			if (wcelHistory.length > 0) {
				...
				mangoJSCtx.font = "9px monospace";
				mangoJSCtx.fillText(
					`${(maxWCEL / 1000).toFixed(0)}ms (WCEL)`,
					jsWidth - 60,
					10,
				);
...
function initAstralGeometry() {
	...
	const materialDarkTeal = new THREE.LineBasicMaterial({
		color: 0x1e90ff,
		transparent: true,
		opacity: 0.3,
		blending: THREE.AdditiveBlending,
	});
	const materialGold = new THREE.LineBasicMaterial({
		color: 0x87cefa,
		transparent: true,
		opacity: 0.5,
		blending: THREE.AdditiveBlending,
	}); // Changed gold to light sky blue for the reference look
	...
	for (let i = 0; i < 12; i++) {
		const curve = new THREE.QuadraticBezierCurve3(
			...
			new THREE.Vector3(
				Math.cos((i * Math.PI) / 6 + Math.PI / 2) * 20,
				Math.sin((i * Math.PI) / 6 + Math.PI / 2) * 20,
				2,
			),
			new THREE.Vector3(
				Math.cos((i * Math.PI) / 6 + Math.PI) * 16,
				Math.sin((i * Math.PI) / 6 + Math.PI) * 16,
				0,
			),
		);
	...
	function animate() {
		...
		layers.forEach((layer, index) => {
			if (layer !== stars) {
				layer.rotation.z -=
					0.0003 * (index % 2 === 0 ? 1 : -1) * (1 + index * 0.05);
			}
		});
...
async function startAgenticVoice() {
	...
	try {
		...
		alert(
			"Camera and Microphone permissions are required for Multimodal Handless Agentic Mode.",
		);
	}
}
...
async function stopAndSendAgenticVoice() {
	...
	if (vadElem) {
		vadElem.innerText = "Analyzing VAD via Server...";
		vadElem.style.color = "#a855f7";
	}
	...
	try {
		...
		const response = await fetch(
			portFileUrl +
				(visionContextB64
					? `?vision_context_b64=${encodeURIComponent(visionContextB64)}`
					: ""),
			fetchOptions,
		);
...
async function stopAndSendStandardVoice() {
	...
	try {
		const response = await fetch(
			"http://127.0.0.1:11420/v1/audio/transcriptions",
			{
				method: "POST",
				body: combinedFloats.buffer,
				headers: { "Content-Type": "application/octet-stream" },
...
if (transcribeBtn && chatInputBox) {
	...
	transcribeBtn.addEventListener("touchstart", (e) => {
		e.preventDefault();
		startStandardVoice();
	});
	transcribeBtn.addEventListener("touchend", (e) => {
		e.preventDefault();
		stopAndSendStandardVoice();
	});
}
