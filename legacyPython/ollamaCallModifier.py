...
VENV_DIR = os.path.join(SCRIPT_DIR, ".venv_ollama_modifier")
REQUIREMENTS = ["flask", "requests", "werkzeug", "tiktoken", "numpy", "numba", "pyrefly", "html2image", "json5", "soundfile", "deal"]
...
def start_local_ollama():
    ...
    try:
        ollama_engine_process = subprocess.Popen(["ollama", "serve"], env=env, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except Exception as e:
                        ...
                        if text:
                            ...
                            if think_state == 0 and not full_response:
                                ...
                                if "<think>" not in text.lower():
                                    ...
                                    # Model is starting its own thought, merge it by removing its opening tag
                                    text = re.sub(r'<think>', '', text, flags=re.IGNORECASE)
                            ...
                            if think_state == 0:
                                if "</think>" in text:
                                    ...
                                    # Still in thinking state, sanitize any rogue tags
                                    text = sanitize_think_tags(text)
                            else:
                                # In answer state, ensure no internal tags bleed through
                                text = sanitize_think_tags(text)
                            if text:
                                full_response += text
