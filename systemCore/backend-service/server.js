...
// Note: Express app will handle HTTP requests defined on it.
// The http.createServer(app) makes Express handle all HTTP requests.
// WebSocket server will upgrade connections from this HTTP server.
...
// A simple health check endpoint for the HTTP server (handled by Express)
app.get('/health', (req, res) => {
  res.status(200).json({ status: 'ok', message: 'HTTP and WebSocket server is healthy' });
});

// Fallback for other HTTP GET requests (optional, you might want Express to handle 404s)
// If you want a generic message for the root or unhandled routes:
app.get('/', (req, res) => {
...
const clients = new Map(); // This seems unused currently, but kept for potential future use.
...
app.post('/api/v1/images/generations', (req, res) => {
  ...
  const placeholderUrl = `https://picsum.photos/seed/${imageId}/1024`;

  // Save to database
  const stmt = db.prepare('INSERT INTO generated_images (id, user_id, prompt, image_url) VALUES (?, ?, ?, ?)');
  stmt.run(imageId, userId, prompt, placeholderUrl, (err) => {
    if (err) {
