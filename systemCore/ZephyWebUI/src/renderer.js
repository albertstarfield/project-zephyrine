...
document.onreadystatechange = (event) => {
  ...
  document.querySelector(
    "#path-dialog-bg > div > div.dialog-button > button.secondary"
  ).style.display = "none";
  document.querySelector(
    "#path-dialog-bg > div > div.dialog-title > h2"
  ).innerText = "Couldn't load model";
...
document
  .querySelector("#path-dialog-bg > div > div.dialog-button > button.secondary")
...
ipcRenderer.on("pathIsValid", (_event, { data }) => {
  console.log(data);
  if (data) {
    document.querySelector("#path-dialog > p.error-text").style.display =
      "none";
    document.getElementById("path-dialog-bg").classList.add("hidden");
...
form.addEventListener("submit", (e) => {
  ...
  if (input.value) {
    ...
    console.debug(
      "[DEBUG MULTISUBMISSION]: It has a value! Forwarding to the system!"
    );
    say(input.value, `user${gen}`, true); // so ${gen} is for determining what part of the chat history that its belongs to, Nice!
    gen++;
    console.debug(
      "[DEBUG MULTISUBMISSION]: Forwarding to main backend Engine!"
    );
...
const say = (msg, id, isUser) => {
  ...
  item.innerHTML = msg;
  if (
    document.getElementById("bottom").getBoundingClientRect().y - 40 <
    window.innerHeight
  ) {
    setTimeout(() => {
...
ipcRenderer.on("result", async (_event, { data }) => {
  ...
  if (data == "\n\n<end>") {
    setTimeout(() => {
      // this timeout is for the final render, if somehow after the
      isRunningModel = false;
      ...
      try {
        ...
        console.error(
          prefixConsoleLogStreamCapture,
          "ERROR Marked, Preventing Disappearing",
          error,
          "Captured Content:",
          responses[id]
        );
      ...
      if (!totalStreamResultData == undefined) {
        ...
        totalStreamResultData =
          "🛑 An Unexpected Error has been occoured and Adelaide Engine lost context capture from mainLLM Thread. Please restart the program and try again!🛑 ";
      }
      /*
       */
      totalStreamResultData = firstLineParagraphCleaner(totalStreamResultData);
      console.log(
        "responsesStreamCaptureTrace_markedConverted:",
        totalStreamResultData
      );
      //totalStreamResultData = HTMLInterpreterPreparation(totalStreamResultData); //legacy old alpaca-electron interpretation
      console.log(
        "responsesStreamCaptureTrace_markedConverted_InterpretedPrep:",
        totalStreamResultData
      );
      ...
      ipcRenderer.send("AutomataLLMMainResultReciever", {
        data: responses[id],
      });
...
const LLMChildEngineIndicatorText = document.querySelector(
  "#LLMChildEngineIndicator .text"
);
const LLMChildEngineIndicatorTextBar = document.querySelector(
  "#LLMChildEngineIndicator .bar-inner"
);
const SystemBackplateInfoText = document.querySelector(
  "#SystemBackendPlateInfo .text"
);
...
const BackBrainResultQueueText = document.querySelector(
  "#BackBrainFinished .text"
);
const BackBrainResultQueueBar = document.querySelector(
  "#BackBrainFinished .bar-inner"
);
...
let dynamicTips = document.querySelector(
  "#dynamicTipsUIPart .info-bottom-changable"
);
...
ipcRenderer.on(
  "timingDegradationFactorReciever_renderer",
  (_event, { data }) => {
...
ipcRenderer.on("BackBrainQueueCheck_render", (_event, { data }) => {
  ...
  BackBrainQueueBar.style.transform = `scaleX(${
    data / maxBackBrainQueue_Fallback
  })`; //since it uses 0.0 to 1.0
  if (BackBrainQueueBar.style.transform > 1) {
    BackBrainQueueBar.style.transform = 1;
  }
  // later on the bar is going to be color spectrum representing the emotion
  setTimeout(() => {
...
ipcRenderer.on("BackBrainQueueResultCheck_render", (_event, { data }) => {
  ...
  BackBrainResultQueueBar.style.transform = `scaleX(${
    data / maxBackBrainQueue_Fallback
  })`; //since it uses 0.0 to 1.0
  if (BackBrainResultQueueBar.style.transform > 1) {
    BackBrainResultQueueBar.style.transform = 1;
  }
  // later on the bar is going to be color spectrum representing the emotion
  setTimeout(() => {
  ...
  if (data == 0) {
    ...
    setTimeout(() => {
      if (dynamiTipsProgress == "") {
        dynamicTips.innerText =
          "Waiting Adelaide Engine to Give Progress Report!";
...
ipcRenderer.on("freemem", (_event, { data }) => {
  ...
  setTimeout(() => {
    ...
    ramText.innerText = `💾 Allocated RAM ${
...
ipcRenderer.on("UMAAllocSizeStatisticsGB", (_event, { data }) => {
  ...
  UMALoadBar.style.transform = `scaleX(${
    UMAallocGB / maxDefaultUMAJavascriptAlloc
  })`;
  setTimeout(() => {
...
document
  .getElementById("interaction-session-reset")
  .addEventListener("click", () => {
    stopButton.click();
    stopButton.removeAttribute("disabled");
    ipcRenderer.send("restart");
...
ipcRenderer.send("getParams");
document
  .getElementById("interactionHistorySession")
  .addEventListener("click", () => {
    document
      .getElementById("interactionHistory-dialog-bg")
...
ipcRenderer.send("getParams");
document
  .getElementById("autonomousHandlessInteraction")
  .addEventListener("click", () => {
    document
      .getElementById("AutonomousHandlessAssistantSwitch-dialog-bg")
      .classList.remove("hidden");
    ipcRenderer.send("getParams");
  });
...
ipcRenderer.on("params", (_event, data) => {
  ...
  document.getElementById("QoSTimeoutLLMChildGlobal").value =
    data.qostimeoutllmchildglobal;
  document.getElementById("QoSTimeoutLLMChildSubCategory").value =
    data.qostimeoutllmchildsubcategory;
  document.getElementById("QoSTimeoutLLMChildBackBrainGlobalQueueMax").value =
    data.qostimeoutllmchildbackbrainglobalqueuemax;
  ...
  document.getElementById("saverestoreinteraction").checked =
    data.SaveandRestoreInteraction;
  document.getElementById("historyChatCtx").checked = data.hisChatCTX;
  document.getElementById("foreveretchedmemory").checked =
    data.foreverEtchedMemory;
  document.getElementById("throwInitialGarbageResponse").checked =
    data.throwInitResponse;
  ...
  document.getElementById("hardwarelayeroffloading").value =
    data.hardwareLayerOffloading;
  document.getElementById("sideloadexperienceuma").checked =
    data.sideloadExperienceUMA;
  document.getElementById("emotionalllmchildengine").checked =
    data.emotionalLLMChildengine;
  document.getElementById("profilepictureemotion").checked =
    data.profilePictureEmotion;
  document.getElementById("longchainthought-neverfeelenough").checked =
    data.longChainThoughtNeverFeelenough;
  ...
  document.getElementById("ragprepromptprocesscontexting").checked =
    data.ragPrePromptProcessContexting;
  document.getElementById("selfreintegrate").checked = data.selfReintegrate;
});

document
  .querySelector(
    "#settings-dialog-bg > div > div.dialog-button > button.primary"
  )
  .addEventListener("click", () => {
    ipcRenderer.send("storeParams", {
      params: {
        repeat_last_n:
          document.getElementById("repeat_last_n").value ||
          document.getElementById("repeat_last_n").placeholder,
        repeat_penalty:
          document.getElementById("repeat_penalty").value ||
          document.getElementById("repeat_penalty").placeholder,
        top_k:
          document.getElementById("top_k").value ||
          document.getElementById("top_k").placeholder,
        top_p:
          document.getElementById("top_p").value ||
          document.getElementById("top_p").placeholder,
        temp:
          document.getElementById("temp").value ||
          document.getElementById("temp").placeholder,
        seed:
          document.getElementById("seed").value ||
          document.getElementById("seed").placeholder,
        qostimeoutllmchildglobal:
          document.getElementById("QoSTimeoutLLMChildGlobal").value ||
          document.getElementById("QoSTimeoutLLMChildGlobal").placeholder,
        qostimeoutllmchildsubcategory:
          document.getElementById("QoSTimeoutLLMChildSubCategory").value ||
          document.getElementById("QoSTimeoutLLMChildSubCategory").placeholder,
        qostimeoutllmchildbackbrainglobalqueuemax:
          document.getElementById("QoSTimeoutLLMChildBackBrainGlobalQueueMax")
            .value ||
          document.getElementById("QoSTimeoutLLMChildBackBrainGlobalQueueMax")
            .placeholder,
        ...
        ragPrePromptProcessContexting: document.getElementById(
          "ragprepromptprocesscontexting"
        ).checked,
        webAccess: document.getElementById("web-access").checked,
        openaiapiserverhost: document.getElementById("openaiapiserverhost")
          .checked,
        ...
        SaveandRestoreInteraction: document.getElementById(
          "saverestoreinteraction"
        ).checked,
        hisChatCTX: document.getElementById("historyChatCtx").checked,
        foreverEtchedMemory: document.getElementById("foreveretchedmemory")
          .checked,
        throwInitResponse: document.getElementById(
          "throwInitialGarbageResponse"
        ).checked,
        ...
        emotionalLLMChildengine: document.getElementById(
          "emotionalllmchildengine"
        ).checked,
        profilePictureEmotion: document.getElementById("profilepictureemotion")
          .checked,
        websearch_amount:
          document.getElementById("websearch_amount").value ||
          document.getElementById("websearch_amount").placeholder,
        hardwareLayerOffloading:
          document.getElementById("hardwarelayeroffloading").value ||
          document.getElementById("hardwarelayeroffloading").placeholder,
        longChainThoughtNeverFeelenough: document.getElementById(
          "longchainthought-neverfeelenough"
        ).checked,
        selfReintegrate: document.getElementById("selfreintegrate").checked,
        sideloadExperienceUMA: document.getElementById("sideloadexperienceuma")
          .checked,
      },
    });
...
document
  .querySelector(
    "#settings-dialog-bg > div > div.dialog-button > button.secondary"
  )
  .addEventListener("click", () => {
    document.getElementById("settings-dialog-bg").classList.add("hidden");
  });
...
document
  .querySelector("#info-dialog-bg > div > div.dialog-button > button.secondary")
  .addEventListener("click", () => {
...
document
  .querySelector(
    "#interactionHistory-dialog-bg > div > div.dialog-button > button.secondary"
  )
  .addEventListener("click", () => {
    document
      .getElementById("interactionHistory-dialog-bg")
...
document
  .querySelector(
    "#interactionHistory-dialog-bg > div > div.dialog-button > button.primary"
  )
  .addEventListener("click", () => {
    document
      .getElementById("interactionHistory-dialog-bg")
...
document
  .querySelector(
    "#AutonomousHandlessAssistantSwitch-dialog-bg > div > div.dialog-button > button.secondary"
  )
  .addEventListener("click", () => {
    document
      .getElementById("AutonomousHandlessAssistantSwitch-dialog-bg")
...
document
  .querySelector(
    "#AutonomousHandlessAssistantSwitch-dialog-bg > div > div.dialog-button > button.primary"
  )
  .addEventListener("click", () => {
    document
      .getElementById("AutonomousHandlessAssistantSwitch-dialog-bg")
...
document.getElementById("BackbrainQueue").addEventListener("change", () => {
  ...
  ipcRenderer.send(
    "backbrainqueue",
    document.getElementById("BackbrainQueue").checked
  );
});
...
document.getElementById("AutomateLoopback").addEventListener("change", () => {
  ...
  ipcRenderer.send(
    "automateLoopback",
    document.getElementById("AutomateLoopback").checked
  );
});
...
document.getElementById("QoSTimeoutSwitch").addEventListener("change", () => {
  ...
  ipcRenderer.send(
    "qostimeoutswitch",
    document.getElementById("QoSTimeoutSwitch").checked
  );
});
...
document
  .getElementById("ragprepromptprocesscontexting")
...
document.getElementById("local-file-access").addEventListener("change", () => {
  ...
  ipcRenderer.send(
    "localAccess",
    document.getElementById("local-file-access").checked
  );
});
...
document.getElementById("LLMChildDecision").addEventListener("change", () => {
  ...
  ipcRenderer.send(
    "llmdecisionMode",
    document.getElementById("LLMChildDecision").checked
  );
});
...
document.getElementById("historyChatCtx").addEventListener("change", () => {
  ...
  ipcRenderer.send(
    "hisChatCTX",
    document.getElementById("historyChatCtx").checked
  );
});
...
document.getElementById("attemptaccelerate").addEventListener("change", () => {
  ...
  ipcRenderer.send(
    "AttemptAccelerate",
    document.getElementById("attemptaccelerate").checked
  );
});

// This event listener listens for changes in the "emotionalllmchildengine" checkbox.
document
  .getElementById("emotionalllmchildengine")
  .addEventListener("change", () => {
...
document
  .getElementById("sideloadexperienceuma")
  .addEventListener("change", () => {
...
document.getElementById("selfreintegrate").addEventListener("change", () => {
  ...
  ipcRenderer.send(
    "selfReintegrate",
    document.getElementById("selfreintegrate").checked
  );
});
