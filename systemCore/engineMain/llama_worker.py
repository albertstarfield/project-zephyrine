...
import time
import traceback
from typing import Any, Dict, List, Optional, Union  # Added for type hints
...
parser.add_argument(
    "--task-type", type=str, required=True, help="Task: chat, embedding, etc."
)
...
parser.add_argument(
    ...
    type=str,
    default="chatml",
    help="Chat template format (e.g., chatml, llama-3)",
)
...
try:
    ...
    try:
        ...
        try:
            cl100k_base_encoder = tiktoken.encoding_for_model(
                "gpt-4"
...
try:
    ...
    from CortexConfiguration import LLAMA_CPP_N_CTX, MODULE_DIR

    STATE_SAVE_DIR = os.path.join(MODULE_DIR, "staticModelState")
    EMBEDDING_CTX_CONFIG = LLAMA_CPP_N_CTX
    log_worker(
        "INFO",
        f"Loaded EMBEDDING_CTX_CONFIG from CortexConfiguration: {EMBEDDING_CTX_CONFIG}",
    )
except ImportError:
    # Fallback if CortexConfiguration is not found (e.g., during standalone testing)
    STATE_SAVE_DIR = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "staticModelState"
    )
    log_worker(
        "WARNING",
        f"Could not import STATE_SAVE_DIR from config. Using default: {STATE_SAVE_DIR}",
    )
    EMBEDDING_CTX_CONFIG = 512  # Fallback to old hardcoded value
    log_worker(
        "WARNING",
        f"Could not import LLAMA_CPP_N_CTX. Using fallback for embedding n_ctx: {EMBEDDING_CTX_CONFIG}",
    )
...
DEFAULT_N_CTX_FOR_FALLBACK = INFERCOMPLETION_CTX_BINNING[
    2
]  # If token counting fails or not applicable
MIN_DYNAMIC_N_CTX = INFERCOMPLETION_CTX_BINNING[
    1
]  # Minimum context size for dynamic calculation (can be tuned)

MAX_DYNAMIC_N_CTX = INFERCOMPLETION_CTX_BINNING[
    -1
]  # Maximum context size for dynamic calculation
...
def count_tokens_cl100k(text_or_messages: Union[str, List[Dict[str, str]]]) -> int:
    ...
    if not text_or_messages:
        log_worker(
            "TRACE", "count_tokens_cl100k: Input is None or empty. Returning 0 tokens."
        )
        return 0
    ...
    if not TIKTOKEN_AVAILABLE or not cl100k_base_encoder:
        log_worker(
            "WARNING",
            "count_tokens_cl100k: Tiktoken not available or encoder not loaded. Using character-based estimate.",
        )
    ...
    try:
        if isinstance(text_or_messages, str):
            ...
            if not any(
                msg.get("content") for msg in text_or_messages if isinstance(msg, dict)
                        ...
                        for part_idx, part in enumerate(content_item):
                            if (
                                ...
                                if part.get("text", "").strip():
                                    tokens = cl100k_base_encoder.encode(
                                        part.get("text", "")
                                    )
                                    current_msg_tokens += len(tokens)
        ...
        log_worker(
            "ERROR",
            f"Tiktoken counting error in worker: {e}. Returning -2 to indicate error.",
        )
        return -2  # Indicate a counting error, distinct from 0 or character estimate


# --- Adaptive Middle Truncate Function ---
def adaptive_middle_truncate(
    text: str, target_max_tokens: int, model_actual_n_ctx: int
) -> str:
    """
    ...
    if not TIKTOKEN_AVAILABLE or not cl100k_base_encoder:
        log_worker(
            "WARNING",
            "adaptive_middle_truncate: Tiktoken not available. Using basic char limit as fallback.",
        )
    ...
    try:
        ...
        log_worker(
            "ERROR",
            f"adaptive_middle_truncate: Error counting initial tokens: {e_count}. Using char count.",
        )
    ...
    while (
        current_tokens > target_max_tokens
        and iteration_count < max_iterations
        and len(truncated_text) > 10
    ):  # Ensure some text remains
        iteration_count += 1
        if len(truncated_text) <= chars_to_remove_at_once:
            log_worker(
                "WARNING",
                "adaptive_middle_truncate: Text too short for further middle removal. Stopping.",
            )
            break
        ...
        end_remove_index = min(
            len(truncated_text), middle_index + (chars_to_remove_at_once - remove_half)
        )
        ...
        truncated_text = (
            truncated_text[:start_remove_index] + truncated_text[end_remove_index:]
        )
        ...
        if current_tokens > target_max_tokens:
            ...
            if tokens_still_over > initial_overshoot_tokens * 0.5:  # Still far
                chars_to_remove_at_once = min(
                    chars_to_remove_at_once * 2, len(truncated_text) // 8, 2000
                )  # Aggressive, capped
            elif tokens_still_over > initial_overshoot_tokens * 0.1:  # Getting closer
                chars_to_remove_at_once = min(
                    max(10, chars_to_remove_at_once), len(truncated_text) // 10, 500
                )  # Moderate
            else:  # Very close
                chars_to_remove_at_once = max(
                    2, chars_to_remove_at_once // 2
                )  # Fine-tune

        chars_to_remove_at_once = max(
            2, chars_to_remove_at_once
        )  # Ensure at least 2 chars are attempted
    ...
    if iteration_count >= max_iterations:
        log_worker(
            "WARNING",
            "adaptive_middle_truncate: Reached max iterations. Truncation might be inexact.",
        )
    ...
    if len(truncated_text) < original_length_chars:
        ...
        if count_tokens_cl100k(final_text_with_marker) > target_max_tokens + (
            count_tokens_cl100k(marker) // 2
        ):  # Allow some leeway for marker
            log_worker(
                "WARNING",
...
def main():
    # --- Argument Parsing ---
    parser = argparse.ArgumentParser(
        description="Llama.cpp Worker Process with Automatic KV Caching and Dynamic Context"
    )
    parser.add_argument("--model-path", required=True, help="Path to GGUF model file")
    parser.add_argument(
        "--task-type",
        required=True,
        choices=["chat", "embedding", "raw_text_completion", "vision"],
        help="Task type",
    )
    parser.add_argument(
        "--n-gpu-layers", type=int, default=-1, help="Number of GPU layers"
    )
    parser.add_argument(
        "--n-ctx",
        type=int,
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Enable llama.cpp verbose logging"
    )
    parser.add_argument(
        "--chat-format",
        type=str,
    ...
    try:
        ...
        log_worker(
            "INFO",
            f"Initial estimated input tokens (for n_ctx calc): {input_tokens_for_dynamic_ctx}",
        )

    except (json.JSONDecodeError, ValueError, TypeError) as e_stdin:
        log_worker(
            "CRITICAL",
            f"Failed to read/parse stdin: {e_stdin}\n{traceback.format_exc()}",
        )
        # This ensures the script stops and reports the error correctly.
        print(
            json.dumps(
                {
                    "error": f"Worker critical error: Failed to read/parse request: {e_stdin}"
                }
            )
        )
        sys.exit(1)
    ...
    if args.task_type == "chat":
        ...
        prompt_for_llm = format_messages_as_string(
            messages_from_provider
        )  # Ensure you have this helper
    else:
    ...
    if args.task_type == "embedding":
        ...
        log_worker(
            "INFO",
            f"Forcing n_ctx to EMBEDDING_CTX_CONFIG ({EMBEDDING_CTX_CONFIG}) for embedding task, ignoring CLI args.",
        )
    elif args.n_ctx is not None:
        calculated_n_ctx_for_model_load = args.n_ctx
        log_worker(
            "INFO",
            f"Using n_ctx={calculated_n_ctx_for_model_load} from cortex_backbone_provider override.",
        )
    ...
    # ------------------- MODIFIED BLOCK FOR CONTEXT BINNING -------------------
        ...
        log_worker(
            "INFO",
            f"Input tokens ({input_tokens_for_dynamic_ctx}) need ~{target_n_ctx} context. Locking to bin: {calculated_n_ctx_for_model_load}",
        )
    # ------------------------------------------------------------------------
        ...
        log_worker(
            "INFO",
            f"Falling back to default n_ctx={calculated_n_ctx_for_model_load} for model load.",
        )

    log_worker("INFO", f"--- Worker Config (effective for model load) ---")
    log_worker(
        "INFO",
        f"  Model: '{os.path.basename(args.model_path)}', Task: '{args.task_type}'",
    )
    log_worker(
        "INFO", f"  Effective n_ctx for Llama load: {calculated_n_ctx_for_model_load}"
    )

    # --- Main Execution Block ---
    try:
        # 1. Determine the dedicated state path for this model AND n_ctx
        state_file_path = get_state_path_for_model(
            args.model_path, calculated_n_ctx_for_model_load
        )
        ...
        if should_attempt_cache_load:
            log_worker(
                "INFO",
                f"KV cache found. LMExec will attempt to load: {state_file_path}",
            )
            ...
            log_worker(
                "INFO", f"Starting {binary_to_use} fresh (No cache or embedding task)."
            )
        ...
        log_worker(
            "INFO",
            f"Model loaded in {(time.monotonic() - load_start_time) * 1000:.2f} ms.",
        )
        ...
        if args.task_type == "chat":
            ...
            if current_input_token_count_chat == -1:
                current_input_token_count_chat = len(json.dumps(messages_for_llm)) // 3
            max_gen_tokens_chat = min(
                original_kwargs_from_provider.get(
                    "max_tokens", calculated_n_ctx_for_model_load // 2
                ),
                calculated_n_ctx_for_model_load - current_input_token_count_chat - 64,
            )
            if max_gen_tokens_chat <= 0:
                max_gen_tokens_chat = GENERATION_OUTPUT_BUFFER_TOKENS // 2
            original_kwargs_from_provider["max_tokens"] = max_gen_tokens_chat
            chat_cmd = base_cmd + [
                "-c",
                str(calculated_n_ctx_for_model_load),
                "--no-display-prompt",
                "--chat-template",
                args.chat_format or "chatml",
                "--prompt",
                format_messages_as_string(messages_for_llm),
                "--n-predict",
                str(max_gen_tokens_chat),
            ]
            ...
            completion_result_dict = {
                ...
                "choices": [
                    {
                        ...
                        "message": {
                            "role": "assistant",
                            "content": cleanup_initial_think_tag(
                                raw_generated_text
                ...
                "usage": {
                    ...
                    "completion_tokens": count_tokens_cl100k(
                        raw_generated_text
                    ),  # Reuse your token counter
                    "total_tokens": input_tokens_for_dynamic_ctx
                    + count_tokens_cl100k(raw_generated_text),
                },
            }
            ...
            if not prompt_from_provider or not isinstance(prompt_from_provider, str):
                raise ValueError(
                    "Missing or invalid 'prompt' for 'raw_text_completion' task."
                )
            target_input_prompt_max_tokens = (
                calculated_n_ctx_for_model_load - GENERATION_OUTPUT_BUFFER_TOKENS
            )
            if target_input_prompt_max_tokens <= 0:
                target_input_prompt_max_tokens = calculated_n_ctx_for_model_load // 2
            prompt_for_llm = adaptive_middle_truncate(
                prompt_from_provider,
                target_input_prompt_max_tokens,
                calculated_n_ctx_for_model_load,
            )
            current_input_token_count_raw = count_tokens_cl100k(prompt_for_llm)
            if current_input_token_count_raw == -1:
            ...
            cli_cmd = [
                ...
                prompt_for_llm,
                "--n-predict",
                str(max_gen_tokens_raw),
                "--prompt-cache",
                state_file_path,  # Native binary KV cache
            ...
            if "temperature" in original_kwargs_from_provider:
                cli_cmd.extend(
                    ["--temp", str(original_kwargs_from_provider["temperature"])]
                )
            ...
            process = subprocess.run(
                cli_cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True
            )
            ...
            completion_result_dict = {
                "choices": [{"text": process.stdout.strip(), "finish_reason": "stop"}]
            }
            ...
            for text in texts_to_embed:
                ...
                embed_cmd = [
                    ...
                    "mean",
                    "--embd-output-format",
                    "json",  # Returns OpenAI-compatible JSON
                    "--prompt",
                    text,
                ]
                ...
                if process.returncode == 0:
                    try:
                        ...
                        output_data = json.loads(
                            process.stdout
                        )  # recieve the stdio (THIS IS RAW DATA So it is debug target!)
                        ...
                        if (
                            ...
                            and isinstance(output_data["data"], list)
                        ):
                        ...
                        # {"embedding": [...]}
                            ...
                            isinstance(output_data, dict) and "embedding" in output_data
                        ):
                        ...
                        if vector and isinstance(vector, list):
                            ...
                            log_worker(
                                "ERROR",
                                f"Parsed JSON did not contain 'embedding' field. Debug Dump: {output_data} with Key Dump {output_data.keys() if isinstance(output_data, dict) else 'List'}",
                            )
                        ...
                        log_worker(
                            "ERROR",
                            f"Failed to parse LMText2Vector JSON: {e} Debug Dump: {output_data}",
                        )
                        completion_result_dict = {
                            "error": "Invalid JSON from embedding binary"
                        }
                else:
                    log_worker(
                        "ERROR", f"LMText2Vector failed with code {process.returncode}"
                    )
                    completion_result_dict = {
                        "error": "Embedding binary execution failed"
                    }
            ...
            if not image_path or not os.path.exists(image_path):
                log_worker(
                    "WARNING", f"Vision Task: Image path not found: {image_path}"
                )
            ...
            # Check 2: Is file too small (< 1024 bytes)?
                ...
                log_worker(
                    "WARNING",
                    f"Vision Task: Image too small ({size} bytes). Treating as broken/empty.",
                )
                should_skip_vision_proc = True
            ...
            if should_skip_vision_proc:
                ...
                completion_result_dict = {
                    "choices": [
                        {
                            "message": {"content": vision_error_msg},
                            "finish_reason": "stop",
                        }
                    ]
                }
            # --- [END] IMAGE VALIDATION LOGIC ---
                ...
                prompt_for_llm = request_data_dict.get(
                    "prompt",
                ...
                vision_cmd = [
                    ...
                    "off",
                    "-ot",
                    ".ffn_.*_exps.=CPU",  # Unsloth recommendation on optimizaiton of memory
                    "--no-warmup",
                    "--offline",
                    "--no-host",
                    "--n-gpu-layers",
                    ...
                    "--temp",
                    str(original_kwargs_from_provider.get("temperature", 0.8)),
                    "-n",
                    "2048",  # Limit generation for descriptions
                    "-p",
                    prompt_for_llm,
                ]
        ...
        if args.task_type != "embedding":
            ...
            if TIKTOKEN_AVAILABLE and token_count_after_cleanup == 0:
                log_worker(
                    "WARNING",
                    "Empty response detected. Attempting re-request with system nudge.",
                )
                ...
                if args.task_type == "chat" or args.task_type == "vision":
                    ...
                    rereq_cmd = rereq_base + [
                        "--n-predict",
                        str(calculated_n_ctx_for_model_load // 2),
                        "--prompt",
                        modified_prompt,
                    ]
                else:  # raw_text_completion
                    modified_prompt = complaint_prefix + request_data_dict.get(
                        "prompt", ""
                    )
                    if max_gen_tokens_raw <= 0:
                        log_worker(
                            "WARNING",
                            "max_gen_tokens_raw was 0 for re-request, defaulting to 256.",
                        )
                ...
                raw_generated_text_attempt2 = extract_final_answer(
                    process.stdout.strip()
                )
                cleaned_text_attempt2 = cleanup_initial_think_tag(
                    raw_generated_text_attempt2
                )

                if args.task_type == "chat" or args.task_type == "vision":
                    completion_result_dict["choices"][0]["message"]["content"] = (
        ...
        task_processing_duration_ms = (
            time.monotonic() - task_processing_start_time
        ) * 1000
        log_worker(
            "INFO",
        ...
        if args.task_type != "embedding":
            ...
            log_worker(
                "SUCCESS",
                f"KV cache automatically updated via llama C++ binary at: {state_file_path}",
            )
        else:
            log_worker(
                "INFO",
                "Embedding task finished. Native state caching skipped as per logic.",
            )

    except Exception as e:
        log_worker(
            "ERROR", f"Error during worker execution: {e}\n{traceback.format_exc()}"
        )
    ...
    # --- Send Result to Parent and Exit ---
        ...
        try:
            ...
            log_worker(
                "CRITICAL",
                f"Failed to serialize/write final result_payload to stdout: {e_serialize}",
            )
            fallback_error_msg = {
    ...
    log_worker(
        "INFO", f"Llama Worker (PID: {os.getpid()}) process finished gracefully."
    )
    sys.exit(0)
