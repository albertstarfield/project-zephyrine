...
from typing import Any, Dict, List, Optional, Tuple  # <<< ADD Optional and List HERE
...
try:
    ...
    logger.info(
        "✅ pillow-avif-plugin found. On-the-fly AVIF/HEIC conversion for OCR is enabled."
    )
except ImportError:
    AVIF_SUPPORT_AVAILABLE = False
    logger.warning(
        "⚠️ pillow-avif-plugin not installed. On-the-fly AVIF/HEIC conversion for OCR is disabled. Run: pip install pillow-avif-plugin"
    )
...
try:
    ...
    logger.warning(
        "pdf2image or poppler not installed. PDF -> Image conversion disabled."
    )
...
try:
    ...
    logger.critical(
        "❌ Failed to import database components. Indexer functionality will be disabled."
    )
...
OFFICE_EXTENSIONS = {
    ...
    ".pptx",
    ".ppt",
}  # Added from CortexConfiguration.py

# --- File types we want to embed ---
EMBEDDABLE_EXTENSIONS = TEXT_EXTENSIONS.union(DOC_EXTENSIONS).union(
    {".csv"}
)  # Explicitly add csv here if not in TEXT_EXTENSIONS


def log_worker(level: str, message: str):
    """Basic logging to stderr for the worker itself."""
    print(
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')} AUDIO_WORKER(PID:{os.getpid()})|{level}] {message}",
        file=sys.stderr,
        flush=True,
    )
...
class FileIndexer:
    ...
    def __init__(
        ...
        server_busy_event: threading.Event,
    ):
        ...
        self.vectorstore = (
            get_global_file_index_vectorstore()
        ...
        if self.vectorstore is None:
            ...
            logger.success(
                f"✅ {self.thread_name}: Successfully acquired vector store instance."
            )
        ...
        vlm_model_info = (
            getattr(
                self.vlm_model,
                "model",
                getattr(self.vlm_model, "model_name", type(self.vlm_model).__name__),
            )
            if self.vlm_model
            else "Not Available"
        )
        latex_model_info = (
            getattr(
                ...
                "model",
                getattr(
                    self.latex_model, "model_name", type(self.latex_model).__name__
                ),
            )
            if self.latex_model
            else "Not Available"
        ...
        if not self.embedding_model:
            logger.warning("⚠️ Embeddings disabled.")
        if not self.vlm_model or not self.latex_model:
            logger.warning(
                "⚠️ VLM->LaTeX processing disabled (one or both models missing)."
            )

    def _wait_if_server_busy(
        self, check_interval=15, log_wait=True
    ):  # make the check interval longer so it's only indexing when it's truly idle
        ...
        while self.server_busy_event.is_set():
            if not waited and log_wait:  # Log only once per wait period
                logger.info(
                    f"🚦 {self.thread_name}: Server is busy, pausing indexing..."
                )
            waited = True
            # Check stop event frequently while waiting
            if self.stop_event.wait(timeout=check_interval):
                logger.info(
                    f"🛑 {self.thread_name}: Stop signaled while waiting for server."
                )
    ...
    def _get_latex_from_image(
        self, image: Image.Image, session_id_for_log: str
        ...
        try:
            ...
            timing_data = {
                ...
                "mode": "vlm_latex_extraction",
            }

            response_markdown = asyncio.run(
                asyncio.to_thread(
            ...
            latex_code = None
            tikz_code = None
            explanation = response_markdown
            latex_match = re.search(
                r"```latex\s*(.*?)\s*```", response_markdown, re.DOTALL
            )
            tikz_match = re.search(
                r"```tikz\s*(.*?)\s*```", response_markdown, re.DOTALL
            )
    ...
    def _extract_text_with_ocr_fallback(self, file_path: str, file_ext: str) -> str:
        ...
        try:
            ...
            if file_ext == ".pdf":
                ...
                image_to_process = Image.frombytes(
            ...
            if process.returncode == 0:
                ...
                logger.error(
                    f"{log_prefix}: Tesseract subprocess failed. Stderr: {process.stderr.strip()}"
                )
            ...
            if temp_image_path and os.path.exists(temp_image_path):
                try:
                    ...
                    logger.warning(
                        f"Failed to delete temp OCR file {temp_image_path}: {del_err}"
                    )
    ...
    def _convert_office_to_pdf(self, office_path: str) -> Optional[str]:
        ...
        if not shutil.which("unoconv"):
            logger.error(
                f"unoconv command not found. Cannot convert Office file: {office_path}"
            )
            return None

        temp_pdf_dir = os.path.join(
            os.path.dirname(office_path), "temp_office_conversions"
        )
        ...
        logger.debug(
            f"Attempting to convert '{office_path}' to PDF at '{temp_pdf_path}' using unoconv..."
        )
        try:
            ...
            process = subprocess.run(
                ["unoconv", "-f", "pdf", "-o", temp_pdf_path, office_path],
                capture_output=True,
                text=True,
                timeout=120,
                check=False,
            ...
            if process.returncode == 0 and os.path.exists(temp_pdf_path):
                logger.info(
                    f"Successfully converted '{office_path}' to '{temp_pdf_path}'."
                )
                return temp_pdf_path
            else:
                logger.error(
                    f"unoconv conversion failed for '{office_path}'. RC: {process.returncode}"
                )
    ...
    def _get_initial_vlm_description(
        ...
        logger.trace(
            "Sending image page to VLM Model for initial analysis (Priority: ELP0)..."
        )
        ...
        try:
            ...
            messages = [
                HumanMessage(
                    content=[
                        image_content_part,
                        {"type": "text", "text": PROMPT_VLM_INITIAL_ANALYSIS},
                    ]
                )
            ]
            ...
            logger.trace(
                f"  VLM initial analysis raw response length: {len(raw_description)}"
            )
            return (
                raw_description,
                None,
            )  # Return description, no error string if successful
    ...
    def _refine_to_latex_tikz(
        self, image: Image.Image, initial_analysis: str
    ) -> Tuple[Optional[str], Optional[str]]:
        """Calls the 'latex' model to refine VLM analysis into LaTeX/TikZ with ELP0 priority."""
        # Check server busy/stop event first
        if self._wait_if_server_busy():
            return None, "[LaTeX Model Skipped - Server Busy]"
        if self.stop_event.is_set():
            return None, "[LaTeX Model Skipped - Stop Requested]"
        if not self.latex_model:
            return None, "[LaTeX Model Unavailable]"
        ...
        logger.trace(
            "Sending image and initial analysis to LaTeX Model for refinement (Priority: ELP0)..."
        )
        ...
        try:
            ...
            # Prepare messages and chain
            prompt_text = PROMPT_LATEX_REFINEMENT.format(
                initial_analysis=initial_analysis
            )
            messages = [
                HumanMessage(
                    content=[image_content_part, {"type": "text", "text": prompt_text}]
                )
            ]
            chain = self.latex_model | StrOutputParser()
            ...
            latex_code = None
            tikz_code = None
            explanation = response_markdown.strip()
            try:
                latex_match = re.search(
                    r"```(?:latex)?\s*(.*?)\s*```",
                    response_markdown,
                ...
                if latex_match:
                    ...
                    cleaned_response = cleaned_response.replace(
                        latex_match.group(0), "", 1
                    )
                    logger.trace("  LaTeX Model extracted LaTeX.")
                if tikz_match:
                    tikz_code = tikz_match.group(1).strip()
                    cleaned_response = cleaned_response.replace(
                        tikz_match.group(0), "", 1
                    )
                    logger.trace("  LaTeX Model extracted TikZ.")
                explanation = cleaned_response.strip()
                if not explanation and (latex_code or tikz_code):
                    explanation = (
                        "(Code extracted, no separate explanation provided by model)"
                    )
            except Exception as parse_e:
                logger.warning(
                    f"Could not parse refined LaTeX/TikZ response: {parse_e}"
                )
    ...
    def _calculate_md5(
        self, file_path: str, size_bytes: Optional[int]
    ) -> Optional[str]:
        ...
        if size_bytes > MAX_HASH_FILE_SIZE_BYTES:
            logger.trace(
                f"Skipping MD5 hash (file too large: {size_bytes / 1024 / 1024:.1f} MB > {MAX_HASH_FILE_SIZE_MB} MB): {file_path}"
            )
        ...
        try:
            with open(file_path, "rb") as f:
                while True:
                    if self.stop_event.is_set():
                        logger.info(
                            f"MD5 calculation interrupted by stop signal: {file_path}"
                        )
    ...
    def _get_root_paths(self) -> list[str]:
        ...
        if sys.platform.startswith("win"):
            ...
            logger.warning(
                f"Unsupported platform '{sys.platform}'. Using '/' as default root."
            )
            paths = ["/"]
    ...
    def _get_file_metadata(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "last_modified_os": datetime.datetime.fromtimestamp(
    ...
    def _extract_text(self, file_path: str, size_bytes: int) -> Optional[str]:
        ...
        if size_bytes > MAX_TEXT_FILE_SIZE_BYTES:
            logger.trace(
                f"Skipping text read (too large: {size_bytes / 1024 / 1024:.1f} MB): {file_path}"
            )
        ...
        try:
            ...
            for i, page in enumerate(reader.pages):
                ...
                try:
                    ...
                    logger.warning(
                        f"Error extracting text from page {i + 1} of {file_path}: {page_err}"
                    )
            return text_content.strip() if text_content else None
        except pypdf.errors.PdfReadError as pe:
            logger.warning(
                f"Failed to read PDF (possibly encrypted or corrupted): {file_path} - {pe}"
            )
            return None
        except PermissionError:
    ...
    def _process_single_file(
        self, file_path: str, db_session: Session, loop: asyncio.AbstractEventLoop
    ):
        """
        ...
        try:
            ...
            loop.run_until_complete(self._process_file_phase1(file_path, db_session))
    ...
    def _scan_directory(
        self, root_path: str, db_session: Session, loop: asyncio.AbstractEventLoop
    ):
        """
        ...
        current_script_dir_for_project_paths = os.path.dirname(
            os.path.dirname(os.path.abspath(__file__))
        )  # Assuming file_indexer is in mainEngineFrame_MacroController_EngineSharedResources, and ROOT_DIR is one level up
        ...
        common_system_dirs_raw = {
            ...
            "out",
            "bin",
            "obj",  # Note: "bin" and "lib" are very generic
            ".git",
            ".hg",
            ".svn",
            ".gradle",
            ...
            "temp",
            "logs",
            "log",  # "logs" is also your project log dir
        ...
        for p_raw_item in common_system_dirs_raw:
            ...
            if "%" in p_raw_item and (IS_WINDOWS or platform.system() == "Windows"):
                ...
                if (
                    p_expanded_item == p_raw_item and p_raw_item.count("%") >= 2
                ):  # Unexpanded var
                    logger.trace(
                        f"Path with '%' not expanded: '{p_raw_item}'. Treating as relative name."
                    )
            ...
            if os.path.isabs(p_expanded_item):
                absolute_skip_dirs_resolved.add(
                    os.path.normcase(os.path.realpath(p_expanded_item))
                )
            else:
        ...
        try:
            for current_dir, dirnames, filenames in os.walk(
                root_path, topdown=True, onerror=None
            ):
                # logger.debug(f"SCANNER_WALK_DEBUG: Entering directory: {current_dir}")
                if self.stop_event.is_set():
                    logger.info(
                        f"Phase 1 Scan interrupted by stop signal in {current_dir}"
                    )
                    break

                norm_current_dir_for_abs_match = os.path.normcase(
                    os.path.realpath(current_dir)
                )
                current_dir_basename_lower_for_rel_match = os.path.basename(
                    current_dir
                ).lower()
                ...
                for skip_abs_path in absolute_skip_dirs_resolved:
                    if (
                        norm_current_dir_for_abs_match == skip_abs_path
                        or norm_current_dir_for_abs_match.startswith(
                            skip_abs_path + os.sep
                        )
                    ):
                ...
                if (
                    not should_skip_dir
                    and current_dir_basename_lower_for_rel_match
                    in relative_skip_dir_names_lower
                ):
                    should_skip_dir = True
                ...
                ]  # Prune hidden subdirs from recursion
                ...
                for filename in filenames:
                    ...
                    if filename.startswith("."):
                        continue
                    if filename.lower() in files_to_skip_lower:
                        continue
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Phase 1 Permission denied processing file: {file_path}"
                        )
                        file_errored_this_iter = True
                        try:
                            existing_rec = (
                                db_session.query(FileIndex)
                                .filter(FileIndex.file_path == file_path)
                                .first()
                            )
                            err_vals = {
                                "index_status": "error_permission",
                                "processing_error": "Permission denied during scan.",
                                "last_indexed_db": datetime.datetime.now(
                                    datetime.timezone.utc
                                ),
                            }
                            if existing_rec:
                                if existing_rec.index_status != "error_permission":
                                    db_session.execute(
                                        update(FileIndex)
                                        .where(FileIndex.id == existing_rec.id)
                                        .values(**err_vals)
                                    )
                            else:
                                db_session.add(
                            ...
                            logger.error(
                                f"Failed to log perm error for {file_path}: {db_perm_err}"
                            )
                            db_session.rollback()
                        ...
                        if file_processed_this_iter:
                            total_processed_this_root_scan += 1
                            files_since_last_report += 1
                        if file_errored_this_iter:
                            total_errors_this_root_scan += 1
                            errors_since_last_report += 1
                            current_dir_file_errors += 1

                        current_time_monotonic = time.monotonic()
                        if (
                            current_time_monotonic - last_report_time
                            >= report_interval_seconds
                        ):
                            rate = (
                                files_since_last_report / report_interval_seconds
                                if report_interval_seconds > 0
                                else files_since_last_report
                            )
                            logger.info(
            ...
            logger.error(
                f"Outer error during Phase 1 os.walk for {root_path}: {outer_walk_err}",
                exc_info=True,
            )
            total_errors_this_root_scan += 1
    ...
    async def _embed_and_store(
        ...
        if not self.embedding_model:
            logger.error(
                f"{log_prefix}: Embedding model not available. Cannot proceed."
            )
        ...
        if not text_to_embed:
            logger.warning(
                f"{log_prefix}: No text content provided to embed. Marking as indexed_meta."
            )
        ...
        try:
            ...
            if not chunk_embeddings or len(chunk_embeddings) != len(chunks):
                raise ValueError(
                    "Embedding model returned empty or mismatched vectors."
                )
            ...
            if not global_file_vs:
                raise RuntimeError(
                    "Global file vector store is not available for updating."
                )
            ...
            record.index_status = (
                IndexStatusEnum.indexed_complete
                if is_final_embedding
                else IndexStatusEnum.indexed_text
            )
            record.processing_error = None  # Clear previous errors
            logger.success(
                f"✅ {log_prefix}: Successfully embedded {len(chunks)} chunks. Final Status: {record.index_status.value}"
            )

        except TaskInterruptedException as tie:
            logger.warning(
                f"🚦 {log_prefix}: Embedding INTERRUPTED: {tie}. Status will remain pending."
            )
            # Keep status as 'pending_embedding' to allow retry
            record.index_status = IndexStatusEnum.pending_embedding
        except Exception as e:
            logger.error(
                f"❌ {log_prefix}: Failed to embed or update vector store: {e}",
                exc_info=True,
            )
            record.index_status = IndexStatusEnum.error_embedding
    ...
    async def _embed_and_update_vector_store(
        self, record: FileIndex, db_session: Session
    ):
        """
        ...
        logger.info(
            f"--> {log_prefix}: Starting final embedding process for File ID {record.id}"
        )  # type: ignore
        ...
        try:
            ...
            logger.info(
                f"{log_prefix}: Split combined document into {len(chunks)} chunks."
            )
            if not chunks:
                raise ValueError("Text splitter produced no chunks.")
        except Exception as e_split:
            logger.error(
                f"{log_prefix}: Failed to split text for File ID {record.id}: {e_split}"
            )
            record.index_status = "error_embedding"
        ...
        try:
            ...
            if not chunk_embeddings or len(chunk_embeddings) != len(chunks):
                raise ValueError(
                    "Embedding model returned empty or mismatched number of vectors."
                )

            # 4. Update ChromaDB: Delete old entries and add new chunks
            logger.info(
                f"{log_prefix}: Embedding successful. Updating vector store for {len(chunks)} chunks..."
            )

            # Delete any existing vectors for this file ID to avoid duplicates
            await asyncio.to_thread(
                self.vectorstore.delete, where={"file_id": record.id}
            )
            ...
            record.processing_error = (
                f"Final embedding/Chroma update failed: {e_embed_update}"
            )
    ...
    def _extract_office_text(self, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            # --- Excel Spreadsheets (.xlsx, .xls) ---
                ...
                log_worker(
                    "WARNING",
                    f"{log_prefix}: Unsupported Office file type for text extraction: {file_ext}",
                )
                return None
            ...
            logger.error(
                f"{log_prefix}: Failed to extract text from Office file '{file_path}': {e}"
            )
        ...
        try:
            ...
            try:
                if not os.path.exists(file_path) or not os.path.isfile(file_path):
                    logger.error(
                        f"{log_prefix}: Path does not exist or is not a file. Skipping."
                    )
                    return
                ...
                if file_size is None:
                    raise PermissionError(
                        "Could not retrieve file metadata, likely a permission issue."
                    )
                ...
                logger.warning(
                    f"{log_prefix}: Stat failed or permission denied: {e_stat}. Logging error and skipping file."
                )
            ...
            if not current_md5 and file_size <= MAX_HASH_FILE_SIZE_BYTES:
                logger.warning(
                    f"{log_prefix}: MD5 calculation failed for a hashable file size. Skipping."
                )
                return

            existing_record: Optional[FileIndex] = (
                db_session.query(FileIndex)
                .filter(FileIndex.file_path == file_path)
                .first()
            )
            ...
            if not existing_record:
                ...
                hashes_match = (
                    current_md5 is not None and existing_record.md5_hash == current_md5
                )
                large_file_is_unchanged = (
                    current_md5 is None
                    and existing_record.md5_hash is None
                    and existing_record.size_bytes == file_size
                    and mtime_os_check
                    and existing_record.last_modified_os
                    and mtime_os_check <= existing_record.last_modified_os
                )
                if hashes_match or large_file_is_unchanged:
                    if (
                        not existing_record.indexed_content
                        and existing_record.index_status
                        not in [
                            IndexStatusEnum.error_read,
                            IndexStatusEnum.error_permission,
                            IndexStatusEnum.skipped_type,
                        ]
                    ):
                        needs_processing = True
                        logger.info(
                            f"-> {log_prefix}: Unchanged file is missing text content. Re-processing."
                        )
                    else:
                        logger.trace(
                            f"{log_prefix}: Unchanged and already processed. Skipping."
                        )
                        return
                else:
                    needs_processing = True
            ...
            if needs_processing:
                ...
                try:
                    if file_ext in OCR_TARGET_EXTENSIONS:
                        content = self._extract_text_with_ocr_fallback(
                            file_path, file_ext
                        )
                        if file_ext in VLM_TARGET_EXTENSIONS:
                            values_to_update["vlm_processing_status"] = "pending_vlm"
                        values_to_update["index_status"] = (
                            IndexStatusEnum.pending_embedding
                        )
                    ...
                    logger.warning(
                        f"{log_prefix} Permission denied during text extraction."
                    )
                    values_to_update["index_status"] = IndexStatusEnum.error_permission
                    values_to_update["processing_error"] = (
                        "Permission denied during text extraction."
                    )
                except Exception as e_extract:
                    logger.error(
                        f"{log_prefix} Error during text extraction: {e_extract}",
                        exc_info=True,
                    )
                    values_to_update["index_status"] = IndexStatusEnum.error_read
                    values_to_update["processing_error"] = (
                        f"Text extraction failed: {str(e_extract)[:255]}"
                    )

                values_to_update["last_indexed_db"] = datetime.datetime.now(
                    datetime.timezone.utc
                )
                ...
                if existing_record:
                    ...
                    db_session.execute(
                        update(FileIndex)
                        .where(FileIndex.id == existing_record.id)
                ...
                if content and record_to_embed:
                    logger.info(
                        f"{log_prefix}: Text extracted. Proceeding to initial embedding..."
                    )
                    await self._embed_and_store(
                        record_to_embed, content, db_session, is_final_embedding=False
                    )
                elif not content:
            ...
            logger.error(
                f"{log_prefix}: Unhandled error in Phase 1 for {file_path}: {e_phase1}",
                exc_info=True,
            )
    ...
    def _cleanup_llm_output(self, text: str, replacement_char: str = " ") -> str:
        ...
        start_token = (
            CHATML_START_TOKEN if "CHATML_START_TOKEN" in globals() else "<|im_start|>"
        )
        end_token = (
            CHATML_END_TOKEN if "CHATML_END_TOKEN" in globals() else "<|im_end|>"
        )
        ...
        sanitized = re.sub(
            rf"[{re.escape(replacement_char)}\s]+", replacement_char, sanitized
        )

        return sanitized.strip()

    async def _get_initial_vlm_description_async(
        self, image: Image.Image, session_id_for_log: str
    ) -> Tuple[Optional[str], Optional[str]]:
        """
        ...
        if self._wait_if_server_busy():
            return None, "[VLM Description Skipped - Server Busy]"
        if self.stop_event.is_set():
            return None, "[VLM Description Skipped - Stop Requested]"

        if not self.vlm_model:
            log_worker(
                "WARNING",
                "VLM model ('vlm' role) not available for initial description.",
            )
            return None, "[VLM Model Unavailable]"
        ...
        if width > MAX_DIMENSION or height > MAX_DIMENSION:
            ...
            new_height = int(height * scale)
            logger.trace(
                f"Resizing image from {width}x{height} to {new_width}x{new_height} for VLM."
            )
        ...
        try:
            ...
            if response_text and not (
                ...
                and "ERROR" in response_text
                and "Traceback" in response_text
            ):
                description_output = self._cleanup_llm_output(response_text.strip())
                logger.trace(
                    f"{log_prefix}: VLM description successful. Snippet: '{description_output[:100]}...'"
                )
            else:
                error_output = (
            ...
            logger.error(
                f"{log_prefix} VLM description call failed: {e}", exc_info=True
            )
    ...
    async def _process_pending_vlm_files(self, db_session: Session):
        ...
        if not self.vlm_model and not self.latex_model:
            logger.warning(
                "Skipping Phase 2 processing: VLM and/or LatexOCR models are not available."
            )
            return
        ...
        while (
            ...
            and processed_in_cycle < max_to_process_in_cycle
        ):
            if self._wait_if_server_busy():
            ...
            try:
                ...
                pending_vlm_files = (
                    db_session.query(FileIndex)
                    .filter(
                        FileIndex.vlm_processing_status.in_(
                            ["pending_vlm", "pending_conversion"]
                        )
                    )
                    .limit(10)
                    .all()
                ...
                logger.error(
                    f"Phase 2: Error querying for pending VLM files: {e_query}"
                )
                break

            if not pending_vlm_files:
                logger.info(
                    "Phase 2: No more files pending VLM/OCR processing in this batch."
                )
            ...
            for record in pending_vlm_files:
                if self.stop_event.is_set():
                    logger.info(
                        "Phase 2: Stop signal received during batch processing."
                    )
                    break
                ...
                logger.info(
                    f"--> {log_prefix}: Starting Phase 2 processing for File ID {record.id}"
                )
                # (Sorry this may make a ghost ELP0 session that cause a lockup overnight)
                # self._unload_all_models()
                ...
                try:
                    ...
                    if file_ext == ".pdf":
                        images = await asyncio.to_thread(
                            self._convert_pdf_to_images, record.file_path
                        )  # type: ignore
                    elif file_ext in OFFICE_EXTENSIONS:
                        temp_pdf_to_process = await asyncio.to_thread(
                            self._convert_office_to_pdf, record.file_path
                        )  # type: ignore
                        if temp_pdf_to_process and os.path.exists(temp_pdf_to_process):
                            images = await asyncio.to_thread(
                                self._convert_pdf_to_images, temp_pdf_to_process
                            )
                    elif (
                        file_ext in VLM_TARGET_EXTENSIONS
                    ):  # For single images like PNG, JPG
                        images = [Image.open(record.file_path)]  # type: ignore

                    if not images:
                        raise ValueError(
                            "Failed to convert or load file into images for VLM/OCR processing."
                        )
                        ...
                        try:
                            ...
                            if self.vlm_model:
                                (
                                    vlm_description,
                                    vlm_err,
                                ) = await self._get_initial_vlm_description_async(
                                    page_image, session_id_for_page
                                )
                                if vlm_err:
                                    page_errors.append(
                                        f"Page {page_num + 1} VLM Error: {vlm_err}"
                                    )
                                if vlm_description:
                                    page_vlm_descriptions.append(vlm_description)

                            # Get LaTeX math from specialized LatexOCR model
                            latex_math, latex_explanation = await asyncio.to_thread(
                                self._get_latex_from_image,
                                page_image,
                                session_id_for_page,
                            )
                            if latex_explanation and "[Error:" in latex_explanation:
                                page_errors.append(
                                    f"Page {page_num + 1} VLM-LaTeX Error: {latex_explanation}"
                                )
                            if latex_math:
                    ...
                    gc.collect()
                    logger.trace(
                        f"{log_prefix}: Manually triggered garbage collection after processing images."
                    )

                    # Step 3: Combine results and update the record in the database session
                    final_vlm_desc = (
                        "\n\n--- Page Break ---\n\n".join(page_vlm_descriptions).strip()
                        if page_vlm_descriptions
                        else None
                    )
                    final_latex_math = (
                        "\n\n".join(page_latex_ocr_outputs).strip()
                        if page_latex_ocr_outputs
                        else None
                    )
                    final_errors = "\n".join(page_errors) if page_errors else None

                    record.latex_representation = (
                        final_latex_math  # Column for specialized math OCR
                    )
                    record.latex_explanation = (
                        final_vlm_desc  # Column for general VLM description
                    )
                    record.processing_error = final_errors

                    if final_vlm_desc or final_latex_math:
                        record.vlm_processing_status = (
                            "success_vlm_ocr"
                            if not page_errors
                            else "partial_vlm_ocr_error"
                        )
                    else:
            ...
            if self.stop_event.is_set():
                break  # Exit outer while loop if stopped during batch
        ...
        while (
            ...
            try:
                ...
                pending_embedding_files = (
                    db_session.query(FileIndex)
                    .filter(FileIndex.index_status == "pending_embedding")
                    .limit(20)
                    .all()
                )  # Process in batches of 20
            except Exception as e_query_embed:
                logger.error(
                    f"Phase 3: Error querying for pending embedding files: {e_query_embed}"
                )
                break

            if not pending_embedding_files:
                logger.info(
                    "Phase 3: No more files pending final embedding in this batch."
                )
                break
            ...
            for record in pending_embedding_files:
                if self.stop_event.is_set():
                    logger.info(
                        "Phase 3: Stop signal received during batch processing."
                    )
                    break
                ...
                logger.info(
                    f"--> {log_prefix}: Starting final embedding for File ID {record.id}"
                )
    ...
    def run(self):
        ...
        logger.info(
            f"✅ {self.thread_name} started (Multi-Phase, Isolated Session Logic)."
        )
        ...
        try:
            loop.run_until_complete(
                initialize_global_file_index_vectorstore(self.provider)
            )
        except Exception as e_init_vs:
            logger.error(
                f"CRITICAL: Vector store loading failed: {e_init_vs}. Thread will exit."
            )
            return
        ...
        while not self.stop_event.is_set():
            ...
            try:
                with SessionLocal() as db_session:  # Session for Phase 1
                    logger.info(
                        f"--- {self.thread_name}: Starting Phase 1 (Scan & Text Extraction) ---"
                    )
                ...
                logger.error(
                    f"💥 Unhandled error in Phase 1: {e_phase1}", exc_info=True
                )
            ...
            try:
                with SessionLocal() as db_session:  # New Session for Phase 2
                    logger.info(
                        f"--- {self.thread_name}: Starting Phase 2 (VLM & LaTeX-OCR) ---"
                    )
                    loop.run_until_complete(self._process_pending_vlm_files(db_session))
            except Exception as e_phase2:
                logger.error(
                    f"💥 Unhandled error in Phase 2: {e_phase2}", exc_info=True
                )
            ...
            try:
                with SessionLocal() as db_session:  # New Session for Phase 3
                    logger.info(
                        f"--- {self.thread_name}: Starting Phase 3 (Final Embedding) ---"
                    )
                    loop.run_until_complete(
                        self._process_pending_embeddings(db_session)
                    )
            except Exception as e_phase3:
                logger.error(
                    f"💥 Unhandled error in Phase 3: {e_phase3}", exc_info=True
                )

            cycle_duration = time.monotonic() - cycle_start_time
            logger.info(
                f"--- {self.thread_name}: Full pipeline cycle finished in {cycle_duration:.2f} seconds. ---"
            )

            wait_time = FILE_INDEXER_IDLE_WAIT_SECONDS
            logger.debug(
                f"{self.thread_name} waiting for {wait_time}s before next full cycle..."
            )
        ...
        try:
            ...
            logger.warning(
                f"Error closing asyncio loop in {self.thread_name}: {e_loop_close}"
            )
...
def _locked_initialization_task(provider_ref: CortexEngine) -> Dict[str, Any]:
    ...
    for attempt in range(2):
        ...
        try:
            with _file_index_vs_init_lock:
                ...
                if _file_index_vs_initialized_event.is_set():
                    if global_file_index_vectorstore is not None:
                        ...
                        return {
                            "status": "skipped_already_initialized",
                ...
                logger.info(
                    ">>> FileIndex VS Init: Stage 0: Validating Provider and Embeddings. <<<"
                )
                ...
                if not _persist_dir_to_use or not _collection_name_to_use:
                    ...
                    task_message = (
                        "Chroma DB persist directory or collection name not configured."
                    )
                    logger.error(task_message)
                    initialization_succeeded_or_known_empty = True
                    global_file_index_vectorstore = None
                ...
                if os.path.exists(_persist_dir_to_use) and os.path.isdir(
                    _persist_dir_to_use
                ):
                    # Check for actual Chroma data files before attempting to load
                    if any(
                        fname.endswith((".sqlite3", ".duckdb", ".parquet"))
                        for fname in os.listdir(_persist_dir_to_use)
                    ):
                        logger.info(
                            f"Found persisted Chroma data files. Attempting to load collection..."
                        )
                        loaded_store = Chroma(
                        ...
                        logger.info(
                            f"No Chroma data files found in '{_persist_dir_to_use}'. Will build from SQL DB."
                        )
                else:
                    logger.info(
                        f"Persist directory '{_persist_dir_to_use}' does not exist. Will build from SQL DB."
                    )
                    os.makedirs(_persist_dir_to_use, exist_ok=True)

                if should_rebuild_from_sql:
                    logger.info(
                        ">>> FileIndex VS Init: Proceeding to rebuild from SQL Database. <<<"
                    )
                    db_session = SessionLocal()
                    if not db_session:
                        raise RuntimeError(
                            "Failed to create DB session for rebuild phase."
                        )

                    logger.info(
                        f">>> FileIndex VS Init: Stage 2: Querying SQL DB for records with embeddings... <<<"
                    )
                    ...
                    if not indexed_files:
                        logger.warning(
                            "No files with embeddings in SQL DB to rebuild. Creating empty persistent VS."
                        )
                        ...
                        for db_record in indexed_files:
                            try:
                                ...
                                emb_json_str = getattr(
                                    db_record, "embedding_json", None
                                )
                                content_str = getattr(
                                    db_record, "indexed_content", None
                                )

                                if (
                                    not emb_json_str
                                    or not content_str
                                    or not content_str.strip()
                                ):
                                    continue
                                ...
                                metadatas_for_vs.append(
                                    {
                                        "source": getattr(
                                            db_record, "file_path", "UnknownPath"
                                        ),
                                        "file_id": int(rec_id_val)
                                        if str(rec_id_val).isdigit()
                                        else -1,
                                        "file_name": getattr(
                                            db_record, "file_name", "UnknownFile"
                                        ),
                                        "last_modified": str(
                                            getattr(
                                                db_record, "last_modified_os", "N/A"
                                            )
                                        ),
                                        "index_status": str(
                                            getattr(
                                        ...
                                        "mime_type": getattr(
                                            db_record, "mime_type", "UnknownMIME"
                                        ),
                                    }
                                )
                        ...
                        if processed_records_for_chroma > 0:
                            ...
                            logger.warning(
                                "No valid data with embeddings from SQL. Creating empty persistent VS."
                            )
                            global_file_index_vectorstore = Chroma(
                                collection_name=_collection_name_to_use,
                                embedding_function=provider_ref.embeddings,
            ...
            if "An instance of Chroma already exists" in str(e) and attempt == 0:
                logger.warning(
                    f"Chroma conflict detected: {e}. Deleting directory and retrying..."
                )
                ...
                if _persist_dir_to_use and os.path.isdir(_persist_dir_to_use):
                    try:
                        ...
                        logger.info(
                            f"Successfully deleted Chroma directory: {_persist_dir_to_use}"
                        )
                ...
                task_message = (
                    f"CRITICAL ERROR during FileIndex VS init (ValueError): {e}"
                )
            ...
            if db_session:
                try:
                    ...
                    logger.warning(
                        f"Error closing DB session in FileIndex VS init: {e_close}"
                    )
            ...
            if attempt == 1 and not initialization_succeeded_or_known_empty:
                logger.error(
                    ">>> All initialization attempts failed. The vector store will be unavailable. <<<"
                )
    ...
    if initialization_succeeded_or_known_empty:
        if not _file_index_vs_initialized_event.is_set():
            _file_index_vs_initialized_event.set()
            logger.info(
                f">>> SET _file_index_vs_initialized_event. Final Status: {task_status}. <<<"
            )
    else:
        logger.error(
            f">>> _file_index_vs_initialized_event NOT SET due to critical error. Status: {task_status}. <<<"
        )

    logger.info(
        f">>> FileIndex VS Init: Task finished. Final Status: '{task_status}', Message: \"{task_message}\" <<<"
    )
    return {"status": task_status, "message": task_message}
...
async def initialize_global_file_index_vectorstore(provider: CortexEngine):
    ...
    logger.info(
        ">>> Entered initialize_global_file_index_vectorstore (async version) <<<"
    )

    if not provider:
        logger.error(
            "  INIT_VS_ERROR (Outer): CortexEngine instance is None! Cannot initialize file index vector store."
        )
        return
    if not provider.embeddings:
        ...
        return
    logger.info(
        f"  INIT_VS_INFO (Outer): Using CortexEngine embeddings of type: {type(provider.embeddings)}"
    )

    if _file_index_vs_initialized_event.is_set():
        logger.info(
            ">>> SKIPPING FileIndex VS Init (Outer): _file_index_vs_initialized_event is ALREADY SET. <<<"
        )
        return
    ...
    try:
        ...
        logger.error(
            f"Exception running _locked_initialization_task via asyncio.to_thread: {e_thread_task}"
        )
        logger.exception(
            "FileIndex VS _locked_initialization_task Thread Execution Traceback:"
        )
...
def get_global_file_index_vectorstore(timeout_seconds=60) -> Optional[Chroma]:
    if not _file_index_vs_initialized_event.is_set():
        logger.warning(
            f"Global file index VS not ready, waiting up to {timeout_seconds}s for initialization event..."
        )
        event_was_set = _file_index_vs_initialized_event.wait(timeout=timeout_seconds)
        if not event_was_set:
            logger.error(
                f"Timeout waiting for file index VS initialization after {timeout_seconds}s."
            )
            return None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test",
        action="store_true",
        help="Run test initialization and optional search.",
    )
    parser.add_argument(
        "--search_query",
        type=str,
        default=None,
        help="Query string to search the file index vector store.",
    )
    ...
    if cli_args.test:
        ...
        )  # Add a new one with DEBUG level for testing
        ...
        try:
            ...
            asyncio.run(
                initialize_global_file_index_vectorstore(test_cortex_backbone_provider)
            )  # Run the async init
            logger.info(
                "Global file index vector store initialization process completed."
            )
        except Exception as e_vs_init:
            logger.error(
                f"Error during file index vector store initialization: {e_vs_init}"
            )
            sys.exit(1)

        # 4. Get the vector store instance
        file_vs = get_global_file_index_vectorstore(
            timeout_seconds=10
        )  # Wait a bit if still initializing
        if file_vs:
            logger.success(
                "Successfully retrieved global file index vector store instance."
            )
            try:
                collection_count = file_vs._collection.count()
                logger.info(
                    f"File index Chroma collection contains {collection_count} items."
                )
            ...
            if cli_args.search_query:
                ...
                try:
                    ...
                    search_results = file_vs.similarity_search_with_relevance_scores(
                        query, k=5
            ...
            logger.error(
                "Failed to get global file index vector store instance after initialization attempt."
            )

        logger.info("--- File Indexer Test Mode Finished ---")
    else:
        print(
            'Run with --test to initialize and optionally --search_query "your query" to search.'
        )
