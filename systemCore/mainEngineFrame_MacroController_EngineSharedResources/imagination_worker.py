...
# if imagination_worker.py and config.py are in the same directory (e.g., mainEngineFrame_MacroController_EngineSharedResources).
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
from CortexConfiguration import *

# Using print directly here as log_worker isn't defined yet.
print(
    f"[IMAGINATION_WORKER|DEBUG] Worker SCRIPT_DIR: {SCRIPT_DIR}",
    file=sys.stderr,
    flush=True,
)
if SCRIPT_DIR not in sys.path:
    ...
    print(
        f"[IMAGINATION_WORKER|INFO] Added {SCRIPT_DIR} to sys.path for worker.",
        file=sys.stderr,
        flush=True,
    )
else:
    print(
        f"[IMAGINATION_WORKER|DEBUG] {SCRIPT_DIR} already in sys.path for worker.",
        file=sys.stderr,
        flush=True,
    )
...
try:
    ...
    if load_dotenv():  # Will load .env from CWD or find_dotenv()
        print(
            "[IMAGINATION_WORKER|INFO] .env file processed by worker using load_dotenv().",
            file=sys.stderr,
            flush=True,
        )
    else:
    ...
    print(
        "[IMAGINATION_WORKER|WARNING] python-dotenv not installed, .env file will not be loaded directly by worker.",
        file=sys.stderr,
        flush=True,
    )
except Exception as e_dotenv:
    print(
        f"[IMAGINATION_WORKER|ERROR] Error during load_dotenv() in worker: {e_dotenv}",
        file=sys.stderr,
        flush=True,
    )
...
try:
    ...
    if hasattr(Image.SAVE, "AVIF"):  # Check if AVIF is already registered
        ...
        print(
            "[IMAGINATION_WORKER|INFO] Pillow AVIF save format detected as potentially available.",
            file=sys.stderr,
            flush=True,
        )
        ...
        try:
            ...
            PIL_AVIF_PLUGIN_POSSIBLY_AVAILABLE = True
            print(
                "[IMAGINATION_WORKER|INFO] Pillow AVIF plugin dynamically registered and potentially available.",
                file=sys.stderr,
                flush=True,
            )
        except ImportError:
            ...
            print(
                "[IMAGINATION_WORKER|WARNING] pillow-avif-plugin not found. AVIF conversion will be skipped.",
                file=sys.stderr,
                flush=True,
            )
except ImportError:
    print(
        "[IMAGINATION_WORKER|ERROR] Pillow (PIL) not found in imagination_worker environment.",
        file=sys.stderr,
        flush=True,
    )
    sys.exit(1)
...
try:
    ...
    print(
        "[IMAGINATION_WORKER|ERROR] stable-diffusion-cpp library not found.",
        file=sys.stderr,
        flush=True,
    )
...
try:
    ...
    from CortexConfiguration import REFINEMENT_CFG_SCALE as cfg_REFINEMENT_CFG_SCALE
    from CortexConfiguration import (
        REFINEMENT_MODEL_ENABLED as cfg_REFINEMENT_MODEL_ENABLED,
    )
    from CortexConfiguration import REFINEMENT_MODEL_NAME as cfg_REFINEMENT_MODEL_NAME
    from CortexConfiguration import (
    ...
    from CortexConfiguration import REFINEMENT_STRENGTH as cfg_REFINEMENT_STRENGTH
    ...
    print(
        "[IMAGINATION_WORKER|INFO] Successfully imported refinement settings from CortexConfiguration.py.",
        file=sys.stderr,
        flush=True,
    )
...
def _apply_watermark(
    ...
    try:
        if font_path and os.path.exists(font_path):
            ...
            if font_path:
                log_worker(
                    "WARNING",
                    f"Watermark font not found at '{font_path}'. Using basic default font.",
                )
    except Exception as e:
        log_worker(
            "ERROR", f"Failed to load watermark font: {e}. Using basic default font."
        )
        font = ImageFont.load_default()
    ...
    for y in range(0, img_height + tile_height, tile_height * 2):  # Increased spacing
        for x in range(
            0, img_width + tile_width, tile_width + int(tile_width * 0.5)
        ):  # Increased spacing
            overlay.paste(rotated_text_img, (x, y), rotated_text_img)
            overlay.paste(
                rotated_text_img,
                (x - int(tile_width * 0.75), y - tile_height),
                rotated_text_img,
            )  # Offset rows
...
def main():
    ...
    parser.add_argument(
        "--model-dir",
        required=True,
        help="Base directory where model files are located.",
    )
    parser.add_argument(
        "--diffusion-model-name",
        default="flux1-schnell.gguf",
    ...
    parser.add_argument(
        "--clip-l-name",
        default="flux1-clip_l.gguf",
        help="Filename of the FLUX CLIP L GGUF model.",
    )
    parser.add_argument(
        "--t5xxl-name",
        default="flux1-t5xxl.gguf",
        help="Filename of the FLUX T5 XXL GGUF model.",
    )
    parser.add_argument(
        "--vae-name",
        default="flux1-ae.gguf",
        help="Filename of the FLUX VAE GGUF model.",
    )
    parser.add_argument(
        "--w-device",
        default="default",
    ...
    parser.add_argument(
        "--test-mode",
        action="store_true",
        help="Run in test mode with fixed parameters.",
    )
    parser.add_argument(
        "--test-prompt",
        default="photo of a majestic cat king sitting on a throne, epic fantasy art",
        help="Prompt for test mode.",
    )
    parser.add_argument(
        "--test-output-file",
        default="zephyImagination_test_image.png",
    ...
    if not STABLE_DIFFUSION_CPP_AVAILABLE:
        log_worker(
            "CRITICAL",
            "stable-diffusion-cpp library failed to import. Worker cannot run.",
        )
        print(
            json.dumps(
                {
                    "error": "stable-diffusion-cpp library not available in worker environment"
                }
            ),
            flush=True,
        )
    ...
    log_worker(
        "INFO",
        f"Initial Refinement Stage Enabled (from CortexConfiguration/default): {current_refinement_enabled_status}",
    )
    if current_refinement_enabled_status:
        log_worker(
            "INFO",
            f"Refinement Noise Strength (from CortexConfiguration/default): {REFINEMENT_ADD_NOISE_STRENGTH}",
        )

    if PIL_AVIF_PLUGIN_POSSIBLY_AVAILABLE:
        log_worker(
            "INFO",
            "AVIF plugin seems available, AVIF conversion will be attempted if enabled by format.",
        )
    else:
        log_worker(
            "WARNING",
            "AVIF plugin not available or not detected, AVIF conversion will be skipped.",
        )
    ...
    if args.n_threads > 0:
        ...
        if cpu_count is not None and cpu_count > 0:
            ...
            log_worker(
                "DEBUG",
                f"Auto-detected n_threads using os.cpu_count(): {n_threads_for_sd_constructor}",
            )
        else:
    ...
    log_worker(
        "INFO",
        f"Using n_threads={n_threads_for_sd_constructor} for StableDiffusion instances.",
    )
    ...
    try:
        ...
        paths_to_check_flux = {
            "FLUX Diffusion Model": flux_diffusion_path,
            "FLUX CLIP L Model": flux_clip_l_path,
            "FLUX T5 XXL Model": flux_t5xxl_path,
            "FLUX VAE Model": flux_vae_path,
        }
        ...
        print(
            json.dumps(
                {"error": f"Worker critical error: FLUX model file missing: {fe_flux}"}
            ),
            flush=True,
        )
        sys.exit(1)
        ...
        print(
            json.dumps(
                {
                    "error": f"Worker critical error: Failed to load FLUX model: {e_flux_load}"
                }
            ),
            flush=True,
        )
    ...
    if args.test_mode:
        ...
        if current_refinement_enabled_status:
            try:
                ...
                refinement_model_path_test = os.path.join(
                    args.model_dir, REFINEMENT_MODEL_NAME
                )
                if not os.path.exists(refinement_model_path_test):
                    raise FileNotFoundError(
                        f"Refinement model for test not found at {refinement_model_path_test}"
                    )
        ...
        log_worker(
            "INFO",
            f"[Test Mode] Stage 1: Generating FLUX image with prompt: '{current_prompt_for_test}'",
        )
        stage1_test_images = sd_flux_instance.generate_image(
            prompt=current_prompt_for_test,
            negative_prompt="Bad Morphed Graphic, ugly, disfigured",
            sample_method="euler",
            sample_steps=flux_sample_steps_for_test,
            cfg_scale=1.0,
            width=768,
            height=448,
            seed=-1,
        ...
        if not stage1_test_images:
            log_worker(
                "ERROR",
                "[Test Mode] Stage 1 FLUX generation failed to produce an image.",
            )
            sys.exit(1)
        ...
        log_worker(
            "INFO",
            f"[Test Mode] Stage 1 FLUX image generated. Size: {final_test_image.size}",
        )
        ...
        if current_refinement_enabled_status and sd_refiner_instance_test:
            ...
            if REFINEMENT_ADD_NOISE_STRENGTH > 0:
                try:
                    ...
                    img_array_test = np.array(stage1_image_for_refiner_test).astype(
                        np.float32
                    )
                    noise_test = np.random.normal(
                        0, REFINEMENT_ADD_NOISE_STRENGTH, img_array_test.shape
                    )
                    noisy_img_array_test = np.clip(img_array_test + noise_test, 0, 255)
                    stage1_image_for_refiner_test = Image.fromarray(
                        noisy_img_array_test.astype(np.uint8), "RGB"
                    )
            ...
            refiner_prompt_for_test = (
                REFINEMENT_PROMPT_PREFIX
                + current_prompt_for_test
            ...
            try:
                refined_test_images = sd_refiner_instance_test.generate_image(
                    init_image=stage1_image_for_refiner_test,
                    prompt=refiner_prompt_for_test,
                    negative_prompt="blurry, low quality, noisy, grain",
                    sample_method=REFINEMENT_SAMPLE_METHOD,
                    sample_steps=refiner_steps_for_test,
                    cfg_scale=REFINEMENT_CFG_SCALE,
                ...
                if refined_test_images:
                    ...
                    log_worker(
                        "INFO",
                        f"[Test Mode] Stage 2 Refinement successful. Size: {final_test_image.size}",
                    )
                else:
                    log_worker(
                        "WARNING",
                        "[Test Mode] Refinement failed or returned no image. Using Stage 1 image.",
                    )
        ...
        try:
            ...
            print(
                json.dumps(
                    {
                        "result": {
                            "status": "Test image generated",
                            "file": output_file_path_test,
                        }
                    }
                ),
                flush=True,
            ...
            print(
                json.dumps({"error": f"Test mode image save error: {save_err}"}),
                flush=True,
            )
        sys.exit(0)
    ...
    try:
        ...
        negative_prompt_stage1 = request_data.get(
            "negative_prompt", "Bad Morphed Graphic, ugly, disfigured, blurry"
        )
        ...
        if not original_prompt_from_request:
        ...
        if task_type == "txt2img":
            ...
            stage1_pil_images_list = sd_flux_instance.generate_image(
                prompt=original_prompt_from_request,
                negative_prompt=negative_prompt_stage1,
                sample_method=sample_method_flux,
                sample_steps=sample_steps_flux,
                cfg_scale=cfg_scale_flux,
                width=width,
                height=height,
                seed=seed,
            )
        elif task_type == "img2img":
            if not input_image_b64_for_stage1:
                raise ValueError("Missing 'input_image_b64' for Stage 1 img2img task.")
            img_bytes_stage1 = base64.b64decode(input_image_b64_for_stage1)
            input_pil_image_stage1 = Image.open(BytesIO(img_bytes_stage1)).convert(
                "RGB"
            )
            strength_stage1 = float(request_data.get("strength", 0.65))
            log_worker(
                "INFO",
                f"[Stage 1] Executing FLUX img_to_img... Strength: {strength_stage1}",
            )
            stage1_pil_images_list = sd_flux_instance.generate_image(
                init_image=input_pil_image_stage1,
                prompt=original_prompt_from_request,
                negative_prompt=negative_prompt_stage1,
                sample_method=sample_method_flux,
                sample_steps=sample_steps_flux,
                cfg_scale=cfg_scale_flux,
                strength=strength_stage1,
                seed=seed,
                width=width,
                height=height,
            )
        ...
        if current_refinement_enabled_status:
            log_worker(
                "INFO",
                "[Transition] Unloading FLUX model to free memory for Refiner...",
            )
            ...
            log_worker(
                "INFO",
                "[Transition] FLUX model instance deleted. Waiting briefly for resources to release...",
            )
            time.sleep(1.5)  # Slightly adjusted wait time
            try:
                log_worker(
                    "INFO",
                    f"[Stage 2] Loading Refinement Model ({REFINEMENT_MODEL_NAME})...",
                )
                refinement_model_path_main = os.path.join(
                    args.model_dir, REFINEMENT_MODEL_NAME
                )
                if not os.path.exists(refinement_model_path_main):
                    raise FileNotFoundError(
                        f"Refinement model for main task not found at {refinement_model_path_main}"
                    )
        ...
        if current_refinement_enabled_status and sd_refiner_instance_main:
            ...
            if REFINEMENT_ADD_NOISE_STRENGTH > 0:
                try:
                    ...
                    noise = np.random.normal(
                        0, REFINEMENT_ADD_NOISE_STRENGTH, img_array.shape
                    )
                    noisy_img_array = np.clip(img_array + noise, 0, 255)
                    stage1_image_for_refiner = Image.fromarray(
                        noisy_img_array.astype(np.uint8), "RGB"
                    )
                    log_worker(
                        "INFO",
            ...
            try:
                refined_pil_images_list = sd_refiner_instance_main.generate_image(
                    init_image=stage1_image_for_refiner,
                    prompt=refiner_input_prompt,
                    negative_prompt=refiner_negative_prompt,
                    sample_method=REFINEMENT_SAMPLE_METHOD,
                    sample_steps=refiner_num_sample_steps,
                    cfg_scale=REFINEMENT_CFG_SCALE,
                    strength=REFINEMENT_STRENGTH,
                    seed=seed + 1 if seed != -1 else -1,  # Vary seed
                    width=width,
                    height=height,
                )
                log_worker(
                ...
                if refined_pil_images_list:
                    ...
                    log_worker(
                        "INFO",
                        "[Stage 2] Refinement successful. Using refined image as final output.",
                    )
                else:
                    log_worker(
                        "WARNING",
            ...
            if current_refinement_enabled_status and not sd_refiner_instance_main:
                log_worker(
                    "INFO",
                    "Refinement stage skipped (Refiner model failed to load). Using Stage 1 image.",
                )
            elif not current_refinement_enabled_status:
                log_worker(
                    "INFO",
                    "Refinement stage skipped (disabled by config/default). Using Stage 1 image.",
                )
        ...
        if response_format == "b64_json":
            ...
            try:
                ...
                current_image_item_data["b64_json"] = base64.b64encode(
                    png_byte_buffer.getvalue()
                ).decode("utf-8")
                if PIL_AVIF_PLUGIN_POSSIBLY_AVAILABLE:
                    try:
                        avif_byte_buffer = BytesIO()
                        final_output_image.save(
                            avif_byte_buffer, format="AVIF", quality=80
                        )
                        current_image_item_data["b64_avif"] = base64.b64encode(
                            avif_byte_buffer.getvalue()
                        ).decode("utf-8")
                    except Exception as avif_e:
                        current_image_item_data["b64_avif"] = None
                        log_worker("WARNING", f"AVIF encoding failed: {avif_e}")
                else:
                    current_image_item_data["b64_avif"] = None
                ...
                log_worker(
                    "ERROR",
                    f"Final image encoding (PNG/AVIF) failed: {final_encode_error}",
                )
                output_data_list_final.append(
                    {"error": f"Final image encoding failed: {final_encode_error}"}
                )
        elif response_format == "url":
            log_worker(
                "WARNING",
                "response_format 'url' requested, but worker returns b64_json as fallback.",
            )
            current_image_item_data = {
                "comment": "URL format requested, returning b64_json as fallback."
            }
            try:
                png_byte_buffer = BytesIO()
                final_output_image.save(png_byte_buffer, format="PNG")
                    ...
                    try:
                        ...
                        if PIL_AVIF_PLUGIN_POSSIBLY_AVAILABLE:
                            try:
                                ...
                                final_output_image.save(
                                    avif_byte_buffer, format="AVIF", quality=40
                                )
                                current_image_item_data["b64_avif"] = base64.b64encode(
                                    avif_byte_buffer.getvalue()
                                ).decode("utf-8")
                                log_worker(
                                    "INFO",
                                    "Successfully encoded final image to AVIF with quality=40.",
                                )
                            except Exception as avif_e:
                                current_image_item_data["b64_avif"] = None
                                log_worker(
                                    "WARNING",
                                    f"AVIF encoding failed with quality=40: {avif_e}",
                                )
                        else:
                            current_image_item_data["b64_avif"] = (
                                None  # Indicate AVIF is not available
                            )
                        ...
                        log_worker(
                            "ERROR",
                            f"Final image encoding (PNG) failed: {final_encode_error}",
                        )
                        output_data_list_final.append(
                            {
                                "error": f"Final image encoding failed: {final_encode_error}"
                ...
                output_data_list_final.append(
                    {"error": "Encoding fallback for URL format failed."}
                )
        else:
        ...
        if not output_data_list_final or "error" in output_data_list_final[0]:
            error_detail = (
                output_data_list_final[0].get(
                    "error", "Unknown error during final image processing."
                )
                if output_data_list_final
                else "No image data processed."
            )
            result_payload = {
                "error": f"Failed to prepare final image output: {error_detail}"
            }
        else:
            result_payload = {
                "created": int(time.time()),
                "data": output_data_list_final,
            }
        ...
        log_worker("ERROR", f"JSON Decode Error: {e}")
        result_payload = {"error": f"Worker JSON Decode Error: {e}"}
        ...
        result_payload = {"error": f"Worker Image Gen Error: {e}"}
    except Exception as e:
        log_worker("ERROR", f"Unexpected Error: {e}\n{traceback.format_exc()}")
        result_payload = {"error": f"Worker Unexpected Error: {e}"}

    try:
        output_json_str_final = json.dumps(result_payload)
        log_worker(
            "DEBUG",
            f"Sending output JSON (len={len(output_json_str_final)}): {output_json_str_final[:200]}...",
        )
        print(output_json_str_final, flush=True)
        log_worker("INFO", "Result/Error sent to stdout.")
    except Exception as e_print_final:
        log_worker(
            "ERROR", f"Failed to serialize/write result to stdout: {e_print_final}"
        )
        ...
        try:
            ...
            log_worker(
                "ERROR",
                f"Failed to even serialize the fallback critical error JSON: {final_json_err_print}",
            )
            raw_err_msg_print = (
    ...
    final_exit_code = 0
    if (
        "e_print_final" in locals()
        and e_print_final is not None
        and "error" not in result_payload
    ):
        final_exit_code = 1
