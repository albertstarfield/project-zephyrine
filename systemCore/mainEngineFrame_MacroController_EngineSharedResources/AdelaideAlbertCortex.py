...
try:
    ...
    try:
        ...
        ocr_reader = None
        logger.error(
            f"❌ Failed to initialize EasyOCR Reader. OCR functionality will be disabled. Error: {e}"
        )
...
try:
    ...
    logger.error(
        f"❌ Failed to import Selenium/WebDriverManager: {e}. Web scraping/download tools disabled."
    )
    logger.error(
        "   Install dependencies: pip install selenium webdriver-manager requests beautifulsoup4"
    )
...
try:
    ...
    logger.warning(
        "⚠️ thefuzz not installed. Fuzzy search RAG fallback disabled. Run 'pip install thefuzz python-Levenshtein'."
    )
...
try:
    from CortexConfiguration import (
        ENABLE_STELLA_ICARUS_HOOKS,  # Ensure this is imported
    )
    from stella_icarus_utils import (  # <<< NEW IMPORT
        StellaIcarusAdaDaemonManager,
        StellaIcarusHookManager,
    )
except ImportError as e:
...
try:
    ...
    if MAX_CONCURRENT_BACKGROUND_GENERATE_TASKS <= 0:
        logger.warning(
            f"MAX_CONCURRENT_BACKGROUND_GENERATE_TASKS was <= 0, defaulting to {_default_max_bg_tasks}"
        )
        MAX_CONCURRENT_BACKGROUND_GENERATE_TASKS = _default_max_bg_tasks
except ValueError:
    logger.warning(
        f"Invalid value for MAX_CONCURRENT_BACKGROUND_GENERATE_TASKS env var, defaulting to {_default_max_bg_tasks}"
    )
    MAX_CONCURRENT_BACKGROUND_GENERATE_TASKS = _default_max_bg_tasks

logger.info(
    f"🚦 Max concurrent background generate tasks: {MAX_CONCURRENT_BACKGROUND_GENERATE_TASKS}"
)
background_generate_task_semaphore = asyncio.Semaphore(
...
logger.debug(
    f"Columns found by SQLAlchemy: {[c.name for c in Interaction.__table__.columns]}"
)
if "tot_delivered" in [c.name for c in Interaction.__table__.columns]:
    logger.debug("✅ 'tot_delivered' column IS present.")
else:
    logger.error("❌ 'tot_delivered' column IS STILL MISSING!")
logger.debug("-------------------------------------------------------------")
...
try:
    ...
    logger.warning(
        "tiktoken not available in AdelaideAlbertCortex. Context truncation will be less accurate (char-based)."
    )
...
)  # Get the directory containing AdelaideAlbertCortex
# --- Use SCRIPT_DIR to define the source path ---
ASSISTANT_PROXY_SOURCE_PATH = os.path.join(
    SCRIPT_DIR, "AssistantProxy.applescript"
)  # Assumes script is next to AdelaideAlbertCortex
...
try:
    ...
    if "tot_delivered" in [c.name for c in Interaction.__table__.columns]:
        ...
        logger.error(
            "❌ 'tot_delivered' column IS MISSING from mapped model BEFORE init_db!"
        )
    logger.debug("---------------------------------------------------------")
...
def start_file_indexer():
    ...
    if not FileIndexer:
        logger.error(
            "Cannot start file indexer: FileIndexer class not available (import failed?)."
        )
        return
    if (
        not cortex_backbone_provider
    ):  # <<< Check if CortexEngine initialized successfully
        logger.error(
            "Cannot start file indexer: CortexEngine (and embedding model) not available."
        )
        return
    ...
    if not embedding_model:
        logger.error(
            "Cannot start file indexer: Embedding model not found within CortexEngine."
        )
        return
    ...
    if _indexer_thread is None or not _indexer_thread.is_alive():
        ...
        try:
            ...
            _indexer_thread = threading.Thread(
                target=indexer_instance.run, name="FileIndexerThread", daemon=True
            )
            _indexer_thread.start()
            logger.success("✅ File indexer thread started successfully.")
        except Exception as e:
            logger.critical(
                f"🔥🔥 Failed to instantiate or start FileIndexer thread: {e}"
            )
...
_reflector_lock = (
    threading.Lock()
)  # Lock to prevent concurrent reflection cycles if one runs long
...
def run_self_reflection_loop():
    ...
    if not cortex_backbone_provider or not cortex_text_interaction:
        logger.error(
            f"🛑 {thread_name}: CortexEngine or CortexThoughts not initialized. Cannot run reflection."
        )
        return
    ...
    logger.info(
        f"{thread_name}: Eligible Input Types for new reflection: {REFLECTION_ELIGIBLE_INPUT_TYPES}"
    )
    ...
    while not _reflector_stop_event.is_set():
        ...
        if not _reflector_lock.acquire(blocking=False):
            logger.warning(
                f"{thread_name}: Previous reflection cycle lock still held? Skipping this cycle attempt."
            )
            _reflector_stop_event.wait(
        ...
        try:
            ...
            while server_is_busy_event.is_set():
                if not was_busy_waiting:
                    logger.info(
                        f"🚦 {thread_name}: Server busy, pausing reflection start..."
                    )
                    was_busy_waiting = True
                if _reflector_stop_event.is_set():
                    break
                if _reflector_stop_event.wait(timeout=0.5):
                    break
            if was_busy_waiting:
                logger.info(
                    f"🟢 {thread_name}: Server free, resuming reflection cycle."
                )
            if _reflector_stop_event.is_set():
                break
            ...
            if not db:
                raise RuntimeError("Failed to create DB session for reflection cycle.")
            ...
            while not _reflector_stop_event.is_set():
                ...
                try:
                    ...
                    logger.debug(
                        f"{thread_name}: Querying for pending Socratic questions..."
                    )
                    socratic_query = (
                        ...
                        .filter(
                            ...
                            == "socratic_thought",  # <-- Look for our specific classification
                        )
                    ...
                    if interactions_to_reflect:
                        logger.info(
                            f"{thread_name}: Found {len(interactions_to_reflect)} Socratic question(s) to reflect on."
                        )
                    else:
                        # Stage 2: Fallback to other eligible types if no Socratic questions are pending.
                        logger.debug(
                            f"{thread_name}: No Socratic questions found. Falling back to general reflection query."
                        )
                        fallback_query = (
                            db.query(Interaction)
                            .filter(
                    ...
                    logger.error(
                        f"{thread_name}: Error querying interactions for reflection: {query_err}"
                    )
                ...
                if not interactions_to_reflect:
                    logger.debug(
                        f"{thread_name}: No new eligible interactions found in this batch/query."
                    )
                ...
                for (
                    interaction_obj
                ) in interactions_to_reflect:  # Renamed to avoid conflict
                    if _reflector_stop_event.is_set():
                        logger.info(f"{thread_name}: Stop signal during batch.")
                        break
                    if server_is_busy_event.is_set():
                        logger.info(
                            f"{thread_name}: Server became busy, pausing batch."
                        )
                        break

                    original_input_text = ""
                    if (
                        isinstance(interaction_obj.user_input, str)
                        and interaction_obj.user_input.count(CHATML_START_TOKEN) >= 2
                    ):
                        original_input_text = interaction_obj.user_input
                    else:
                        original_input_text = (
                            interaction_obj.llm_response
                            or interaction_obj.user_input
                    ...
                    socratic_task_prompt = (
                        cortex_text_interaction._transform_text_to_socratic_task(
                    ...
                    reflection_session_id_for_bg = (
                        f"reflection_{interaction_obj.id}_{str(uuid.uuid4())[:4]}"
                    )
                    ...
                    try:
                        ...
                        task_launched_successfully = (
                            True  # Indicates background_generate ran
                        )
                        ...
                        logger.exception(
                            f"{thread_name} Background Generate Trigger/Run Traceback:"
                        )
                    ...
                    if (
                        not _reflector_stop_event.is_set()
                        and ACTIVE_CYCLE_PAUSE_SECONDS > 0
                    ):
                        _reflector_stop_event.wait(timeout=ACTIVE_CYCLE_PAUSE_SECONDS)

                total_processed_this_active_cycle += (
                    batch_processed_count_this_inner_loop
                )
                logger.info(
                ...
                if _reflector_stop_event.is_set() or server_is_busy_event.is_set():
                    break
                # --- End of Inner Loop for NEW interactions ---

            # --- "Gabut State" - Proactive Re-Reflection Logic ---
            if (
                not work_found_in_this_cycle
                and ENABLE_PROACTIVE_RE_REFLECTION
                and random.random() < PROACTIVE_RE_REFLECTION_CHANCE
            ):
                logger.info(
                    f"{thread_name}: Gabut State - No new work. Attempting proactive re-reflection."
                )
                ...
                try:
                    ...
                    if candidate_for_re_reflection:
                        ...
                        work_found_in_this_cycle = (
                            True  # Signal that work was "found" for next cycle logic
                        )
                        logger.info(
                        ...
                        logger.info(
                            f"{thread_name}: No suitable old interactions found for proactive re-reflection."
                        )
                except Exception as e_re_reflect:
                    logger.error(
                        f"{thread_name}: Error during proactive re-reflection: {e_re_reflect}"
                    )
                    if db:
                        db.rollback()
            ...
            logger.error(
                f"💥 {thread_name}: Runtime error in reflection cycle (likely DB session): {rt_err}"
            )
        except Exception as cycle_err:
            logger.error(
                f"💥 {thread_name}: Unhandled error during active reflection cycle: {cycle_err}"
            )
            ...
            if db:
                try:
                    ...
                    logger.error(
                        f"{thread_name}: Error closing DB session: {close_err}"
                    )
        ...
        wait_time_seconds = (
            IDLE_WAIT_SECONDS
            if not work_found_in_this_cycle
            else ACTIVE_CYCLE_PAUSE_SECONDS
        )
        if wait_time_seconds > 0 and not _reflector_stop_event.is_set():
            logger.debug(
                f"{thread_name}: Waiting {wait_time_seconds:.2f} seconds before next cycle check..."
            )
            stopped_during_wait = _reflector_stop_event.wait(timeout=wait_time_seconds)
            if stopped_during_wait:
                logger.info(f"{thread_name}: Stop signal received during idle wait.")
                break
    ...
    if _reflector_thread is None or not _reflector_thread.is_alive():
        ...
        try:
            ...
            _reflector_thread = threading.Thread(
                target=run_self_reflection_loop, name="SelfReflectorThread", daemon=True
            )
...
def stop_self_reflector():
    ...
    global _reflector_thread
    if not ENABLE_SELF_REFLECTION:
        return  # Don't try to stop if disabled
...
def shutdown_app_services():
    ...
    if (
        "cortex_backbone_provider" in globals()
        and cortex_backbone_provider
        and hasattr(cortex_backbone_provider, "_priority_quota_lock")
    ):
        if hasattr(
...
def _extract_json_candidate_string(
    raw_llm_text: str, log_prefix: str = "JSONExtract"
) -> Optional[str]:
    """
    ...
    logger.trace(
        f"{log_prefix}: Starting JSON candidate extraction from raw text (len {len(raw_llm_text)})."
    )

    # 1. Remove <think> tags and ChatML tokens first
    # Remove <think> tags
    text_cleaned = re.sub(
        r"<think>.*?</think>", "", raw_llm_text, flags=re.DOTALL | re.IGNORECASE
    )
    ...
    if not text_after_think_removal:
        logger.trace(
            f"{log_prefix}: Text empty after <think> and ChatML token removal."
        )
        return None
    ...
    if CHATML_END_TOKEN and cleaned_text.endswith(
        CHATML_END_TOKEN
    ):  # CHATML_END_TOKEN from CortexConfiguration
        cleaned_text = cleaned_text[: -len(CHATML_END_TOKEN)].strip()

    # 3. Look for JSON within markdown code blocks (```json ... ```)
    json_markdown_match = re.search(
        r"```json\s*(\{[\s\S]*?\})\s*```", cleaned_text, re.DOTALL
    )
    if json_markdown_match:
        extracted_str = json_markdown_match.group(1).strip()
        logger.trace(
            f"{log_prefix}: Extracted JSON from markdown block: '{extracted_str[:100]}...'"
        )
    ...
    if first_brace != -1 and last_brace != -1 and last_brace > first_brace:
        ...
        logger.trace(
            f"{log_prefix}: Extracted JSON using outermost braces: '{extracted_str[:100]}...'"
        )
        return extracted_str
    ...
    if cleaned_text.startswith("{") and cleaned_text.endswith("}"):
        logger.trace(
            f"{log_prefix}: Assuming cleaned text itself is the JSON candidate: '{cleaned_text[:100]}...'"
        )
        return cleaned_text
...
def _programmatic_json_parse_and_fix(
    ...
    for attempt in range(max_fix_attempts):
        ...
        try:
            ...
            )  # Langchain's parser can sometimes fix minor issues
            # Or use: parsed_object = json.loads(current_text_to_parse)
            logger.debug(
                f"{log_prefix}: Successfully parsed JSON on attempt {attempt + 1}."
            )
            return parsed_object
        except (json.JSONDecodeError, OutputParserException) as e:
            logger.warning(
                f"{log_prefix}: Attempt {attempt + 1} parse failed: {type(e).__name__} - {str(e)[:100]}"
            )

            if (
                attempt == max_fix_attempts - 1
            ):  # Last attempt failed, don't try to fix further
                logger.error(
                    f"{log_prefix}: Max fix attempts reached. Could not parse after error: {e}"
                )
                break
            ...
            current_text_to_parse = re.sub(
                r",\s*([\}\]])", r"\1", current_text_to_parse
            )
            ...
            if "Expecting property name enclosed in double quotes" in str(e):
                text_after_key_quote_fix = re.sub(
                    r"([{,\s])(\w+)(\s*:)", r'\1"\2"\3', current_text_to_parse
                )
            ...
            text_after_comment_removal = re.sub(
                r"//.*?\n", "\n", current_text_to_parse, flags=re.MULTILINE
            )
            text_after_comment_removal = re.sub(
                r"/\*.*?\*/", "", text_after_comment_removal, flags=re.DOTALL
            )
            ...
            if (
                current_text_to_parse == text_before_fixes
                and attempt < max_fix_attempts - 1
            ):
                logger.warning(
...
def add_cors_headers(response):
    ...
    response.headers["Access-Control-Allow-Credentials"] = (
        "true"  # If you send cookies/credentials
    )
    return response
        ...
        if request:
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        "    RESP Body: Could not get/decode JSON data for logging snippet."
                    )
            elif response.is_streamed:
                logger.debug("    RESP Body: Streamed response (not logging snippet).")
            else:
                logger.debug(
                    f"    RESP Body: Not logging snippet (Type: {response.mimetype}, Streamed: {response.is_streamed})."
                )
        else:
...
def setup_assistant_proxy():
    ...
    if not os.path.isfile(ASSISTANT_PROXY_SOURCE_PATH):
        logger.critical(
            f"❌ Source AppleScript file not found at: {ASSISTANT_PROXY_SOURCE_PATH}"
        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        compile_cmd = [
            "osacompile",
            "-o",
            compiled_app_path_tmp,
            ASSISTANT_PROXY_SOURCE_PATH,
        ]
        ...
        try:
            ...
            logger.success(
                f"✅ Successfully compiled proxy app in temporary location: {compiled_app_path_tmp}"
            )
        except subprocess.CalledProcessError as e:
            logger.error(
                f"❌ osacompile failed (RC={e.returncode}): {e.stderr or e.stdout}"
            )
            logger.error(
                f"   Check syntax in source file: {ASSISTANT_PROXY_SOURCE_PATH}"
            )
            return False
        except FileNotFoundError:
            logger.error(
                "❌ osacompile command not found. Is Xcode Command Line Tools installed?"
            )
            return False
        except Exception as e:
        ...
        logger.info(
            f"Attempting to copy compiled app to {ASSISTANT_PROXY_DEST_PATH}..."
        )
        try:
            if os.path.exists(ASSISTANT_PROXY_DEST_PATH):
                logger.warning(
                    f"'{ASSISTANT_PROXY_DEST_PATH}' already exists. Removing old version (requires sudo)."
                )
            ...
            subprocess.run(
                copy_cmd, check=True
            )  # Run without explicit sudo here, as parent script has it

            logger.success(
                f"✅ Successfully copied '{ASSISTANT_PROXY_APP_NAME}' to /Applications."
            )

            # --- 4. Attempt to Prime Permissions ---
            logger.info(
                "Attempting to trigger initial permission prompts (may require user interaction)..."
            )
            ...
            try:
                ...
                prime_process = subprocess.run(
                    osa_command, capture_output=True, text=True, timeout=15, check=False
                )
                logger.info("Permission priming command sent.")
                if prime_process.stdout:
                    logger.debug(f"Priming stdout: {prime_process.stdout.strip()}")
                if prime_process.stderr:
                    logger.warning(
                        f"Priming stderr: {prime_process.stderr.strip()}"
                    )  # Stderr expected if prompts shown/denied
            except subprocess.TimeoutExpired:
                logger.warning(
                    "Permission priming script timed out (might be waiting for user input)."
                )
            except Exception as prime_e:
                logger.warning(
                    f"Failed to run priming script (this might be ok): {prime_e}"
                )
            # --- End Priming ---
            ...
            logger.error(
                f"❌ Failed to copy/remove app in /Applications (RC={e.returncode}): {e.stderr or e.stdout}"
            )
...
class CortexThoughts:
    ...
    def __init__(self, provider: CortexEngine):
        ...
        self.vectorstore_history: Optional[Chroma] = (
            None  # In-memory store for current request
        )
        ...
        if DCTD_ENABLE_QUANTUM_PREDICTION:
            try:
                self.branch_predictor = BranchPredictorProvider(
                    priority_lock=self.provider._priority_quota_lock
                )
                logger.success(
                    "✅ Branch Predictor Provider initialized with priority lock."
                )
            except Exception as e:
                logger.error(
                    f"❌ Failed to initialize Branch Predictor Provider: {e}. Branch prediction will be disabled."
                )
        else:
            logger.info(
                "DCTD Branch Prediction is disabled by configuration. Branch Predictor Provider not initialized."
            )
        # --- END NEW ---
        ...
        if (
            ENABLE_STELLA_ICARUS_HOOKS and StellaIcarusHookManager is not None
        ):  # Check if class was imported
            try:
                self.stella_icarus_manager = StellaIcarusHookManager()
                if self.stella_icarus_manager.hook_load_errors:
                    logger.warning(
                        "CortexThoughts Init: StellaIcarusHookManager loaded with some errors."
                    )
                elif not self.stella_icarus_manager.hooks:
                    logger.info(
                        "CortexThoughts Init: StellaIcarusHookManager loaded, but no hooks found/active."
                    )
                else:
                    logger.success(
                        "CortexThoughts Init: StellaIcarusHookManager loaded successfully with hooks."
                    )
            except Exception as e_sihm_init:
                logger.error(
                    f"CortexThoughts Init: Failed to initialize StellaIcarusHookManager: {e_sihm_init}"
                )
                self.stella_icarus_manager = None
        elif not StellaIcarusHookManager:
            logger.error(
                "CortexThoughts Init: StellaIcarusHookManager class not available (import failed?). Hooks disabled."
            )
    ...
    def _get_temporal_context_string(self) -> str:
        ...
        Holocene, Hebrew, and Hijri dates to ground the AI's sense of time.
        """
        now = datetime.now()

        # 1. Common Era (Gregorian)
        # Format: "Monday, January 27, 2026 | 07:29 AM"
        common_era_str = now.strftime("%A, %B %d, %Y | %I:%M %p")

        # 2. Holocene Calendar (Human Era)
        # Simply add 10,000 to the current year
        holocene_year = now.year + 10000
        holocene_str = f"{holocene_year} HE"

        # 3. Cultural Calendars (Hebrew & Hijri)
        cultural_str = ""
        if TEMPORAL_LIBRARIES_AVAILABLE:
            try:
                # Hebrew
                h_y, h_m, h_d = hebrew.from_gregorian(now.year, now.month, now.day)
                # Tuple to string (e.g., 5786-5-10). You can map months to names if preferred.
                hebrew_str = f"{h_y}-{h_m}-{h_d} (Hebrew)"

                # Hijri (Islamic)
                i_y, i_m, i_d = islamic.from_gregorian(now.year, now.month, now.day)
                hijri_str = f"{i_y}-{i_m}-{i_d} (Hijri)"

                cultural_str = f" | {hebrew_str} | {hijri_str}"
            except Exception as e:
                logger.error(f"Error calculating cultural dates: {e}")

        # Construct Final String
        # Output: [TEMPORAL ANCHOR: Monday, January 27, 2026... ]
        return f"[Time internal Reference (Just Time Awareness for later Memory retrieval): {common_era_str} (Common Era) | {holocene_str}{cultural_str}]"
    ...
    def _construct_raw_chatml_prompt(
        ...
        current_turn_content: Optional[
            str
        ] = None,  # Content for the current user/instruction turn
        ...
        for turn in history_turns:
            ...
            if role not in [
                ...
                "assistant",
                "system",
            ]:  # System in history is rare but possible
                logger.warning(
                    f"ChatML Constructor: Unknown role '{role}' in history. Skipping."
                )
                continue
            if content:  # Only add turns with actual content
                prompt_parts.append(
                    f"{CHATML_START_TOKEN}{role}{CHATML_NL}{content}{CHATML_END_TOKEN}{CHATML_NL}"
                )

        if current_turn_content and current_turn_content.strip():
            current_turn_role = current_turn_role.lower()
            if current_turn_role not in [
                "user",
                "system",
            ]:  # Typically "user" or "system" for instructions
                logger.warning(
    ...
    async def _generate_with_comparative_evaluation(
        ...
        db: Session,
        session_id: str,
        user_prompt_str: str,  # The actual text prompt sent to the models
        prompt_inputs: Dict,  # The inputs dict for the chain
        expert_chain: Any,  # The LangChain runnable for the Expert
        step_name: str,  # e.g., "specialist_draft" or "correction"
        priority: int = ELP0,
    ) -> str:
        """
        Runs a 'Fast vs. Expert' battle.
        1. Generates a baseline using 'general_fast'.
        2. Generates the actual result using the provided 'expert_chain'.
        3. Evaluates the difference.
        4. Logs the TRINITY (Baseline + Expert + Eval) to the DB in one entry.
        5. Returns the Expert result so the pipeline can continue.
        """
        log_prefix = f"⚖️ COMPARE|{step_name}|{session_id[:8]}"
        logger.info(f"{log_prefix} Starting comparative inference...")

        # --- 1. Generate Baseline (Fast Path) ---
        fast_model = self.provider.get_model("general_fast")
        baseline_response = "[Fast Model Unavailable]"

        if fast_model:
            try:
                # We use the raw prompt string for the fast model to simulate a "direct" hit
                # Or you can bind it to a simple chain if prompt_inputs are complex
        ...
        # --- 2. Generate Expert (The Actual Task) ---
        # This is the result the rest of the pipeline actually needs
        expert_response = ""
        timing_data = {"session_id": session_id, "mode": f"expert_{step_name}"}

        try:
            expert_response = await asyncio.to_thread(
                self._call_llm_with_timing,
                expert_chain,
                prompt_inputs,
                timing_data,
            ...
            raise e  # We must raise this, as the pipeline relies on it

        # --- 3. Run Evaluation (The Judge) ---
        evaluator_model = self.provider.get_model("router")  # Or 'general'
        evaluation_result = "Evaluation skipped."

        if evaluator_model and baseline_response and expert_response:
            try:
                eval_input = (
                    f"User Prompt: {user_prompt_str[:1000]}...\n\n"  # Truncate prompt if huge
                    f"--- BASELINE RESPONSE ---\n{baseline_response}\n\n"
                    f"--- EXPERT RESPONSE ---\n{expert_response}"
                )
                eval_chain = (
                    ChatPromptTemplate.from_template(PROMPT_COMPARATIVE_EVALUATION)
                    | evaluator_model
                    | StrOutputParser()
                )
                ...
                evaluation_result = await asyncio.to_thread(
                    ...
                    {
                        "input": eval_input
                    },  # Assuming prompt template uses {input} or adjusts accordingly
                )
            except Exception as e:
                logger.warning(f"{log_prefix} Evaluation failed: {e}")

        # --- 4. Construct the Recursive Learning Record ---
        # This is the "One Entry" you requested
        composite_log = (
            f"=== COMPOSITE LEARNING RECORD ===\n\n"
            f"--- 1. PROMPT ---\n{user_prompt_str}\n\n"
            f"--- 2. BASELINE (Fast) ---\n{baseline_response}\n\n"
            f"--- 3. EXPERT (Slow) ---\n{expert_response}\n\n"
            f"--- 4. EVALUATION ---\n{evaluation_result}"
        ...
        try:
            await asyncio.to_thread(
                ...
                db,
                session_id=session_id,
                input_type="text",  # Keep strict text type
                user_input=f"[Comparative Inference: {step_name}]",  # Summary title for user_input
                llm_response=composite_log,  # The massive concatenated string
                classification=f"recursive_learning_{step_name}",  # Special classification for training
            )
            await asyncio.to_thread(db.commit)
            logger.success(f"{log_prefix} Comparative record saved.")
        except Exception as e:
            logger.error(f"{log_prefix} DB Save failed: {e}")

        # Return ONLY the expert response, because that's what the pipeline needs next
    ...
    async def generate(
        ...
        user_input: str,
        session_id: str,
        classification: str = "chat_simple",
    ) -> str:
        """
        ...
        log_prefix = f"🧠 {req_id}"
        logger.info(
            f"{log_prefix} Dispatching generate call. Session: {session_id}, Class: {classification}"
        )
        ...
        if not cortex_text_interaction:
            logger.error(
                f"{log_prefix} cortex_text_interaction (CortexThoughts) instance is not available."
            )
        ...
        try:
            ...
            logger.error(
                f"{log_prefix} Error in generate dispatcher: {e}", exc_info=True
            )
    ...
    def _count_tokens(self, text: str) -> int:
        ...
        if TIKTOKEN_AVAILABLE_APP and cl100k_base_encoder_app and text:
            try:
                ...
                logger.warning(
                    f"Tiktoken counting error in CortexThoughts: {e}. Falling back to char count."
                )
    ...
    def setup_prompts(self):
        ...
        self.text_prompt_template = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    PROMPT_BACKGROUND_MULTISTAGE_GRINDING,
                ),  # Expects various context keys
                ("human", "{input}"),
            ]
        )
        ...
        self.input_classification_prompt = ChatPromptTemplate.from_template(
            PROMPT_COMPLEXITY_CLASSIFICATION
        )
        self.tot_prompt = ChatPromptTemplate.from_template(PROMPT_TREE_OF_THOUGHTS)
        self.emotion_analysis_prompt = ChatPromptTemplate.from_template(
            PROMPT_EMOTION_ANALYSIS
        )
        self.image_latex_prompt = ChatPromptTemplate.from_template(
            PROMPT_IMAGE_TO_LATEX
        )
    ...
    def _convert_avif_to_png_b64(self, avif_b64: str) -> Optional[str]:
        ...
        try:
            ...
            if not features.check("avif"):
                logger.error(
                    f"{log_prefix}: Pillow AVIF plugin not available or registered!"
                )
                return None
            ...
            logger.error(
                f"{log_prefix}: Failed to convert AVIF to PNG: {e}", exc_info=True
            )
            return None

    # Modify the generic _describe_image_async
    async def _should_i_imagine_async(
        self, db: Session, session_id: str, user_input: str, context_summary: str
    ) -> bool:
        """
        ...
        """
        user_input = self._sanitize_text_for_prompt(
            user_input
        )  # self sanitize user_input from defective ChatML Token being leaked everywhere

        request_id_suffix = str(uuid.uuid4())[:8]
        log_prefix = f"🤔 ShouldImagine|ELP0|{session_id[:8]}-{request_id_suffix}"
        logger.info(
            f"{log_prefix} Analyzing input for imagination potential: '{user_input[:50]}...'"
        )
        ...
        prompt_input = {"user_input": user_input, "context_summary": context_summary}
        ...
        try:
            ...
            json_candidate = self._extract_json_candidate_string(
                raw_llm_response, log_prefix
            )
            if json_candidate:
                parsed_json = self._programmatic_json_parse_and_fix(
                    json_candidate, log_prefix=log_prefix
                )
                if (
                    parsed_json
                    and isinstance(parsed_json, dict)
                    and "should_imagine" in parsed_json
                ):
                    should_imagine = bool(parsed_json.get("should_imagine", False))
                    decision_json_str = json.dumps(
                        parsed_json, indent=2
                    )  # Store the full, well-formatted JSON
                else:
                    ...
                    decision_json_str = json.dumps(
                        {"should_imagine": False, "reasoning": reason}
                    )
            else:
            ...
            logger.warning(
                f"🚦 {log_prefix} 'should_imagine' task was interrupted: {tie}"
            )
            decision_json_str = json.dumps(
                {
                    "should_imagine": False,
                    "reasoning": f"Task interrupted by higher priority request: {tie}",
                }
            )
            should_imagine = False
            ...
            logger.error(
                f"{log_prefix} Exception during 'should_imagine' analysis: {e}"
            )
        ...
        await asyncio.to_thread(
            db.commit
        )  # Commit this log entry so it's visible to the next steps in the process
    ...
    async def _refine_direct_image_prompt_async(
        ...
        if not refiner_model:
            logger.error(
                f"{log_prefix} Model for image prompt refinement ('general') not available."
            )
            try:
                add_interaction(
                    db,
            ...
            return (
                user_image_request  # Fallback to original request if model unavailable
            )
        ...
        chain = (
            ChatPromptTemplate.from_template(
                PROMPT_REFINE_USER_IMAGE_REQUEST
            )  # Use the new prompt
            | refiner_model
            | StrOutputParser()
        )
        ...
        try:
            ...
            prompt_after_think_removal = re.sub(
                r"<think>.*?</think>",
                "",
                refined_prompt_raw,
                flags=re.DOTALL | re.IGNORECASE,
            ...
            logger.trace(
                f"{log_prefix}: After <think> removal:\n```\n{prompt_after_think_removal}\n```"
            )
            ...
            for i, preamble_pattern in enumerate(preambles):
                ...
                cleaned_prompt_intermediate = re.sub(
                    preamble_pattern,
                    "",
            ...
            cleaned_prompt_intermediate = re.sub(
                r'^["\'](.*?)["\']$', r"\1", cleaned_prompt_intermediate
            )
            if before_quote_strip != cleaned_prompt_intermediate:
                logger.trace(
                    f"{log_prefix}: After surrounding quote strip:\n```\n{cleaned_prompt_intermediate}\n```"
                )
            ...
            if before_output_only_strip != cleaned_prompt:
                logger.trace(
                    f"{log_prefix}: After '(Output only this):' strip:\n```\n{cleaned_prompt}\n```"
                )

            if not cleaned_prompt:
                logger.warning(
                    f"{log_prefix} LLM generated an empty image prompt after all cleaning steps."
                )
            ...
            logger.warning(
                f"🚦 {log_prefix} Image prompt refinement INTERRUPTED: {tie}"
            )
    ...
    async def _generate_image_generation_prompt_async(
        ...
        logger.info(
            f"{log_prefix} Generating image prompt for session {session_id} with rich context."
        )

        prompt_gen_model = self.provider.get_model("general")  # Or "router"
        if not prompt_gen_model:
            logger.error(
                f"{log_prefix} Model for image prompt generation ('general') not available."
            )
            try:
                add_interaction(
                    db,
                ...
                logger.error(f"Failed log img prompt gen model error: {db_err}")
            return None
        ...
        chain = (
            ChatPromptTemplate.from_template(
                PROMPT_CREATE_IMAGE_PROMPT
            )  # Uses the updated prompt from CortexConfiguration
            | prompt_gen_model
            | StrOutputParser()
        )
        ...
        try:
            ...
            if not generated_prompt_raw:
                logger.warning(
                    f"{log_prefix} LLM returned empty image generation prompt string."
                )
                return None
            ...
            cleaned_prompt = re.sub(
                r"\(Output only this\):?", "", cleaned_prompt, flags=re.IGNORECASE
            ).strip()

            if not cleaned_prompt:
                logger.warning(
                    f"{log_prefix} LLM generated an empty image prompt after cleaning."
                )
                try:
                    add_interaction(
                        db,
                        ...
                        user_input="[ImgPromptGen Empty]",
                        llm_response=f"Raw: {generated_prompt_raw[:200]}",
                    )
                except Exception:
                    pass
                return None
            ...
            logger.info(
                f"{log_prefix} Generated image prompt: '{cleaned_prompt}' (Raw len: {len(generated_prompt_raw)}, Cleaned len: {len(cleaned_prompt)})"
            )
            try:
                add_interaction(
                    db,
                    session_id=session_id,
                    mode="chat",
                    input_type="log_debug",
                    user_input="[ImgPromptGen Success]",
                    llm_response=f"Prompt: '{cleaned_prompt}'. Raw: {generated_prompt_raw[:200]}",
                )
            except Exception:
                pass
            ...
            logger.warning(
                f"🚦 {log_prefix} Image prompt generation INTERRUPTED: {tie}"
            )
            raise tie
        except Exception as e:
            ...
            try:
                add_interaction(
                    ...
                    mode="chat",
                    input_type="log_error",
                    user_input="[ImgPromptGen Failed]",
                    llm_response=f"Error: {e}. Raw: {str(generated_prompt_raw)[:200]}",
                )
            except Exception:
                pass
            return None
    ...
    async def _describe_generated_image_async(
        self, db: Session, session_id: str, image_avif_b64: str
    ) -> Optional[str]:
        """
    ...
    async def _should_i_create_a_hook_async(
        self, db: Session, session_id: str, query: str, rag_context: str, answer: str
    ) -> Tuple[bool, str]:
        ...
        if not decision_model:
            logger.error(
                f"{log_prefix} Router model unavailable. Cannot decide on hook creation."
            )
            return False, "Router model unavailable."

        prompt_input = {
            "user_query": query,
            "rag_context": self._truncate_rag_context(
                rag_context, 500
            ),  # Truncate for decision prompt
            "final_answer": answer,
        }
        chain = (
            ChatPromptTemplate.from_template(PROMPT_SHOULD_I_CREATE_HOOK)
            | decision_model
            | StrOutputParser()
        )
        ...
        try:
            ...
            if json_candidate:
                ...
                if (
                    ...
                    and "should_create_hook" in parsed
                ):
            ...
            logger.error(
                f"{log_prefix} Error during hook creation decision: {e}", exc_info=True
            )

        return False, "Analysis failed."

    async def _generate_stella_icarus_hook_async(
        self, db: Session, session_id: str, query: str, rag_context: str, answer: str
    ) -> None:
        ...
        try:
            ...
            logger.error(
                f"{log_prefix} Could not read hook template file '{template_path}': {e}"
            )
            return
        ...
        chain = (
            ChatPromptTemplate.from_template(PROMPT_GENERATE_STELLA_ICARUS_HOOK)
            | code_model
            | StrOutputParser()
        )
        ...
        try:
            ...
            if (
                not generated_code
                or "PATTERN" not in generated_code
                or "def handler" not in generated_code
            ):
                logger.warning(
            ...
            if self.stella_icarus_manager:
                ...
                try:
                    ...
                    logger.error(f"{log_prefix} Hot reload failed: {e_reload}")

            logger.success(
                f"{log_prefix} ✅ Successfully generated and saved new hook: {new_hook_filename}"
            )
            ...
            await asyncio.to_thread(
                ...
                input_type="hook_generated",
                user_input=f"Generated hook for query: {query}",
                llm_response=f"Saved to {new_hook_filename}",
            )
            await asyncio.to_thread(db.commit)

        except Exception as e:
            logger.error(
                f"{log_prefix} Error during hook generation or saving: {e}",
                exc_info=True,
            )

    async def _spawn_socratic_inquiry_for_text(
        self, source_text: str, session_id: str, request_id: str, step_name: str
    ):
        """
        ...
        try:
            ...
            if not db_session:
                raise RuntimeError(
                    "Failed to create a database session for the Socratic spawner task."
                )

            # 1. Generate the question using our refactored helper.
            generated_question = await self._generate_socratic_question_async(
                db=db_session, source_text=source_text, session_id=session_id
            )
            ...
            if generated_question:
                ...
                prompt_used = PROMPT_GENERATE_SOCRATIC_QUESTION.format(
                ...
                logger.success(
                    f"✅ {log_prefix}: Socratic question successfully saved and queued."
                )
            else:
                logger.info(
                    f"{log_prefix}: No question was generated, nothing to save."
                )

        except Exception as e:
            logger.error(
                f"❌ {log_prefix}: An unexpected error occurred in the Socratic spawner task: {e}"
            )
            logger.exception(f"{log_prefix} Traceback:")
            if db_session:
    ...
    def _transform_text_to_socratic_task(
        self, db: Session, text_to_transform: str
    ) -> str:
        """
        ...
        if not text_to_transform or not text_to_transform.strip():
            logger.warning(
                f"{log_prefix}: Input text is empty. Skipping transformation."
            )
            return text_to_transform

        logger.info(
            f"{log_prefix}: Transforming text into Socratic task: '{text_to_transform[:70]}...'"
        )
        ...
        try:
            ...
            if not transformation_model:
                raise RuntimeError(
                    "Model 'general_fast' for Socratic transformation not found."
                )

            # 2. Define the prompt structure using the "Pure LangChain" pattern.
            #    The entire prompt is treated as a single system message.
            prompt_reformed = [("system", PROMPT_CREATE_SOCRATIC_REFLECTION_TASK)]
            ...
            socratic_task_prompt = chain.invoke(
            ...
            logger.success(
                f"{log_prefix}: Socratic task created successfully: '{socratic_task_prompt[:100]}...'"
            )
            return socratic_task_prompt.strip()

        except Exception as transform_err:
            logger.error(
                f"{log_prefix}: Failed to transform text into Socratic task: {transform_err}"
            )
    ...
    async def _generate_socratic_question_async(
        self, db: Session, source_text: str, session_id: str
    ) -> Optional[str]:
        """
        ...
        if not source_text or not source_text.strip():
            logger.debug(
                f"{log_prefix} Source text is empty. Skipping Socratic question generation."
            )
            return None

        logger.info(
            f"{log_prefix} Generating Socratic question based on source text: '{source_text[:70]}...'"
        )

        socratic_model = self.provider.get_model("general")
        if not socratic_model:
            logger.error(
                f"{log_prefix} Model for Socratic questioning ('general') not available. Skipping."
            )
            return None

        # The prompt now only requires one variable.
        prompt_inputs = {"source_text_for_reflection": source_text}
        ...
        try:
            ...
            if (
                not generated_question_raw
                or "ERROR" in str(generated_question_raw).upper()
            ):
                logger.warning(
                    f"{log_prefix} LLM returned an empty or error response. Raw: '{generated_question_raw}'"
                )
                return None
            ...
            if not cleaned_question:
                logger.warning(
                    f"{log_prefix} LLM generated an empty question after cleaning."
                )
                return None

            logger.success(
                f"✅ {log_prefix} Successfully generated Socratic question: '{cleaned_question[:100]}...'"
            )
            return cleaned_question

        except TaskInterruptedException as tie:
            logger.warning(
                f"🚦 {log_prefix} Socratic question generation was interrupted: {tie}"
            )
            return None
        except Exception as e:
            logger.error(
                f"❌ {log_prefix} An error occurred during Socratic question generation: {e}"
            )
            logger.exception(f"{log_prefix} Traceback:")
            return None

    def _find_most_similar_interaction_by_vector(
        self, predicted_vector: np.ndarray, interactions: List[Interaction]
        ...
        for interaction in interactions:
            if interaction.embedding_json:
                try:
                    ...
                    # Cosine similarity
                    similarity = np.dot(predicted_vector, interaction_vector) / (
                        norm(predicted_vector) * norm(interaction_vector)
                    )
    ...
    def _get_rag_retriever_thread_wrapper(
        self, db_session: Session, user_input_str: str, priority_val: int
    ) -> Dict[str, Any]:
        """
        ...
        """
        log_prefix = (
            f"RAGThreadWrap|ELP{priority_val}|{self.current_session_id or 'NoSession'}"
        )
        try:
            logger.debug(f"{log_prefix}: Executing _get_rag_retriever in thread...")
            # Call the actual synchronous _get_rag_retriever method
            result_tuple = self._get_rag_retriever(
                db_session, user_input_str, priority_val
            )  # Passes it correctly
            logger.debug(
            ...
            logger.exception(
                f"{log_prefix} _get_rag_retriever_thread_wrapper Exception Details:"
            )
            return {"status": "error", "error_message": str(e_wrapper)}

    # --- NEW HELPER: Temporal Sieve Data for Branch Prediction ---
    def _get_temporal_sieve_data(
        self, db: Session, limit: int = MEMORY_SIZE
    ) -> List[Dict[str, Any]]:
        """
        ...
        logger.debug(
            f"{log_prefix}: Extracting last {limit} interactions for temporal sieve."
        )
        ...
        try:
            ...
            if not interactions:
                logger.debug(
                    f"{log_prefix}: No interactions with embeddings found for temporal sieve."
                )
                return []
            ...
            logger.debug(
                f"{log_prefix}: Extracted {len(sieve_data)} items for temporal sieve."
            )
            return sieve_data
    ...
    def _build_on_the_fly_retriever(
        ...
        log_prefix = (
            f"OnTheFlyRAG|ELP{priority}|{self.current_session_id or 'NoSession'}"
        )
        ...
        for interaction in interactions:
            ...
            if content.strip():
                ...
                metadata_map.append(
                    {"source": "on_the_fly_session", "interaction_id": interaction.id}
                )
        ...
        if texts_to_embed:
            ...
            embeddings = self.provider.embeddings.embed_documents(
                texts_to_embed, priority=priority
            )
            ...
            if embeddings and len(embeddings) == len(texts_to_embed):
                ...
                documents_to_add = [
                    ...
                    for text, meta in zip(texts_to_embed, metadata_map)
                ]
                ...
                vector_results = temp_vs.similarity_search_by_vector(
                    query_vector, k=RAG_HISTORY_COUNT // 2
                )
                retrieved_docs.extend(vector_results)
                logger.info(
                    f"{log_prefix} On-the-fly vector search found {len(vector_results)} docs."
                )
            else:
                logger.error(
                    f"{log_prefix} On-the-fly embedding failed or returned mismatched vectors."
                )
        ...
        if FUZZY_AVAILABLE and len(retrieved_docs) < RAG_HISTORY_COUNT // 2:
            ...
            processed_ids = {
                doc.metadata.get("interaction_id")
            ...
            for interaction in interactions:
                ...
                text_to_match = (
                    f"{interaction.user_input or ''} {interaction.llm_response or ''}"
                )
            ...
            if fuzzy_matches:
                ...
                logger.info(
                    f"{log_prefix} On-the-fly fuzzy search added {len(fuzzy_matches[:needed_count])} docs."
                )
    ...
    def _get_rag_retriever(
        ...
        """
        log_prefix = (
            f"RAGRetriever|ELP{priority}|{self.current_session_id or 'NoSession'}"
        )
        logger.info(
            f"Hybrid RAG Retriever: Combining searches for query: '{user_input_for_rag_query[:30]}...'"
        )
        ...
        try:
            ...
            if user_input_for_rag_query and self.provider and self.provider.embeddings:
                logger.debug(
                    f"{log_prefix} Pre-embedding main RAG query with priority ELP{priority}..."
                )
                rag_query_vector = self.provider.embeddings.embed_query(
                    user_input_for_rag_query, priority=priority
                )
                if not rag_query_vector:
                    logger.error(
                        f"{log_prefix} Main RAG query embedding resulted in None."
                    )

            # --- Step 1: URL Retriever (no change here) ---
            if (
                hasattr(self, "vectorstore_url")
                and self.vectorstore_url
                and rag_query_vector
            ):
            ...
            recent_unindexed_interactions = [
                ...
                if not inter.is_indexed_for_rag
            ]

            logger.info(
                f"[RAG VERBOSITY] Found {len(recent_unindexed_interactions)} recent unindexed interactions for on-the-fly search."
            )
            ...
            if recent_unindexed_interactions and rag_query_vector:
                ...
                logger.info(
                    f"[RAG VERBOSITY] On-the-fly hybrid search found {len(on_the_fly_docs)} documents."
                )
            ...
            if interaction_vs and rag_query_vector:
                ...
                logger.info(
                    f"[RAG VERBOSITY] Persistent vector store search found {len(persistent_docs or [])} documents."
                )
                all_history_docs.extend(persistent_docs or [])

            logger.info(
                f"[RAG VERBOSITY] Total combined documents for history_rag: {len(all_history_docs)}"
            )

            # 2c. Combine all history results into a single retriever
            if all_history_docs:
                unique_docs = list(
                    {doc.page_content: doc for doc in all_history_docs}.values()
                )
                ...
                temp_combined_vs = Chroma.from_documents(
                    documents=unique_docs, embedding=self.provider.embeddings
                )
                session_history_retriever = temp_combined_vs.as_retriever(
                    search_kwargs={"k": RAG_HISTORY_COUNT}
                )
            ...
            if reflection_vs and rag_query_vector:
                ...
                reflection_docs = reflection_chunks_retriever.invoke(
                    user_input_for_rag_query
                )
                logger.info(
                    f"[RAG VERBOSITY] Reflection vector store search found {len(reflection_docs or [])} documents."
                )
            ...
            logger.info(
                f"[RAG VERBOSITY] Total combined documents for final context: {len(final_context_docs)}"
            )
            ...
            if final_context_docs:
                ...
                logger.info(
                    f"[RAG VERBOSITY] De-duplicated to {len(unique_docs)} unique documents."
                )
            ...
            return (
                url_retriever,
                session_history_retriever,
                None,
                "",
            )  # Return None for the third retriever
    ...
    async def _generate_file_search_query_async(
        ...
        logger.debug(
            f"{query_gen_id}: Debugging file search query ctx input {user_input_for_analysis} dirHist {recent_direct_history_str} histRAGVec{history_rag_str}"
        )
        ...
        if not file_search_query_gen_model:
            logger.error(
                f"{query_gen_id}: Router model not available for file query generation. Falling back to user input."
            )
        ...
        chain = (
            ...
            # --- MODIFICATION START ---
            | file_search_query_gen_model.bind(
                max_tokens=FILE_SEARCH_QUERY_GEN_MAX_OUTPUT_TOKENS
        ...
        logger.debug(
            f"{query_gen_id}: Prompt Query _generate_file_search_query_async LLM to be processed {chain}"
        )
        logger.debug(
            f"{query_gen_id}: Prompt Query _generate_file_search_query_async LLM to be processed prompt_input {prompt_input}"
        )
        ...
        try:
            ...
            if not cleaned_query:
                logger.warning(
                    f"{query_gen_id}: LLM generated an empty search query. Falling back to user input."
                )
                return user_input_for_analysis

            logger.info(
                f"{query_gen_id}: Generated file search query: '{cleaned_query}'"
            )
    ...
    def _format_file_index_results(self, results: List[FileIndex]) -> str:
        ...
        if not isinstance(results, list):
            logger.error(
                f"_format_file_index_results received non-list: {type(results)}"
            )
    ...
    def _cleanup_llm_output(self, text: str) -> str:
        ...
        if not isinstance(text, str):
            logger.trace(
                f"Cleanup received non-str type: {type(text)}, returning as is."
            )
        ...
        cleaned_text = re.sub(
            r"<\|im_start\|>.*", "", cleaned_text, flags=re.IGNORECASE
        )
        ...
        processing_complete_pattern = (
            r"^\s*(Processing complete|Log stream complete)\.?\s*\n?"
        )
        cleaned_text = re.sub(
            processing_complete_pattern,
            "",
        ...
        draft_response_pattern = (
            r"^\s*(?:Draft Response|Your Final, Refined Response).*?:?\s*\n?"
        )
        cleaned_text = re.sub(
            draft_response_pattern, "", cleaned_text, flags=re.IGNORECASE | re.MULTILINE
        )

        # Remove think tags
        cleaned_text = re.sub(
            r"<think>.*?</think>", "", cleaned_text, flags=re.DOTALL | re.IGNORECASE
        )
        ...
        if cleaned_text != text:
            logger.warning(
                f"Applied cleanup to LLM output. Original len: {len(text)}, Cleaned len: {len(cleaned_text)}"
            )
    ...
    async def _trigger_web_search(
        ...
        logger.info(
            f"🔍 {req_id} Triggering background web search task for query: '{query}'"
        )
        ...
        async def run_search_and_log_result():
            ...
            try:
                if search_results:
                    ...
                    logger.success(
                        f"{req_id} Web search successful. Logging {len(search_results)} results to DB."
                    )
                    add_interaction(
                        bg_db,
                        session_id=session_id,
    ...
    async def _correct_response(
        ...
        if not corrector_model:
            logger.error(
                f"{log_prefix} Corrector model (key 'router') not available! Returning cleaned draft."
            )
            ...
            try:  # Use try-except for DB logging
                ...
                logger.error(
                    f"{log_prefix} Failed log corrector model unavailable: {db_err}"
                )
        ...
        try:
            prompt_reformed = [
                ...
                (
                    "human",
                    f"Please review and correct the following draft response: {draft_response}",
            ...
            logger.error(
                f"{log_prefix} Failed to set up corrector chain: {chain_setup_err}"
            )
            try:  # Use try-except for DB logging
                add_interaction(
                    db,
                ...
                logger.error(
                    f"{log_prefix} Failed log corrector chain setup error: {db_err}"
                )
        ...
        try:
            ...
            logger.info(
                f"{log_prefix} Cleaned response length: {len(final_response_cleaned)}"
            )
            ...
            logger.exception(
                f"{log_prefix} Corrector Execution Traceback:"
            )  # Log full traceback
            ...
            try:  # Use try-except for DB logging
                ...
                logger.error(
                    f"{log_prefix} Failed log corrector step failure: {db_err}"
                )

            # Fallback: Clean the original draft response if correction fails
            logger.warning(
                f"{log_prefix} Falling back to cleaned original draft due to corrector error."
            )
            cleaned_draft = self._cleanup_llm_output(draft_response)
            return cleaned_draft
            ...
            # Fallback: Clean the original draft response if correction fails
            logger.warning(
                f"{correction_id} Falling back to cleaned original draft due to corrector error."
            )
            cleaned_draft = self._cleanup_llm_output(draft_response)
            return cleaned_draft
    ...
    async def _execute_assistant_action(
        ...
        logger.info(
            f"🚀 {req_id} Handling assistant action: '{action_type}' on platform '{platform}' (Trigger ID: {triggering_interaction.id})"
        )
        ...
        try:
            ...
            if action_type == "search" and parameters.get("query"):
                logger.info(
                    f"{req_id} Handling 'search' action type. Triggering background web search."
                )
                try:
                    ...
                    add_interaction(
                        ...
                        assistant_action_executed=True,
                        assistant_action_result="[Search process launched]",
                    )
                    exec_db.commit()
                    ...
                    if triggering_interaction:
                        ...
                        triggering_interaction.assistant_action_result = (
                    ...
                    logger.error(
                        f"{req_id} Error during web search trigger: {search_err}"
                    )
                    exec_db.rollback()
                    return search_exec_fallback
            elif action_type == "keyword_based_response_fallback":
                logger.info(
                    f"{req_id} Handling 'keyword_based_response_fallback' action type. Returning explanation directly."
                )
                ...
                fallback_message = parameters.get(
                    "explanation", "An internal fallback action was triggered."
                )
                if triggering_interaction:
                    triggering_interaction.assistant_action_executed = True
            ...
            for attempt in range(1, AGENT_MAX_SCRIPT_RETRIES + 1):
                logger.info(
                    f"{req_id} {script_lang.capitalize()} Script Attempt {attempt}/{AGENT_MAX_SCRIPT_RETRIES} for '{action_type}'"
                )
                ...
                if platform == "darwin":
                    gen_prompt = PROMPT_GENERATE_APPLESCRIPT
                    refine_prompt = PROMPT_REFINE_APPLESCRIPT
                elif platform == "win32":
                    gen_prompt = PROMPT_GENERATE_POWERSHELL_SCRIPT
                    refine_prompt = PROMPT_REFINE_POWERSHELL_SCRIPT
                else:  # Linux/other
                    gen_prompt = PROMPT_GENERATE_BASH_SCRIPT
                    refine_prompt = PROMPT_REFINE_BASH_SCRIPT
                ...
                script_chain = (
                    ChatPromptTemplate.from_template(llm_prompt_template)
                    | script_llm
                    | StrOutputParser()
                )

                try:
                    generated_script_raw = await asyncio.to_thread(
                        script_chain.invoke, llm_input
                    )
                    script_to_execute = re.sub(
                        rf"^```(?:{script_lang})?\s*|```\s*$",
                        "",
                        generated_script_raw,
                        flags=re.MULTILINE,
                    ).strip()
                    if not script_to_execute:
                        logger.warning(
                            f"{req_id} LLM returned empty script on attempt {attempt}."
                        )
                        last_error_summary = "LLM generated an empty script."
                        continue  # Go to next attempt
                    logger.info(
                        f"{req_id} LLM {'generated' if attempt == 1 else 'refined'} a {script_lang} script."
                    )
                except Exception as gen_err:
                    logger.error(
                        f"{req_id} Error calling LLM for script attempt {attempt}: {gen_err}"
                    )
                ...
                logger.info(
                    f"{req_id} Script finished in {exec_duration_ms:.0f}ms. RC={rc}."
                )
                if stdout:
                ...
                if success:
                    logger.success(
                        f"✅ {req_id} Script execution successful on attempt {attempt}."
                    )
                    ...
                    logger.error(
                        f"❌ {req_id} {script_lang.capitalize()} attempt {attempt} FAILED. Error: {error_summary}"
                    )
                    last_error_summary, last_stderr, last_stdout, last_rc = (
                        error_summary,
                        stderr,
                        stdout,
                        rc,
            ...
            # --- End of Loop ---
            logger.error(
                f"❌ {req_id} Script execution failed after all {AGENT_MAX_SCRIPT_RETRIES} attempts."
            )
            if triggering_interaction:
                triggering_interaction.assistant_action_executed = (
                    True  # It was attempted to exhaustion
                )
            ...
            err_msg = (
                f"Unexpected error during action execution for '{action_type}': {e}"
            )
    ...
    def _format_script_rag_context(self, attempts: List[Any]) -> str:
        ...
        for i, attempt in enumerate(attempts):
            context_str += (
                f"--- Past Attempt {i + 1} ({attempt.timestamp.isoformat()}) ---\n"
            )
            context_str += (
    ...
    def _format_docs(self, docs: List[Any], source_type: str = "Context") -> str:
        ...
        if hasattr(docs[0], "page_content"):
            ...
            logger.warning(
                f"Unrecognized doc type in _format_docs: {type(docs[0])}. Assuming Interaction list."
            )
            return self._format_interaction_list_to_string(docs)

    def _format_interaction_list_to_string(
        self, interactions: List[Interaction], include_type=False
    ) -> str:
        ...
        if not isinstance(interactions, list):
            logger.error(
                f"_format_interaction_list_to_string received non-list: {type(interactions)}"
            )
        ...
        sorted_interactions = sorted(
            interactions, key=lambda i: i.timestamp
        )  # Oldest first
        ...
        for interaction in sorted_interactions:
            ...
            elif interaction.llm_response and interaction.input_type not in [
                "system",
                "error",
                "log_error",
                "log_warning",
    ...
    def _format_direct_history(self, interactions: List[Interaction]) -> str:
        ...
        if not isinstance(interactions, list):
            logger.error(
                f"_format_direct_history received non-list: {type(interactions)}"
            )
    ...
    def _format_log_history(self, interactions: List[Interaction]) -> str:
        ...
        log_str_parts = []
        sorted_interactions = sorted(
            interactions, key=lambda i: i.timestamp, reverse=True
        )

        for interaction in sorted_interactions:
            log_level = (
                interaction.input_type.split("_")[-1].upper()
                if "_" in interaction.input_type
                else interaction.input_type.upper()
            )
            log_message = (
                interaction.llm_response
                or interaction.user_input
                or "[Log content missing]"
            )
            timestamp_str = interaction.timestamp.strftime("%H:%M:%S")
            log_snippet = (
                (log_message[:200] + "...") if len(log_message) > 200 else log_message
            )
        ...
        return (
            "\n".join(log_str_parts)
            if log_str_parts
            else "No recent relevant logs found."
        )
    ...
    def _casual_mistype(self, text: str) -> str:
        ...
        if random.random() < MISTYPE_PUNCTUATION_OMISSION_CHANCE:
            ...
            punctuation_indices = [
                i for i, char in enumerate(modified_text) if char in [",", "."]
            ]
            if punctuation_indices:
                index_to_remove = random.choice(punctuation_indices)
                modified_text = (
                    modified_text[:index_to_remove]
                    + modified_text[index_to_remove + 1 :]
                )
    ...
    def _call_llm_with_timing(
        ...
        # If available RAM drops below this, the system is unstable.

        allowed_ratio = WARDEN_RAM_ALLOCATION_PERCENTAGE / 100.0
        buffer_ratio = SYSTEMBUFFER_SAFE_PERCENTAGE / 100.0

        # Example: 1.0 - 0.85 - 0.05 = 0.10 (10% Reserved for OS survival)
        reserved_ratio = 1.0 - allowed_ratio - buffer_ratio

        # Safety: Always reserve at least 2% no matter what config says
        if reserved_ratio < 0.02:
            reserved_ratio = 0.02

        critical_threshold_gb = total_gb * reserved_ratio
        ...
        while True:
            ...
            logger.critical(
                f"🛑 WARDEN HALT: System RAM Critical! Inference might be clashing or there's something blocking our way "
                f"Available {current_avail_gb:.2f}GB < Limit {critical_threshold_gb:.2f}GB. "
                f"Sleeping to prevent crash..."
            )
            time.sleep(0.5)
    ...
    def _extract_json_candidate_string(
        self, raw_llm_text: str, log_prefix: str = "JSONExtract"
    ) -> Optional[str]:
        """
        Robustly extracts a potential JSON string from raw LLM text.
        Handles <think> tags, markdown code blocks, and finding outermost braces.
        ...
        text_after_think_removal = re.sub(
            r"<think>.*?</think>", "", raw_llm_text, flags=re.DOTALL | re.IGNORECASE
        ).strip()
        if not text_after_think_removal:
        ...
        cleaned_text = re.sub(
            r"<\|im_start\|>.*?<\|im_end\|>", "", cleaned_text, flags=re.DOTALL
        )
    ...
    def _programmatic_json_parse_and_fix(
        ...
        for attempt in range(max_fix_attempts):
            logger.debug(
                f"{log_prefix}: Parse/fix attempt {attempt + 1}/{max_fix_attempts}."
            )
            try:
                ...
                if current_text_to_parse == text_before_fixes:
                    logger.warning(
                        f"{log_prefix}: Programmatic fixes did not alter the string. Error may be complex."
                    )
                    break  # Break if no progress is made

        logger.error(
            f"{log_prefix}: Failed to parse JSON after all programmatic fix attempts."
        )
        return None

    async def _classify_input_complexity(
        self, db: Session, user_input: str, interaction_data_for_metrics: dict
    ) -> str:
        """
        ...
        current_session_id = interaction_data_for_metrics.get(
            "session_id", "unknown_classify_session"
        )
        log_prefix = f"🤔 Classify|ELP0|{current_session_id[:8]}-{request_id_suffix}"
        logger.info(
            f"{log_prefix} Classifying input complexity for: '{user_input[:50]}...'"
        )

        # Get history summary synchronously (it's a DB call)
        history_summary = await asyncio.to_thread(
            self._get_history_summary, db, MEMORY_SIZE
        )  # MEMORY_SIZE from CortexConfiguration

        classification_model_instance = self.provider.get_model("router")
        if not classification_model_instance:
            logger.warning(
                f"{log_prefix} Router model unavailable for classification. Falling back to default model."
            )
        ...
        if not classification_model_instance:
            ...
            await asyncio.to_thread(
                ...
                mode="chat",
                input_type="log_error",
                user_input="[Classify Model Unavailable]",
        ...
        if hasattr(classification_model_instance, "bind") and callable(
            getattr(classification_model_instance, "bind")
        ):
            try:
                classification_model_for_call = classification_model_instance.bind(
                    temperature=0.1
                )
        ...
        raw_llm_response_for_final_log: str = (
            "Classification LLM call did not produce parsable output."
        )
        ...
        for attempt in range(
        ...
        ):  # DEEP_THOUGHT_RETRY_ATTEMPTS from CortexConfiguration
            ...
            try:
                ...
                json_candidate_str = self._extract_json_candidate_string(
                    raw_llm_text_this_attempt, log_prefix + "-Extract"
                )
                if json_candidate_str:
                    ...
                    if (
                        parsed_json_output
                        and isinstance(parsed_json_output, dict)
                        and "classification" in parsed_json_output
                        and "reason" in parsed_json_output
                    ):
                        classification_val = str(
                            parsed_json_output.get("classification", "chat_simple")
                        ).lower()
                        reason_val = str(parsed_json_output.get("reason", "N/A"))
                        if classification_val not in [
                            "chat_simple",
                            "chat_complex",
                            "agent_task",
                        ]:
                            logger.warning(
                                f"{log_prefix} Invalid category '{classification_val}'. Defaulting."
                            )
                            classification_val = "chat_simple"
                        interaction_data_for_metrics["classification"] = (
                            classification_val
                        )
                        interaction_data_for_metrics["classification_reason"] = (
                            reason_val
                        )
                        logger.info(
                            f"✅ {log_prefix} Input classified as: '{classification_val}'. Reason: {reason_val}"
                        )
        ...
        if not parsed_json_output:  # Check if we still don't have valid JSON
            ...
            reformat_prompt_input = {
                "faulty_llm_output_for_reformat": raw_llm_response_for_final_log
            }
            reformat_chain = (
                ChatPromptTemplate.from_template(PROMPT_REFORMAT_TO_ACTION_JSON)
                | action_analysis_model
                | StrOutputParser()
            )  # PROMPT_REFORMAT_TO_ACTION_JSON from CortexConfiguration
            ...
            if reformatted_llm_output_text and not (
                ...
                if json_candidate_from_reformat:
                    ...
                    if (
                        ...
                        reason_val = str(
                            parsed_json_output.get("reason", "N/A (reformatted)")
                        )
                        if classification_val not in [
                            "chat_simple",
                            "chat_complex",
    ...
    def _run_emotion_analysis(
        ...
        logger.info(
            f"{log_prefix} Analyzing input emotion/context for: '{user_input[:50]}...'"
        )
        ...
        emotion_model_role = (
            "router"  # Configurable: could be "router" or a dedicated role
        )
        ...
        if not emotion_model:
            error_msg = (
                f"Emotion analysis model ('{emotion_model_role}') not available."
            )
            logger.error(f"{log_prefix} {error_msg}")
            interaction_data["emotion_context_analysis"] = (
                error_msg  # Update main interaction data
            )
            analysis_result_for_return = (
                f"Could not analyze emotion (model '{emotion_model_role}' unavailable)."
            )
            try:
                add_interaction(
                    db,
                ...
                logger.error(
                    f"{log_prefix} Failed to log emotion model unavailable error: {db_err}"
                )
        ...
        try:
            ...
            cleaned_analysis = self._cleanup_llm_output(
                analysis
            )  # Use existing cleanup

            logger.info(
                f"{log_prefix} Emotion/Context Analysis Result: {cleaned_analysis[:200]}..."
            )
            interaction_data["emotion_context_analysis"] = (
                cleaned_analysis  # Update main interaction data
            )
            ...
            try:
                ...
                logger.error(
                    f"{log_prefix} Failed log emotion analysis interruption: {db_err}"
                )
            ...
            interaction_data["emotion_context_analysis"] = error_msg
            analysis_result_for_return = (
                f"Could not analyze emotion (processing error: {type(e).__name__})."
            )
            try:
                add_interaction(
                    db,
                ...
                logger.error(
                    f"{log_prefix} Failed to log emotion analysis error: {db_err}"
                )
    ...
    def _normalize_direct_response_text(self, text: str) -> str:
        ...
        try:
            ...
            logger.error(
                f"Error during direct response normalization: {e}. Returning original text."
            )
    ...
    async def _analyze_assistant_action(
        ...
        action_analysis_model = self.provider.get_model(
            "router"
        ) or self.provider.get_model("default")
        if not action_analysis_model:
            logger.error(
                f"{log_prefix} ❌ Action analysis model (router/default) not available!"
            )
            return None
        ...
        raw_llm_output_from_initial_loop: str = (
            "Initial LLM action analysis did not yield parsable output."
        )
        ...
        for attempt in range(DEEP_THOUGHT_RETRY_ATTEMPTS):
            ...
            try:
                raw_llm_text = await asyncio.to_thread(
                    self._call_llm_with_timing,
                    analysis_chain_raw,
                    prompt_input_initial,
                    action_timing_data,
                ...
                json_candidate = self._extract_json_candidate_string(
                    raw_llm_text, log_prefix
                )
                if json_candidate:
                    parsed_action_json = self._programmatic_json_parse_and_fix(
                        json_candidate,
                        1,
                    ...
                    if (
                        parsed_action_json
                        and isinstance(parsed_action_json, dict)
                        and "action_type" in parsed_action_json
                        and "parameters" in parsed_action_json
                    ):
                        ...
                        return (
                            parsed_action_json if action_type != "no_action" else None
                        )
            except TaskInterruptedException as tie:
                raise tie
            except Exception as e:
                logger.warning(
                    f"⚠️ {log_prefix} Initial Action analysis attempt {current_attempt_num} failed: {e}"
                )
  ...
  "explanation": "string (your reasoning for the choice)"
            ...
            if reformatted_llm_output_text and not (
                ...
                logger.info(
                    f"{log_prefix} Received reformatted output from LLM. Parsing with fix retries..."
                )
                json_candidate_from_reformat = self._extract_json_candidate_string(
                    reformatted_llm_output_text, log_prefix + "-ReformatExtract"
                )
                if json_candidate_from_reformat:
                    ...
                    if (
                        ...
                        action_type = parsed_action_json.get("action_type")
                        logger.info(
                            f"✅ {log_prefix} Reformatted Action analysis successful: Type='{action_type}'"
                        )
                        return (
                            parsed_action_json if action_type != "no_action" else None
                        )
        ...
        fallback_params = {
            "original_query": user_input[:200],
            "extracted_keywords": list(set(keywords))[:7],
        }
        logger.info(
            f"{log_prefix} Extracted keywords for fallback: {fallback_params['extracted_keywords']}"
        )
    ...
    async def _translate(
        ...
        if not translator_model:
            logger.error(
                f"{log_prefix}: Translator model not available, cannot translate."
            )
            # Silently return original text, assuming caller handles this possibility
            return text

        logger.debug(
            f"{log_prefix}: Translating from '{source_lang}' to '{target_lang}': '{text[:50]}...'"
        )
        try:
            ...
            if hasattr(message_result, "content") and isinstance(
                message_result.content, str
            ):
                translated_text = message_result.content
            elif isinstance(
                message_result, str
            ):  # Handle if model directly returns a string
                translated_text = message_result
            else:
                # Handle unexpected return types from the translation model
                logger.error(
                    f"{log_prefix}: Translation model returned unexpected type: {type(message_result)}. Full result: {message_result}"
                )
                ...
                try:
                    ...
                    logger.error(
                        f"{log_prefix} Failed log translation type error: {db_err}"
                    )
    ...
    async def _route_to_specialist(
        ...
        logger.info(
            f"{log_prefix} Routing request for: '{user_input_for_routing[:50]}...'"
        )
        ...
        if not router_model:
            logger.error(
                f"{log_prefix}: Router model ('router') not available! Defaulting to '{default_model_key}'."
            )
            ...
            return (
                default_model_key,
                user_input_for_routing,
                "Router model unavailable, using default.",
            )
        ...
        router_chain_raw_output = (
            ChatPromptTemplate.from_messages(
                prompt_reformed
            )  # PROMPT_ROUTER from CortexConfiguration
            | router_model
            | StrOutputParser()
        )
        router_timing_data = {
        ...
        raw_llm_output_from_initial_loop: str = (
            "Initial router LLM call did not yield parsable JSON."
        )
        ...
        for attempt in range(DEEP_THOUGHT_RETRY_ATTEMPTS):
            current_attempt_num = attempt + 1
            logger.debug(
                f"{log_prefix} Router LLM call attempt {current_attempt_num}/{DEEP_THOUGHT_RETRY_ATTEMPTS}"
            )
            raw_llm_text_this_attempt = ""
            try:
                raw_llm_text_this_attempt = await asyncio.to_thread(
                ...
                if json_candidate_str:
                    ...
                    if (
                        parsed_routing_json
                        and isinstance(parsed_routing_json, dict)
                        and all(
                            k in parsed_routing_json
                            for k in ["chosen_model", "refined_query", "reasoning"]
                        )
                    ):
        ...
        reformat_chain = (
            ChatPromptTemplate.from_template(PROMPT_REFORMAT_TO_ROUTER_JSON)
            | router_model
            | StrOutputParser()
        )  # New Prompt
        ...
        if reformatted_llm_output_text and not (
            ...
            logger.info(
                f"{log_prefix} Received reformatted output from LLM for router. Attempting to parse/fix..."
            )
            json_candidate_from_reformat = self._extract_json_candidate_string(
                reformatted_llm_output_text, log_prefix + "-ReformatExtract"
            )
            if json_candidate_from_reformat:
                ...
                if (
                    ...
                    refined_query = str(parsed_routing_json["refined_query"])
                    reasoning = str(
                        parsed_routing_json.get("reasoning", "N/A (reformatted)")
                    )
                    logger.info(
                        f"✅ {log_prefix} Reformatted Routing analysis successful: Chose '{chosen_model}'."
                    )
                    return chosen_model, refined_query, reasoning
            else:
                logger.error(
    ...
    def _sanitize_context_string(self, context_str: str) -> str:
        ...
        for pattern in patterns_to_remove:
            sanitized_str = re.sub(
                pattern, "", sanitized_str, flags=re.DOTALL | re.IGNORECASE
            )

        # Also remove empty lines that might result from the substitution
        sanitized_str = "\n".join(
            line for line in sanitized_str.splitlines() if line.strip()
        )
    ...
    async def _extract_narrative_anchors(
        self, db: Session, user_input: str, priority: int = ELP0
    ) -> str:
        ...
        if not summarizer_model:
            return "Could not determine anchors."

        chain = (
            ChatPromptTemplate.from_template(PROMPT_NARRATIVE_ANCHOR_EXTRACTION)
            | summarizer_model
            | StrOutputParser()
        )
        try:
    ...
    async def _generate_progression_summary(
        self,
        db: Session,
        ...
        if not last_chunk:
            return "Begin the response."

        summarizer_model = self.provider.get_model("general_fast")
        if not summarizer_model:
            return "Continue the previous thought."

        chain = (
            ChatPromptTemplate.from_template(PROMPT_PROGRESSION_SUMMARY)
            | summarizer_model
            | StrOutputParser()
        )
        try:
            ...
            summary = await asyncio.to_thread(
                ...
                chain,
                {
                    "full_response_so_far": full_response_so_far,
            ...
            logger.info(
                f"{log_prefix} Generated Progression Summary: '{cleaned_summary}'"
            )
    ...
    def _extract_themes_from_think_block(self, think_content: str) -> List[str]:
        ...
        if match:
            ...
            themes = [
                theme.strip() for theme in re.split(r"[,;]", theme_str) if theme.strip()
            ]
        return themes
    ...
    async def _generate_topic_summary(
        self,
        db: Session,
        purified_history_interactions: List[Interaction],
        priority: int = ELP0,
    ) -> str:
        """
        ...
        if not purified_history_interactions:
            logger.debug(
                f"{log_prefix} No prior conversation history found. Using default initial summary."
            )
        ...
        if not summarizer_model:
            ...
            return (
                "(Context summary generation failed due to missing model configuration)"
            )

        # --- Build the LangChain Execution Chain ---
        # This chain will take the prompt, send it to the model, and parse the output as a string.
        chain = (
            ChatPromptTemplate.from_template(PROMPT_TOPIC_SUMMARY)
            | summarizer_model
            | StrOutputParser()
        )
        ...
        try:
            ...
            logger.info(
                f"{log_prefix} Successfully generated topic summary: '{cleaned_summary}'"
            )
            return cleaned_summary
            ...
            logger.error(
                f"{log_prefix} An exception occurred while generating the topic summary: {e}",
                exc_info=True,
            )
    ...
    def _parse_think_speak_output(self, raw_llm_output: str) -> Tuple[str, str]:
        ...
        if "</think>" in raw_llm_output:
            try:
                ...
                logger.error(
                    f"{log_prefix} Unexpected error during Think/Speak parsing: {e}. Falling back."
                )
    ...
    def _build_rag_context_within_token_limit(
        self, all_rag_docs: List[Any], token_limit: int
    ) -> str:
        """
    ...
    **Humanized Version:**
    """
        ...
        if not humanizer_model:
            logger.warning(
                f"{log_prefix} Humanizer model ('general_fast') not available. Returning original chunk."
            )
            return clean_chunk

        chain = (
            ChatPromptTemplate.from_template(humanizer_prompt_template)
            | humanizer_model
            | StrOutputParser()
        )
        ...
        try:
            ...
            if not humanized_text.strip():
                logger.warning(
                    f"{log_prefix} Humanizer returned an empty string. Falling back to original chunk."
                )
                return clean_chunk
            ...
            logger.error(
                f"{log_prefix} An exception occurred during the humanization LLM call: {e}",
                exc_info=True,
            )
...
"""
    ...
    async def _get_vector_search_file_index_context(
        ...
        priority: int = ELP0,
        stop_event_param: Optional[threading.Event] = None,
    ) -> str:
        """
        ...
        """
        log_prefix = (
            f"🔍 FileVecSearch|ELP{priority}|{session_id_for_log or 'NoSession'}"
        )
        logger.debug(
            f"{log_prefix} Attempting file search for query: '{query[:50]}...'"
        )
        ...
        if not global_file_vs:
            logger.warning(
                f"{log_prefix} Global file index vector store not available for vector search."
            )
        elif not self.provider or not self.provider.embeddings:
            logger.error(
                f"{log_prefix} Embeddings provider not available for vector search query."
            )
        elif not query:
            logger.debug(
                f"{log_prefix} Empty query for vector search. Skipping vector part."
            )
        else:
            query_vector: Optional[List[float]] = None
            try:
                logger.debug(
                    f"{log_prefix} Explicitly embedding query via _embed_texts with priority ELP{priority}..."
                )
                if hasattr(self.provider.embeddings, "_embed_texts") and callable(
                    ...
                    embedding_result_list = await asyncio.to_thread(
                        self.provider.embeddings._embed_texts,
                        [query],
                    ...
                    if embedding_result_list and len(embedding_result_list) > 0:
                        ...
                        logger.error(
                            f"{log_prefix} _embed_texts returned None or empty list for query."
                        )
                else:
                    logger.error(
                ...
                if not query_vector:
                    logger.error(
                        f"{log_prefix} Failed to embed query for vector search (query_vector is None)."
                    )
                else:
                    logger.debug(
                    ...
                    if search_results_docs:
                        ...
                        logger.info(
                            f"{log_prefix} Found {len(search_results_docs)} results from VECTOR file search."
                        )
                    else:
                        logger.info(
                            f"{log_prefix} No results from VECTOR file search for query '{query[:50]}...'"
                        )
        ...
        if not vector_search_succeeded:
            ...
            try:
                ...
                if not db_for_fuzzy:
                    raise RuntimeError("Failed to get DB session for fuzzy search.")
                ...
                candidate_records = (
                    db_for_fuzzy.query(FileIndex)
                    .filter(
                        FileIndex.index_status.in_(
                            ["indexed_text", "success", "partial_vlm_error"]
                        )
                        # Only search indexed files
                    )
                    .order_by(desc(FileIndex.last_modified_os))
                    .limit(500)
                    .all()
                )  # Limit candidates
                ...
                if not candidate_records:
                    logger.info(
                        f"{log_prefix} FUZZY: No candidate records in SQL DB for fuzzy search."
                    )
                else:
                    logger.debug(
                        f"{log_prefix} FUZZY: Found {len(candidate_records)} candidate records from SQL."
                    )
                    fuzzy_matches: List[
                        Tuple[FileIndex, int]
                    ] = []  # Store (record, score)
                    ...
                    for record in candidate_records:
                        if (
                            stop_event_param and stop_event_param.is_set()
                        ):  # Check if passed and set
                            logger.info(
                                f"{log_prefix} FUZZY search interrupted by stop_event_param."
                            )
                            break
                        ...
                        if record.indexed_content:
                            ...
                            content_snippet = (
                                (record.indexed_content[:500] + "...")
                                if len(record.indexed_content) > 500
                                else record.indexed_content
                            )
                            text_to_match_on += " " + content_snippet

                        if not text_to_match_on.strip():
                            continue

                        # Use fuzz.partial_ratio for substring matching, good for finding queries within larger text
                        score = fuzz.partial_ratio(
                            query.lower(), text_to_match_on.lower()
                        )
                    ...
                    if fuzzy_matches:
                        ...
                        top_fuzzy_matches = fuzzy_matches[
                        ...
                        for i, (record, score) in enumerate(top_fuzzy_matches):
                            content_snippet = (
                                (record.indexed_content[:300] + "...")
                                if record.indexed_content
                                and len(record.indexed_content) > 300
                                else (record.indexed_content or "[No content]")
                            )
                            entry = (
        ...
        if vector_search_succeeded and search_results_docs:
            ...
            for i, doc in enumerate(search_results_docs):
                if not hasattr(doc, "page_content") or not hasattr(doc, "metadata"):
                    logger.warning(
                        f"{log_prefix} Skipping malformed vector document: {doc}"
                    )
                    continue
                content = doc.page_content
                ...
                snippet = content[:max_snippet_len] + (
                    "..." if len(content) > max_snippet_len else ""
                )
                entry = (
                ...
                if current_chars + len(entry) > max_total_chars:
                    context_parts.append(
                        "[Vector file search context truncated due to length]...\n"
                    )
                    break
            ...
            return (
                "".join(context_parts)
                if context_parts
                else "No relevant file content found via vector search."
            )
            ...
            if len(combined_fuzzy_text) > max_fuzzy_chars:
                return (
                    combined_fuzzy_text[:max_fuzzy_chars]
    ...
    async def _extract_text_with_ocr_async(self, image_bytes: bytes) -> str:
        ...
        try:
            ...
            logger.info(
                f"{log_prefix}: OCR successful. Found {len(detected_texts)} text fragments."
            )
            ...
            logger.error(
                f"{log_prefix}: An error occurred during OCR processing: {e}",
                exc_info=True,
            )
    ...
    async def _describe_image_async(
        ...
        logger.info(
            f"{log_prefix} Requesting augmented VLM description (type: {prompt_type}, is_avif: {is_avif})."
        )
        ...
        try:
            ...
            if is_avif:
                logger.info(
                    f"{log_prefix} Input is AVIF, converting to PNG for processing..."
                )
                converted_png_b64 = await asyncio.to_thread(
                    self._convert_avif_to_png_b64, image_b64
                )
                if not converted_png_b64:
                    error_msg = (
                        "Failed to convert stored AVIF image to PNG for VLM analysis."
                    )
                    logger.error(f"{log_prefix} {error_msg}")
                    return None, error_msg
            ...
            # --- 2. Stage 1: Run OCR ---
            extracted_ocr_text = await self._extract_text_with_ocr_async(
                image_bytes_for_processing
            )
            ...
            vlm_prompt_text = PROMPT_VLM_AUGMENTED_ANALYSIS.format(
            ...
            vlm_messages = [
                HumanMessage(content=[image_content_part, text_content_part])
            ]
            ...
            if response_text:
                logger.info(
                    f"{log_prefix} Logging raw VLM output as a distinct interaction."
                )
                try:
                    await asyncio.to_thread(
                        add_interaction,
                    ...
                    logger.error(
                        f"❌ {log_prefix} Failed to log raw VLM output: {e_log_vlm_raw}"
                    )
                    if db:
                        await asyncio.to_thread(db.rollback)
            ...
            if response_text and not (
                isinstance(response_text, str) and "ERROR" in response_text.upper()
            ):
                ...
                await asyncio.to_thread(
                    ...
                    mode="chat",
                    input_type="log_debug",
                    user_input=f"[VLM+OCR Success - {prompt_type}]",
                    llm_response=log_llm_response,
                )
                return description_output, None
            else:
                error_output = (
                    f"[VLM description call failed or returned error: {response_text}]"
                )
            ...
            await asyncio.to_thread(
                ...
                mode="chat",
                input_type="log_error",
                user_input=f"[VLM/OCR Desc Error - {prompt_type}]",
                llm_response=str(e),
            )
            return None, error_output
    ...
    def _is_valid_tot_json(self, parsed_json: Any) -> bool:
        ...
        if not required_keys.issubset(parsed_json.keys()):
            logger.warning(
                f"ToT JSON missing one or more required keys: {required_keys - set(parsed_json.keys())}"
            )
            return False
        ...
        if "requires_background_task" in parsed_json and not isinstance(
            parsed_json["requires_background_task"], bool
        ):
            logger.warning("ToT JSON 'requires_background_task' is not a boolean.")
            return False
        if "next_task_input" in parsed_json and not (
            parsed_json["next_task_input"] is None
            or isinstance(parsed_json["next_task_input"], str)
        ):
            logger.warning("ToT JSON 'next_task_input' is not a string or null.")
            return False
        if parsed_json.get("requires_background_task") is True and not parsed_json.get(
            "next_task_input"
        ):
            logger.warning(
                "ToT JSON 'requires_background_task' is true, but 'next_task_input' is missing or empty."
            )
    ...
    def reset(self, db: Session, session_id: str = None):
        ...
        try:
            add_interaction(
                ...
                mode="chat",
                input_type="system",
                user_input="Chat Session Reset Requested",
                llm_response="Chat state cleared.",
            )
        except Exception as db_err:
    ...
    async def _run_image_latex_analysis_stream(
        self,
        db: Session,
        session_id: str,
        image_content_part: Dict,
        user_input: str,
        ...
        logger.info(
            f"📸 {stream_id}: Starting STREAMING analysis for LaTeX/TikZ. Input: '{user_input[:50]}...'"
        )
        ...
        try:
            yield format_sse(
                {
                    "status": "Initializing LaTeX/Visual Model...",
                    "stream_id": stream_id,
                },
                event_type="progress",
            )
            ...
            logger.error(
                f"Error yielding initial progress for {stream_id}: {yield_err}"
            )
        ...
        if not latex_model:
            ...
            yield format_sse(
                {"error": error_msg, "final": True, "stream_id": stream_id},
                event_type="error",
            )
            ...
            try:
                add_interaction(
                    ...
                    mode="chat",
                    input_type="error",
                    user_input="[Image LaTeX/TikZ Init Failed]",
                    llm_response=error_msg,
                )
            except Exception as db_log_err:
                logger.error(f"Failed to log LaTeX model config error: {db_log_err}")
        ...
        messages = [
            HumanMessage(
                content=[
                    image_content_part,
                    {"type": "text", "text": PROMPT_IMAGE_TO_LATEX},
                ]
            )
        ]
        # Ensure the chain uses the correct model instance
        chain = (
            latex_model | StrOutputParser()
        )  # Assumes StrOutputParser works with stream

        yield format_sse(
            {
                "status": "Sending request to LaTeX/Visual Model...",
                "stream_id": stream_id,
            },
            event_type="progress",
        )
        ...
        try:
            ...
            async for chunk in chain.astream(messages):
                ...
                if isinstance(chunk, str):
                    ...
                    logger.warning(
                        f"{stream_id}: Received non-string chunk during stream: {type(chunk)} - {str(chunk)[:100]}"
                    )

            llm_execution_time_ms = (time.monotonic() - llm_call_start_time) * 1000
            logger.info(
                f"📄 {stream_id}: LaTeX/VLM Raw Stream Complete. Approx Tokens: {token_count}, Duration: {llm_execution_time_ms:.2f} ms. Final Length: {len(full_response_markdown)}"
            )
            yield format_sse(
                {
                    "status": "LLM stream complete. Processing response...",
                    "stream_id": stream_id,
                },
                event_type="progress",
            )
            ...
            latex_match = re.search(
                r"```latex\s*(.*?)\s*```", full_response_markdown, re.DOTALL
            )
            if latex_match:
                latex_code = latex_match.group(1).strip()
                logger.info(
                    f"{stream_id}: Found LaTeX code block ({len(latex_code)} chars)."
                )

            # Regex to find ```tikz ... ``` block
            tikz_match = re.search(
                r"```tikz\s*(.*?)\s*```", full_response_markdown, re.DOTALL
            )
            if tikz_match:
                tikz_code = tikz_match.group(1).strip()
                logger.info(
                    f"{stream_id}: Found TikZ code block ({len(tikz_code)} chars)."
                )
            ...
            if len(parts) > 1:
                ...
                explanation = (
                    description  # Set explanation to description if no clear split
                )
                logger.debug(
                    f"{stream_id}: Using combined text as description/explanation."
                )
            ...
            logger.debug(
                f"{stream_id}: Final Parsed -> Desc:'{description[:50]}...', LaTeX:{latex_code is not None}, TikZ:{tikz_code is not None}, Explain:'{explanation[:50]}...'"
            )
            ...
            try:
                add_interaction(
                    ...
                    mode="chat",
                    input_type="error",
                    user_input=f"[Image LaTeX/TikZ Failed Stream {stream_id}]",
                    llm_response=f"{error_msg}\nRaw Response Snippet: {full_response_markdown[:500]}",
                )
            except Exception as db_log_err:
                logger.error(f"Failed to log LaTeX stream error: {db_log_err}")
            ...
            yield format_sse(
                {
                    "status": f"Stream ended ({final_status}).",
                    "final": True,
                    "stream_id": stream_id,
            ...
            logger.info(
                f"🏁 {stream_id}: LaTeX/VLM analysis stream finished. Status: {final_status}, Duration: {total_duration_ms:.2f} ms"
            )
            ...
            if final_status == "success":
                try:
                    ...
                    logger.debug(
                        f"{stream_id}: Logged successful LaTeX/TikZ analysis results to DB."
                    )
                except Exception as db_log_err:
                    logger.error(
    ...
    def process_url(self, db: Session, url: str, session_id: str = None):
        ...
        interaction_data = {
            ...
            "mode": "chat",
            "input_type": "url",
            "user_input": f"[URL Submitted: {url}]",
            "url_processed": url,
        }
    ...
    def extract_context_through_referencePath(self, url):
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    f"⚠️ URL content type ({content_type}) not standard text."
                )
                try:
    ...
    def create_vectorstore_for_url(self, text: str, url: str):
        ...
        text_splitter = RecursiveCharacterTextSplitter(
            chunk_size=VECTOR_CALC_CHUNK_BATCH_TOKEN_SIZE, chunk_overlap=CHUNK_OVERLAP
        )
    ...
    async def _parse_ingested_text_content(
        ...
        if isinstance(messages, list) and len(messages) >= 1:
            ...
            if first_user_msg:
                user_input_content = first_user_msg
            if first_asst_msg:
                assistant_response_content = first_asst_msg
            if user_input_content or assistant_response_content:
    ...
    async def _initiate_file_ingestion_and_reflection(
        ...
        logger.info(
            f"🚀 {current_job_id}: Starting UNIVERSAL background file ingestion."
        )
        ...
        try:
            ...
            if not uploaded_record:
                raise FileNotFoundError(
                    f"UploadedFileRecord ID {uploaded_file_record_id} not found."
                )
            ...
            if is_raw_text_fallback:
                ...
                for data_entry in loaded_records:
                    ...
                    if "messages" in keys_lower:
                        ...
                        if isinstance(msgs, list):
                            ...
                            (
                                ...
                                asst_content,
                                _,
                            ) = await self._parse_ingested_text_content(
                                {"messages": msgs}
                            )
                    ...
                    if not user_content:
                        if "user" in keys_lower:
                            ...
                            inp = (
                                ...
                                if "input" in keys_lower
                                and pd.notna(data_entry[keys_lower["input"]])
                                else ""
                            )
                    ...
                    if (
                        reasoning_content
                        and pd.notna(reasoning_content)
            ...
            logger.error(
                f"{current_job_id}: Error during ingestion: {e}", exc_info=True
            )
            if bg_db_session:
...
"""
    ...
    async def background_generate(
        ...
        temporal_anchor = self._get_temporal_context_string()
        user_input = f"as context This is current time{temporal_anchor}\n\n{user_input}"
        ...
        if db is None:
            try:
                ...
                logger.debug(
                    f"{log_prefix} Created local DB session for this background task."
                )
            except Exception as e_db_create:
                logger.error(
                    f"{log_prefix} CRITICAL: Failed to create local DB session: {e_db_create}. Aborting."
                )
                return

        if session_id is None:
            session_id = (
                f"session_{int(time.monotonic())}"
        ...
        interaction_data: Dict[str, Any] = {
            ...
            "rag_history_ids": None,
            "rag_source_url": None,
            "requires_deep_thought": (classification == "chat_complex")
            or is_reflection_task,
            "deep_thought_reason": None,
            "tot_analysis_requested": False,
            ...
            "assistant_action_analysis_json": None,
            "assistant_action_type": None,
            "assistant_action_params": None,
        ...
        final_response_text_for_this_turn = (
            "Error: Background processing did not complete as expected."
        )
        ...
        if is_reflection_task:
            try:
                ...
                source_interaction_for_reflection = await asyncio.to_thread(
                    db.query(Interaction)
                    .filter(Interaction.id == update_interaction_id)
                    .first
                )
                ...
                logger.info(
                    f"{log_prefix} Creating initial stub record for this reflection task."
                )
        ...
        logger.info(
            f"   - Is DCTD_ENABLE_QUANTUM_PREDICTION True? -> {DCTD_ENABLE_QUANTUM_PREDICTION}"
        )
        logger.info(
            f"   - Is self.branch_predictor None? -> {self.branch_predictor is None}"
        )
        logger.info(
        ...
        if DCTD_ENABLE_QUANTUM_PREDICTION and self.branch_predictor:
            try:
                ...
                temporal_sieve_data = await asyncio.to_thread(
                    self._get_temporal_sieve_data, db
                )
                ...
                current_embedding_list = await asyncio.to_thread(
                    self.provider.embeddings.embed_documents,
                    [user_input],
                    priority=ELP1,
                )
                current_embedding = (
                    np.array(current_embedding_list[0], dtype=np.float32)
                ...
                prev_interactions = await asyncio.to_thread(
                    get_recent_interactions, db, limit=1
                )
                ...
                if temporal_sieve_data and current_embedding is not None:
                    ...
                    if prediction_result:
                        ...
                        most_similar = await asyncio.to_thread(
                            ...
                            predicted_vector,
                            await asyncio.to_thread(
                                get_recent_interactions, db, limit=100
                            ),
                        )
        ...
        try:
            ...
            while server_is_busy_event.is_set():
                if stop_event_for_bg and stop_event_for_bg.is_set():
                    raise TaskInterruptedException(
                        "Background task stopped during politeness wait."
                    )
                if not was_busy_waiting:
                    logger.info(
                        f"🚦 {log_prefix} Server busy, pausing background task start..."
                    )
                    was_busy_waiting = True
                await asyncio.sleep(1.0)
            if was_busy_waiting:
                logger.info(f"🟢 {log_prefix} Server free, resuming background task.")
            ...
            while current_attempt <= max_retries_for_bg_task:
                ...
                try:
                    ...
                    logger.info(
                        f"{log_prefix} Deciding whether to generate an image..."
                    )
                    temp_history = await asyncio.to_thread(
                        get_global_recent_interactions, db, limit=3
                    )
                    ...
                    if should_imagine:
                        ...
                        temp_history_rag_docs = await asyncio.to_thread(
                            get_recent_interactions, db, 10, session_id, "chat"
                        )
                        temp_history_rag_str = self._format_docs(
                            temp_history_rag_docs, "History RAG"
                        )
                        temp_direct_history_str = self._format_direct_history(
                            temp_history
                        )
                        ...
                        img_prompt = await self._generate_image_generation_prompt_async(
                            ...
                            user_input,
                            idea_to_viz,
                            temp_history_rag_str,
                            "",
                            temp_direct_history_str,
                            "",
                            "",
                        )
                        ...
                        if img_prompt:
                            ...
                            if img_err:
                                ...
                                interaction_data["imagined_image_vlm_description"] = (
                                    gen_vlm_desc
                                )

                                if gen_vlm_desc:
                                    logger.info(
                                        f"{log_prefix} Logging VLM interpretation of imagined image."
                                    )
                                    try:
                                        await asyncio.to_thread(
                                            add_interaction,
                    ...
                    logger.info(
                        f"{log_prefix} Gathering final comprehensive context for response..."
                    )
                    ...
                    if url_ret_obj:
                        url_docs = await asyncio.to_thread(
                            url_ret_obj.invoke, current_input_for_analysis
                        )
                    if sess_hist_ret_obj:
                        session_docs = await asyncio.to_thread(
                            sess_hist_ret_obj.invoke, current_input_for_analysis
                        )
                    if refl_chunk_ret_obj:
                        reflection_docs = await asyncio.to_thread(
                            refl_chunk_ret_obj.invoke, current_input_for_analysis
                        )

                    global_hist_final = await asyncio.to_thread(
                        get_global_recent_interactions, db, limit=5
                    )
                    direct_hist_prompt_final = self._format_direct_history(
                        global_hist_final
                    )
                    log_entries_final = await asyncio.to_thread(
                        get_recent_interactions,
                        db,
                        RAG_HISTORY_COUNT * 2,
                        session_id,
                    ...
                    emotion_analysis_str_final = await asyncio.to_thread(
                        self._run_emotion_analysis, db, user_input, interaction_data
                    )

                    vec_file_ctx_result_str = (
                        await self._get_vector_search_file_index_context(
                            current_input_for_analysis,
                            session_id,
                            ELP0,
                            stop_event_for_bg,
                    ...
                    history_rag_str = self._format_docs(
                        session_docs + reflection_docs, "History/Reflection RAG"
                    )
                    ...
                    if action_details:
                        interaction_data.update(
                            {
                                "assistant_action_analysis_json": json.dumps(
                                    action_details
                                ),
                        ...
                        await asyncio.to_thread(
                            ...
                            llm_response=json.dumps(action_details)
                            if action_details
                            else '{"action_type": "no_action"}',
                        )
                        await asyncio.to_thread(db.commit)
                    ...
                    if detected_action_type != "no_action" and action_details:
                        ...
                        initial_synthesis_or_action_result = (
                            await self._execute_assistant_action(
                                db,
                                session_id,
                                action_details,
                        ...
                        router_payload = {
                            ...
                            "history_rag": self._sanitize_text_for_prompt(
                                history_rag_str
                            ),
                            "file_index_context": vec_file_ctx_result_str,
                            "log_context": log_ctx_prompt_final,
                        ...
                        role, query, reason = await self._route_to_specialist(
                            db, session_id, current_input_for_analysis, router_payload
                        )
                        interaction_data["classification_reason"] = (
                            f"Routed to {role}: {reason}"
                        )
                        await asyncio.to_thread(
                            add_interaction,
                            db,
                        ...
                        specialist_payload["input"] = (
                            query  # Use the refined query from the router
                        )
                        specialist_prompt_template = self.text_prompt_template
                        formatted_prompt_for_specialist = (
                            specialist_prompt_template.format(**specialist_payload)
                        )
                        ...
                        if (
                            draft_response
                            and "ERROR" not in str(draft_response).upper()
                        ):
                            logger.info(
                                f"{log_prefix} Logging initial draft from '{role}' specialist."
                            )
                            try:
                                await asyncio.to_thread(
                                    add_interaction,
                                ...
                                logger.error(
                                    f"❌ {log_prefix} Failed to log specialist draft: {e_log_draft}"
                                )
                                if db:
                                    await asyncio.to_thread(db.rollback)
                        ...
                        if ENABLE_PER_STEP_SOCRATIC_INQUIRY and draft_response:
                            asyncio.create_task(
                                self._spawn_socratic_inquiry_for_text(
                        ...
                        corrector_prompt_input = {
                            ...
                            "file_index_context": specialist_payload.get(
                                "file_index_context", ""
                            ),
                            "log_context": specialist_payload.get("log_context", ""),
                            "recent_direct_history": specialist_payload.get(
                                "recent_direct_history", ""
                            ),
                            "emotion_analysis": specialist_payload.get(
                                "emotion_analysis", ""
                            ),
                            "draft_response": draft_response,
                        }
                        ...
                        corrector_prompt_template = ChatPromptTemplate.from_template(
                            PROMPT_CORRECTOR
                        )
                        formatted_prompt_for_corrector = (
                            corrector_prompt_template.format(**corrector_prompt_input)
                        )
                        ...
                        if (
                            initial_synthesis_or_action_result
                            and "ERROR"
                            not in str(initial_synthesis_or_action_result).upper()
                        ):
                            ...
                            try:
                                ...
                                logger.error(
                                    f"❌ {log_prefix} Failed to log corrected synthesis: {e_log_corrected}"
                                )
                                if db:
                                    await asyncio.to_thread(db.rollback)

                        if (
                            ENABLE_PER_STEP_SOCRATIC_INQUIRY
                            and initial_synthesis_or_action_result
                        ):
                            asyncio.create_task(
                                self._spawn_socratic_inquiry_for_text(
                    ...
                    for chunk_num in range(BACKGROUND_MAX_CHUNKS):
                        ...
                        dynamic_rag_docs = []
                        if sess_hist_ret_obj:
                            dynamic_rag_docs = await asyncio.to_thread(
                                sess_hist_ret_obj.invoke, dynamic_rag_query
                            )
                        dynamic_rag_context_str = self._format_docs(
                            dynamic_rag_docs, "Dynamic RAG"
                        )
                        ...
                        formatted_prompt_for_log = elaboration_prompt_template.format(
                            **elaboration_prompt_input
                        )
                        ...
                        if SELF_TERMINATION_TOKEN in generated_chunk_raw:
                            is_elaboration_finished = True
                            generated_chunk_raw = generated_chunk_raw.split(
                    ...
                    final_response_text_for_this_turn = "".join(
                        elaborated_chunks_array
                    ).strip()

                    logger.info(
                        f"{log_prefix} Cleaning the final assembled response before summarization..."
                    )
                    ...
                    final_save_data.update(
                        {
                            "llm_response": self._cleanup_llm_output(
                                final_response_text_for_this_turn
                            ),
                            "execution_time_ms": (time.monotonic() - request_start_time)
                            * 1000.0,
                        }
                    )
                    ...
                    if is_reflection_task:
                        ...
                        source_interaction_type = (
                            source_interaction_for_reflection.input_type
                            if source_interaction_for_reflection
                            else "unknown"
                        )
                    ...
                    try:
                        valid_keys_final = {
                            c.name for c in Interaction.__table__.columns
                        }
                        db_kwargs_final = {
                            k: v
                            for k, v in final_save_data.items()
                            if k in valid_keys_final and k != "id"
                        }

                        # Add the new record to the database
                        newly_created_record = await asyncio.to_thread(
                            add_interaction, db, **db_kwargs_final
                        )
                        await asyncio.to_thread(db.commit)
                        logger.success(
                        ...
                        logger.error(
                            f"❌ {log_prefix} CRITICAL: Failed to save the final result record: {e_final_save}"
                        )
                        if db:
                            await asyncio.to_thread(db.rollback)
                    ...
                    logger.info(
                        f"{log_prefix} Starting multi-language summarization and translation."
                    )

                    if (
                        final_response_text_for_this_turn
                        and final_response_text_for_this_turn.strip()
                    ):
                        logger.info(
                            f"{log_prefix} Starting multi-language summarization and translation."
                        )
                        ...
                        if not translator_model:
                            ...
                            formatted_prompt_for_summary = (
                                summary_prompt_template.format(**summary_prompt_input)
                            )
                            ...
                            try:
                                ...
                                if raw_summary_output:
                                    logger.info(
                                        f"{log_prefix} Logging multi-language summary thought."
                                    )
                                    await asyncio.to_thread(
                                        add_interaction,
                                        db,
                                ...
                                json_candidate = self._extract_json_candidate_string(
                                    raw_summary_output, log_prefix + "-SummaryExtract"
                                )
                                if json_candidate:
                                    parsed_summaries = (
                                        self._programmatic_json_parse_and_fix(
                                            json_candidate,
                                            log_prefix=log_prefix + "-SummaryFix",
                                        )
                                    )
                                    if parsed_summaries and isinstance(
                                        parsed_summaries, dict
                                    ):
                                        interaction_data["summary_original_lang"] = (
                                            parsed_summaries.get(
                                                "summary_original_lang", ""
                                            )
                                        )
                                        interaction_data["summary_en"] = (
                                            parsed_summaries.get("summary_en", "")
                                        )
                                        interaction_data["summary_zh"] = (
                                            parsed_summaries.get("summary_zh", "")
                                        )
                                        logger.info(
                                            f"{log_prefix} Multi-language summaries generated and stored."
                                        )
                                    else:
                                        logger.warning(
                                            f"{log_prefix} Failed to parse multi-language summary JSON."
                                        )
                                else:
                                    logger.warning(
                                        f"{log_prefix} No JSON candidate found for multi-language summary."
                                    )

                            except Exception as e:
                                logger.error(
                                    f"❌ {log_prefix} Error during multi-language summarization: {e}",
                                    exc_info=True,
                                )
...
"""
                    ...
                    if (
                        ...
                        try:
                            ...
                            should_create = True
                            reason = "Forced always-on generation (picoResponseHookCache strategy)."
                            if should_create:
                                await asyncio.to_thread(
                                    add_interaction,
                                    db,
                    ...
                    logger.warning(
                        f"🚦 {log_prefix} Attempt {current_attempt} INTERRUPTED: {tie}"
                    )
                    ...
                    logger.exception(
                        f"{log_prefix} Attempt {current_attempt} Traceback:"
                    )
                ...
                if ENABLE_SOCRATIC_QUESTION_GENERATION and task_completed_successfully:
                    ...
                    # 2. Manually format the prompt for logging.
                    socratic_prompt_template = ChatPromptTemplate.from_template(
                        PROMPT_GENERATE_SOCRATIC_QUESTION
                    )
                    formatted_prompt_for_socratic = socratic_prompt_template.format(
                        **socratic_prompt_input
                    )

                    # 3. Call the helper (which calls the LLM).
                    generated_socratic_question = (
                        await self._generate_socratic_question_async(
                            db=db,
                    ...
                    if generated_socratic_question:
                        ...
                        try:
                            ...
                            logger.error(
                                f"❌ {log_prefix} Failed to save Socratic question to DB: {e_save_socratic}"
                            )
                            if db:
                                await asyncio.to_thread(db.rollback)
                    else:
            ...
            logger.critical(
                f"🔥🔥 {log_prefix} CRITICAL UNHANDLED exception in outer block: {e_outer}"
            )
            logger.exception(f"{log_prefix} Outer Traceback:")
            final_response_text_for_this_turn = (
                f"Critical Error: {type(e_outer).__name__}"
            )
            interaction_data.update(
                {
                    "llm_response": final_response_text_for_this_turn,
        ...
        # ======================================================================
            ...
            if semaphore_acquired_for_task:
                ...
                logger.info(
                    f"{log_prefix} Released background_generate_task_semaphore."
                )
            ...
            try:
                if not task_completed_successfully:
                    logger.error(
                        f"{log_prefix} Task did not complete successfully. Saving a final failure record."
                    )
                    # Determine the source for the error log
                    source_info = (
                        f"Source Reflection ID: {update_interaction_id}"
                        if is_reflection_task
                        else f"Original User Input: {user_input}"
                    )
                    ...
                    logger.info(
                        f"{log_prefix} Task successful. Putting result into callback queue..."
                    )
                    ...
                    if source_interaction:
                        ...
                        logger.info(
                            f"{log_prefix} Successfully queued result for delivery."
                        )
                    else:
                        logger.warning(
...
def download_content_sync(
    url: str, download_dir: str, filename_prefix: str, timeout: int = 30
) -> bool:
    """
    ...
    try:
        ...
        }  # Use helper if available, or hardcode one
        ...
        with requests.get(
                ...
                for part in disp_parts:
                    if part.strip().lower().startswith("filename="):
                        ...
                        final_filename = sanitize_filename(
                            final_filename or filename_prefix
                        )
                        download_logger.debug(
                            f"Using filename from Content-Disposition: {final_filename}"
                        )
                        break
            ...
            if not final_filename:
                ...
                if not extension:
                    try:
                        ...
                        if (
                            potential_ext and len(potential_ext) < 10
                        ):  # Basic check for valid-looking extension
                            extension = potential_ext
                    except Exception:
                        pass  # Ignore errors parsing URL path extension
                ...
                if not extension:
                    if "html" in content_type:
                        ...
                        extension = ".download"  # Generic fallback

                # Combine sanitized prefix and extension
                final_filename = f"{filename_prefix}{extension}"
                download_logger.debug(
                    f"Generated filename: {final_filename} (Type: {content_type}, Ext: {extension})"
                )
            ...
            download_logger.success(
                f"Download complete. Saved {total_bytes} bytes in {chunk_count} chunks."
            )
            return True
        ...
        if "save_path" in locals() and os.path.exists(save_path):
            try:
                os.remove(save_path)
                download_logger.warning("Removed partial file after save error.")
            except Exception as rm_err:
                download_logger.error(f"Failed to remove partial file: {rm_err}")
        return False
    except Exception as e:
        download_logger.error(f"An unexpected error occurred during download: {e}")
        logger.exception(
            "Download Unexpected Error Traceback:"
...
def _format_ollama_chat_response_nonstream(
    response_text: str, model_name: str, total_duration_ns: int, eval_duration_ns: int
) -> Dict[str, Any]:
    """
...
async def _log_sanitized_interaction(
    original_query: Any, final_response: Any, session_id: str, mode: str
):
    """
    ...
    try:
        ...
        logger.info(
            f"{log_prefix}: Starting sanitization for session {session_id}, mode '{mode}'."
        )
        ...
        if mode in ["chat_mesh", "vlm_chat_mesh"]:
            ...
            if isinstance(original_query, dict) and "content" in original_query:
                ...
                if isinstance(original_query["content"], list):
                    query_text = " ".join(
                        [
                            part.get("text", "")
                            for part in original_query["content"]
            ...
            response_text = str(
                final_response
            )  # e.g., "[Audio generated successfully]"

        elif mode == "image_gen_mesh":
            query_text = str(original_query)
            response_text = str(
                final_response
            )  # e.g., "[1 image(s) generated successfully]"
        ...
        sanitizer_prompt = PROMPT_SANITIZE_FOR_LOGGING.format(
            original_query_text=query_text, original_response_text=response_text
        )
        ...
        try:
            ...
            json_candidate = cortex_text_interaction._extract_json_candidate_string(
                sanitized_json_str, log_prefix
            )
            if json_candidate:
                sanitized_data = (
                    cortex_text_interaction._programmatic_json_parse_and_fix(
                        json_candidate, log_prefix=log_prefix
                    )
                )
                if sanitized_data:
                    sanitized_query_to_save = sanitized_data.get(
                        "sanitized_query", sanitized_query_to_save
                    )
                    sanitized_response_to_save = sanitized_data.get(
                        "sanitized_response", sanitized_response_to_save
                    )
        ...
        db.commit()
        logger.success(
            f"{log_prefix}: Successfully logged sanitized interaction for session {session_id}."
        )

    except Exception as e:
        logger.error(
            f"{log_prefix}: A critical error occurred during the sanitization and logging task: {e}"
        )
        logger.exception(f"{log_prefix} Traceback:")
        if db:
            db.rollback()
...
async def _process_mesh_chat_request_and_get_result(
    request_data: dict, db_session: Session, session_id: str, request_id: str
) -> str:
    """
    ...
    logger.info(
        f"{log_prefix}: Starting ELP0 Chat/VLM processing for session {session_id}."
    )
    ...
    if image_b64:
        logger.info(
            f"{log_prefix}: Image detected. Getting VLM description at ELP0 priority."
        )
        ...
        if vlm_err:
            logger.warning(
                f"{log_prefix}: VLM description failed: {vlm_err}. Proceeding with text only."
            )
        elif vlm_desc:
    ...
    logger.info(
        f"{log_prefix}: Executing the full, sequential ELP0 pipeline to get a result."
    )
    ...
    if wrapped_rag_res.get("status") != "success":
        raise RuntimeError(
            f"RAG retrieval failed: {wrapped_rag_res.get('error_message')}"
        )

    url_ret, sess_hist_ret, refl_chunk_ret, _ = wrapped_rag_res.get(
        "data", (None, None, None, None)
    )

    url_docs, session_docs, reflection_docs = [], [], []
    if url_ret:
        url_docs = await asyncio.to_thread(url_ret.invoke, current_input_for_analysis)
    if sess_hist_ret:
        session_docs = await asyncio.to_thread(
            sess_hist_ret.invoke, current_input_for_analysis
        )
    if refl_chunk_ret:
        reflection_docs = await asyncio.to_thread(
            refl_chunk_ret.invoke, current_input_for_analysis
        )

    url_context_str = cortex_text_interaction._format_docs(url_docs, "URL Context")
    history_rag_str = cortex_text_interaction._format_docs(
        session_docs + reflection_docs, "History/Reflection RAG"
    )
    # ... (Other context gathering like direct history, logs, etc.)
    direct_hist_interactions = await asyncio.to_thread(
        get_global_recent_interactions, db_session, limit=5
    )
    recent_direct_history_str = cortex_text_interaction._format_direct_history(
        direct_hist_interactions
    )
    ...
    logger.debug(
        f"{log_prefix}: Generating draft response with '{role}' model at ELP0..."
    )
    ...
    specialist_payload = router_payload.copy()
    specialist_payload["input"] = (
        query  # Use the (potentially refined) query from the router
    )
    specialist_chain = (
        cortex_text_interaction.text_prompt_template
        | specialist_model
        | StrOutputParser()
    )
    timing_data_specialist = {
        "session_id": session_id,
        "mode": f"mesh_chat_specialist_{role}",
    }
...
async def _process_mesh_tts_request_and_get_result(
    request_data: dict, request_id: str
) -> Tuple[bytes, str]:
    """
    ...
    if not input_text or not isinstance(input_text, str):
        raise ValueError(
            "'input' field is required and must be a non-empty string for TTS."
        )
    if not voice_requested or not isinstance(voice_requested, str):
        raise ValueError(
            "'voice' field (MeloTTS speaker ID, e.g., EN-US) is required for TTS."
        )
    ...
    try:
        ...
        logger.warning(
            f"{log_prefix}: Could not infer language from voice '{voice_requested}'. Defaulting to EN."
        )
    ...
    if not os.path.exists(audio_worker_script_path):
        raise RuntimeError(
            f"Audio worker script is missing at {audio_worker_script_path}"
        )
    ...
    logger.info(
        f"{log_prefix}: Executing audio worker with ELP0 priority and waiting for completion..."
    )
    ...
    if error_string_from_worker:
        ...
        parsed_response_from_worker
        and "result" in parsed_response_from_worker
        and "audio_base64" in parsed_response_from_worker["result"]
    ):
        ...
        raise RuntimeError(
            f"Audio worker returned an invalid or incomplete response: {parsed_response_from_worker}"
        )
...
async def _process_mesh_image_gen_request_and_get_result(
    ...
    logger.info(
        f"{log_prefix}: Starting ELP0 Image Generation processing for session {session_id}."
    )
    ...
    if not prompt_from_user or not isinstance(prompt_from_user, str):
        raise ValueError(
            "'prompt' field is required and must be a non-empty string for image generation."
        )
    ...
    logger.info(
        f"{log_prefix}: Refining user prompt with RAG context at ELP0 priority..."
    )
    ...
    if wrapped_rag_result.get("status") != "success":
        raise RuntimeError(
            f"RAG for image prompt refinement failed: {wrapped_rag_result.get('error_message')}"
        )

    _, session_hist_retriever, _, _ = wrapped_rag_result.get(
        "data", (None, None, None, None)
    )

    retrieved_history_docs = []
    if session_hist_retriever:
        retrieved_history_docs = await asyncio.to_thread(
            session_hist_retriever.invoke, prompt_from_user
        )

    history_rag_str = cortex_text_interaction._format_docs(
        retrieved_history_docs, source_type="History RAG"
    )

    # Get direct history for additional context
    direct_hist_interactions_list = await asyncio.to_thread(
        get_global_recent_interactions, db_session, limit=3
    )
    recent_direct_history_str = cortex_text_interaction._format_direct_history(
        direct_hist_interactions_list
    )
    ...
    if not refined_prompt:
        logger.warning(
            f"{log_prefix}: Prompt refinement failed. Using original user prompt."
        )
        refined_prompt = prompt_from_user
    else:
        logger.info(
            f"{log_prefix}: Using refined prompt for generation: '{refined_prompt[:100]}...'"
        )

    # --- Step 3: Generate Image(s) with ELP0 Priority and Wait for the Result ---
    logger.info(
        f"{log_prefix}: Requesting {n_images} image(s) from CortexEngine at ELP0 priority..."
    )
    ...
    for i in range(n_images):
        ...
        (
            list_of_one_image_dict,
            image_gen_err,
        ) = await cortex_backbone_provider.generate_image_async(
        ...
        if image_gen_err:
            raise RuntimeError(
                f"Image generation failed for attempt {i + 1}: {image_gen_err}"
            )
        elif list_of_one_image_dict and list_of_one_image_dict[0]:
            all_generated_image_data.append(list_of_one_image_dict[0])
        else:
            raise RuntimeError(
                f"Image generation attempt {i + 1} returned no data and no error."
            )

    if not all_generated_image_data:
        raise RuntimeError(
            "Image generation pipeline failed to produce any valid image data."
        )

    logger.success(
        f"{log_prefix}: Successfully generated {len(all_generated_image_data)} image(s)."
    )
...
async def _log_sanitized_asr_interaction(
    raw_transcription: str, analyzed_response: str, session_id: str, request_id: str
):
    """
    ...
    try:
        ...
        logger.info(
            f"{log_prefix}: Sanitizing ASR interaction for session {session_id}."
        )
        sanitizer_prompt = PROMPT_SANITIZE_FOR_LOGGING.format(
        ...
        try:
            ...
            sanitized_response = sanitized_data.get(
                "sanitized_response", sanitized_response
            )
        except (json.JSONDecodeError, TypeError):
            logger.warning(
                f"{log_prefix}: Failed to parse sanitizer JSON. Logging raw-like data."
            )
        ...
        db.commit()
        logger.success(
            f"{log_prefix}: Successfully logged sanitized ASR interaction for session {session_id}."
        )
...
async def _process_mesh_asr_request_and_get_result(
    audio_file_path: str, language: str, session_id: str, request_id: str
) -> str:
    """
    ...
    logger.info(
        f"{log_prefix}: Starting user-facing ELP0 ASR pipeline for file: {audio_file_path}"
    )
    ...
    final_analyzed_text_for_user: str = (
        "[ASR pipeline did not produce a final analysis]"
    )
    ...
    try:
        ...
        if ll_asr_err:
            ...
            raw_transcription = (
                ll_asr_response.get("result", {}).get("text") or ""
            ).strip()

        # --- STEP 2: Sanity Check and Conditional High-Quality Retry at ELP0 ---
        is_transcription_valid = True
        if (
            not raw_transcription
            or len(raw_transcription) < 3
            or raw_transcription.lower() in WHISPER_GARBAGE_OUTPUTS
        ):
            is_transcription_valid = False

        if is_transcription_valid:
            logger.info(
                f"{log_prefix}: Step 2 - Low-latency ASR produced valid text. Using this result."
            )
            ...
            if hq_asr_err:
                ...
                raw_transcription = (
                    ""  # Final result is an empty string, indicating total failure
                )
            else:
                raw_transcription = (
                    hq_asr_response.get("result", {}).get("text") or ""
                ).strip()
        ...
        if raw_transcription:
            logger.info(
                f"{log_prefix}: Step 3 - Submitting RAW transcript for final analysis for the user."
            )
            ...
            final_analyzed_text_for_user = (
                await cortex_text_interaction.direct_generate(
                    db,
            ...
            final_analyzed_text_for_user = (
                "[Transcription failed or audio was silent after processing.]"
            )

    except Exception as e:
        logger.exception(
            f"{log_prefix}: A critical error occurred in the user-facing ASR pipeline:"
        )
        ...
        logger.info(
            f"{log_prefix}: Spawning background task to sanitize and log the interaction."
        )
        asyncio.create_task(
        ...
        if audio_file_path and os.path.exists(audio_file_path):
            try:
                ...
                logger.warning(
                    f"{log_prefix}: Failed to clean up temp audio file: {e_del}"
                )
...
async def _process_mesh_chat_pipeline_elp0(
    messages: list, session_id: str, request_id: str
):
    """
...
def _async_compatible_openai_streamer(
    ...
    logger.info(
        f"ASYNC_STREAMER {resp_id}: Streaming pre-generated text ({len(full_response_text)} chars)."
    )
    ...
    try:
        ...
        if (
            "Error:" in full_response_text
            or "interrupted" in full_response_text.lower()
        ):
        ...
        yield "data: [DONE]\n\n"
        logger.info(
            f"ASYNC_STREAMER {resp_id}: Finished sending all chunks and [DONE] signal."
        )
...
def _pseudo_stream_sync_generator(
    ...
    timestamp = int(time.time())
    logger.info(
        f"SYNC_STREAM {resp_id}: Streaming pre-generated text ({len(full_response_text)} chars)."
    )
    ...
    try:
        ...
        if (
            ...
            logger.error(
                f"SYNC_STREAM {resp_id}: Streaming an error message provided by the main handler."
            )
            yield yield_chunk(delta_content=full_response_text)
            yield yield_chunk(finish_reason="error")
            return
        ...
        yield "data: [DONE]\n\n"
        logger.info(
            f"SYNC_STREAM {resp_id}: Finished sending all chunks and [DONE] signal."
        )


def _create_openai_error_response(
    message: str, err_type="internal_error", code=None, status_code=500
):
...
def _parse_ingested_text_content(
...
def _format_openai_chat_response(
    response_text: str,
    model_name: str = "Amaryllis-Adelaide-IdioticRecursiveLearner-LegacyMoEArch",
) -> Dict[str, Any]:
...
def _format_legacy_completion_response(
    response_text: str, model_name: str = META_MODEL_NAME_NONSTREAM
) -> Dict[str, Any]:
...
def _execute_audio_worker_with_priority(
    ...
    shared_priority_lock: Optional[PriorityQuotaLock] = getattr(
        cortex_backbone_provider, "_priority_quota_lock", None
    )
    ...
    if not shared_priority_lock:
        logger.error(
            f"{log_prefix}: Shared PriorityQuotaLock not available/initialized! Cannot run audio worker."
        )
    ...
    if lock_acquired:
        logger.info(
            f"{log_prefix}: Lock acquired (waited {lock_wait_duration:.2f}s). Starting audio worker."
        )
        try:
            ...
            logger.debug(
                f"{log_prefix}: Sending input JSON (len={len(input_json)}) to audio worker stdin..."
            )
            stdout_data, stderr_data = "", ""  # Initialize

            try:
                stdout_data, stderr_data = worker_process.communicate(
                    input=input_json, timeout=timeout
                )
                logger.debug(f"{log_prefix}: Audio worker communicate() finished.")
            except subprocess.TimeoutExpired:
                logger.error(
                    f"{log_prefix}: Audio worker process timed out after {timeout}s."
                )
                if worker_process and worker_process.poll() is None:
                    worker_process.kill()  # Ensure kill on timeout
                ...
                try:
                    ...
                    pass  # Best effort
                logger.error(
                    f"{log_prefix}: Worker timed out. Stderr: {stderr_data.strip() if stderr_data else 'N/A'}"
                )
                ...
                if (
                    worker_process and worker_process.poll() is None
                ...
                try:
                    ...
                    if (
                        worker_process
                    ):  # Only if worker_process was successfully created
                    ...
                    logger.warning(
                        f"{log_prefix}: Error getting final output after BrokenPipe: {e_bp_comm}"
                    )
                return (
                    None,
                    "Audio worker task interrupted by higher priority request.",
                )  # Use the consistent marker
            except Exception as comm_err:  # Other communication errors
                logger.error(
                    f"{log_prefix}: Error communicating with audio worker: {comm_err}"
                )
                if worker_process and worker_process.poll() is None:
                    try:
                        worker_process.kill()
                        worker_process.communicate()  # Best effort cleanup
                    except:
                        pass
                return None, f"Communication error with audio worker: {comm_err}"

            exit_code = (
                worker_process.returncode if worker_process else -1
            )  # Handle if worker_process is None
            duration = time.monotonic() - start_time
            logger.info(
                f"{log_prefix}: Audio worker finished. Exit Code: {exit_code}, Duration: {duration:.2f}s"
            )

            if stderr_data:  # Log stderr regardless of exit code for diagnostics
                log_level_stderr = "ERROR" if exit_code != 0 else "DEBUG"
                stderr_snippet = (
                    (stderr_data[:2000] + "...[TRUNCATED]")
                    if len(stderr_data) > 2000
                    else stderr_data
                )
                logger.log(
                    log_level_stderr,
            ...
            if exit_code == 0:
                if (
                    not stdout_data or not stdout_data.strip()
                ...
                try:
                    ...
                    if json_start_index == -1:
                        logger.error(
                            f"{log_prefix}: No JSON object start ('{{') found in audio worker stdout."
                        )
                        logger.error(
                            f"{log_prefix}: Raw stdout from worker (first 1000 chars):\n{stdout_data[:1000]}"
                        )
                        return (
                            None,
                            "Audio worker did not produce valid JSON output (no '{' found).",
                        )

                    json_string_to_parse = stdout_data[json_start_index:]
                    parsed_json = json.loads(json_string_to_parse)
                    logger.debug(
                        f"{log_prefix}: Parsed audio worker JSON response successfully."
                    )

                    if (
                        isinstance(parsed_json, dict) and "error" in parsed_json
                    ):  # Check if worker itself reported an error in JSON
                        logger.error(
                            f"{log_prefix}: Audio worker reported internal error: {parsed_json['error']}"
                        )
            ...
            Exception
        ) as e:  # Catch-all for unexpected errors in this function's try block
            ...
            if (
                worker_process and worker_process.poll() is None
            ):  # If Popen succeeded but later error
                try:
                    worker_process.kill()
                    worker_process.communicate()  # Best effort cleanup
                except:
        ...
        logger.error(
            f"{log_prefix}: FAILED to acquire shared resource lock for audio worker."
        )
...
async def _run_background_high_quality_asr(
    ...
    logger.info(
        f"{bg_asr_log_prefix}: Starting background high-quality ASR for audio: {original_audio_path}"
    )
    ...
    if SessionLocal is None:  # Ensure SessionLocal is imported from database.py
        logger.error(
            f"{bg_asr_log_prefix}: SessionLocal is None. Cannot create DB session for background ASR."
        )
        return
    ...
    if not db_bg_task:
        logger.error(
            f"{bg_asr_log_prefix}: Failed to create DB session for background ASR."
        )
        return
    ...
    try:
        ...
        asr_worker_cmd_bg = [
            APP_PYTHON_EXECUTABLE,
            asr_worker_script_bg,
            "--task-type",
            "asr",
            "--model-dir",
            WHISPER_MODEL_DIR,  # from CortexConfiguration
            "--temp-dir",
            os.path.join(SCRIPT_DIR, "temp_audio_worker_files"),  # Consistent temp dir
        ]
        asr_request_data_bg = {
        ...
        logger.info(
            f"{bg_asr_log_prefix}: Executing audio worker for high-quality ASR (ELP0)..."
        )
        ...
        hq_asr_response, hq_asr_err = await asyncio.to_thread(
            ...
            + 120,  # Potentially longer timeout for higher quality model
        )

        elp0_transcription: Optional[str] = None
        if hq_asr_err or not (
            hq_asr_response
            and isinstance(hq_asr_response.get("result"), dict)
            and "text" in hq_asr_response["result"]
        ):
            logger.error(
        ...
        logger.error(
            f"{bg_asr_log_prefix}: Error in background high-quality ASR task: {e_bg_asr}"
        )
        ...
        if db_bg_task:
            try:
                ...
                logger.error(
                    f"{bg_asr_log_prefix}: Failed to log background ASR error to DB: {e_db_log}"
                )
                if db_bg_task:
                    db_bg_task.rollback()
    finally:
...
async def _run_background_asr_and_translation_analysis(
    ...
    elp1_translation_final_for_client: Optional[
        str
    ],  # The quick translation returned to client
    session_id_for_log: str,
    request_id: str,
    language_asr: str,
    target_language_translation: Optional[
        str
    ],  # Target language code for translation (e.g., "es")
):
    bg_log_prefix = f"ASR_Translate_BG_ELP0|{request_id}"
    logger.info(
        f"{bg_log_prefix}: Starting background tasks for audio: {original_audio_path}"
    )
    ...
    try:
        if SessionLocal is None:
            logger.error(
                f"{bg_log_prefix}: SessionLocal is None. Cannot create DB session."
            )
            return
        db_bg_task = SessionLocal()  # type: ignore
        if not db_bg_task:
        ...
        asr_worker_cmd_bg = [
            ...
            WHISPER_MODEL_DIR,
            "--temp-dir",
            os.path.join(SCRIPT_DIR, "temp_audio_worker_files"),  # For ffmpeg in worker
        ]
        asr_request_data_bg = {
        ...
        logger.info(
            f"{bg_log_prefix}: Executing audio worker for high-quality ASR (ELP0) on {original_audio_path}..."
        )
        hq_asr_response, hq_asr_err = await asyncio.to_thread(
            _execute_audio_worker_with_priority,
            asr_worker_cmd_bg,
        ...
        if (
            ...
            and high_quality_transcribed_text
            and not high_quality_transcribed_text.startswith(
                "[High-Quality ASR (ELP0) Failed"
            )
        ):
            logger.info(
            ...
            source_lang_full = (
                langcodes.Language.make(language=language_asr).display_name()
                if language_asr != "auto"
                else "auto-detected"
            )
            target_lang_full = langcodes.Language.make(
                language=target_language_translation
            ).display_name()
            ...
            db_bg_task.commit()
            logger.info(
                f"{bg_log_prefix}: Logged translation comparison info and spawned deep translation."
            )
        ...
        if db_bg_task:
            try:
                ...
                logger.error(
                    f"{bg_log_prefix}: Failed to log background task error to DB: {e_db_log_bg_err}"
                )
                if db_bg_task:
                    db_bg_task.rollback()
    finally:
        ...
        if original_audio_path and os.path.exists(original_audio_path):
            try:
                ...
                )  # Use await for async context
                logger.info(
                    f"{bg_log_prefix}: Deleted original temporary input audio file: {original_audio_path}"
                )
...
async def run_startup_benchmark():
    ...
    if not cortex_text_interaction:
        logger.error(
            "DETERMENISM_ELP1_CALIBRATION: (CortexThoughts) instance not available. Skipping benchmark."
        )
        return
    ...
    try:
        ...
        logger.info(
            f"direct_generate() execution time and will be expected for ELP1 to response within: {BENCHMARK_ELP1_TIME_MS:.2f} ms"
        )  # Display in milliseconds
        logger.info("---------------------------------")

    except Exception as e:
        logger.error(
            f"DETERMENISM_ELP1_CALIBRATION: An error occurred during the startup DETERMENISM_ELP1_CALIBRATION: {e}"
        )
...
async def build_ada_daemons():
    ...
    logger.info(
        "... Scheduling build for StellaIcarus Ada daemons in background thread ..."
    )
...
_sim_state = {
    ...
    "hsi": {
        "selected_course": 185.0,
        "course_deviation": 0.1,
        "hac_turn_angle": 90.0,
        "waypoints": [
...
def _generate_simulated_avionics_data() -> Dict[str, Any]:
    ...
    with _sim_lock:
        ...
        if delta_t == 0:
            delta_t = 1.0 / 60.0  # Prevent division by zero if called too quickly
        ...
        _sim_state["roll"] = max(
            -60, min(60, last_roll + random.uniform(-1, 1) * delta_t * 2)
        )
        _sim_state["pitch"] = max(
            -30, min(30, last_pitch + random.uniform(-0.5, 0.5) * delta_t * 1)
        )
        ...
        _sim_state["yaw_rate"] = _sim_state["roll_rate"] * math.sin(
            math.radians(_sim_state["pitch"])
        )
        _sim_state["yaw"] = (_sim_state["yaw"] + _sim_state["yaw_rate"] * delta_t) % 360
        _sim_state["sideslip_angle"] = (
            _sim_state["yaw_rate"] * -0.1
        )  # Simplified sideslip

        # G-force based on bank angle
        _sim_state["g_force"] = (
            1.0 / math.cos(math.radians(_sim_state["roll"]))
            if abs(_sim_state["roll"]) < 85
            else 6.0
        )

        # Velocity and Position
        _sim_state["vertical_speed"] = max(
            -4000,
            min(
                4000,
                _sim_state["vertical_speed"] + random.uniform(-50, 50) * delta_t * 5,
            ),
        )
        _sim_state["altitude"] += _sim_state["vertical_speed"] * delta_t / 60.0
        _sim_state["keas"] = max(
            60, min(700, _sim_state["keas"] + random.uniform(-2, 2) * delta_t * 5)
        )
        try:
            # Calculate the raw Mach number based on physics
            raw_mach = _sim_state["keas"] / (
                661.47 * (1.0 - _sim_state["altitude"] / 145442.0) ** 2.5
            )
            ...
            ZeroDivisionError,
            ValueError,
        ):  # Added ValueError for potential math domain errors
            # If calculation fails, default to a safe value within the range
            _sim_state["mach"] = 0.8
        _sim_state["angle_of_attack"] = (
            2.5 + _sim_state["pitch"] / 4.0 - (_sim_state["keas"] - 150.0) / 50.0
        )

        # --- Simulate Flight Controls (mirroring pilot input) ---
        _sim_state["flight_controls"]["elevons"]["left_deg"] = (
            _sim_state["pitch"] * -0.5 + _sim_state["roll"] * 0.25
        )
        _sim_state["flight_controls"]["elevons"]["right_deg"] = (
            _sim_state["pitch"] * -0.5 - _sim_state["roll"] * 0.25
        )
        _sim_state["flight_controls"]["rudder"]["actual_deg"] = (
            _sim_state["yaw_rate"] * -0.5
        )
        _sim_state["flight_controls"]["rudder"]["commanded_deg"] = _sim_state[
            "flight_controls"
        ]["rudder"]["actual_deg"]
        ...
        payload = {
            ...
            "flight_dynamics": {
                ...
                "position": {"altitude": round(_sim_state["altitude"], 2)},
                "aero": {
...
def get_current_configurable_settings():
    ...
    for var_name in adjustable_vars:
        if hasattr(config, var_name):
            ...
            logger.warning(
                f"Config API: Variable '{var_name}' not found in CortexConfiguration.py"
            )
...
async def _get_and_process_proactive_interaction():
    ...
    try:
        ...
        decision_chain = (
            ChatPromptTemplate.from_template(PROMPT_XMPP_SHOULD_I_RECALL)
            | cortex_text_interaction.provider.get_model("router")
            | StrOutputParser()
        )

        timing_data = {
            "session_id": f"proactive_decision_{past_interaction.id}",
            "mode": "proactive_gate",
        }
        ...
        if "yes" not in decision.lower():
            logger.info(
                f"💡 AI decided not to revisit this interaction. verb: {decision.lower()}"
            )
            return None
        ...
        proactive_message = asyncio.create_task(
            cortex_text_interaction.background_generate(
                db=db,
        ...
        if is_logical:
            logger.info(
                f"💡 Proactive message received from background task. Pushing to global SSE queue."
            )
    ...
    while True:
        try:
            ...
            if True:  # Using 'if True' for testing
                logger.info(
                    "💡 Proactive message chance MET. Finding and processing interaction in background thread..."
                )
                ...
                if result:
                    logger.info(
                        f"💡 Pushing proactive message to SSE queue: '{result['message'][:70]}...'"
                    )

                    sse_event = format_sse_notification(
                        data=result, event="proactive_thought"
                    )
...
def _create_personal_assistant_stub_response(
    endpoint_name: str,
    method: str,
    resource_id: Optional[str] = None,
    custom_status: str = "not_applicable_personal_assistant",
):
    """
...
def _create_assistants_api_stub_response(
    ...
    if operation_specific_message:
        message += (
            f" Regarding '{method} {endpoint_name}': {operation_specific_message}"
        )
...
async def handle_interaction():
    ...
    if request.method in ["GET", "HEAD"]:
        ...
        logger.info(
            f"Default Redirect: Redirecting unknown GET/HEAD request from User-Agent: '{user_agent}'"
        )
        return redirect("https://www.youtube.com/watch?v=dQw4w9WgXcQ")
    ...
    try:
        ...
        if not prompt:
            ...
            return Response(
                "Request must include a 'prompt'.", status=400, mimetype="text/plain"
            )
        ...
        logger.info(
            f"🚀 Async POST Request: Routing to direct_generate. Session: {session_id}, Prompt: '{prompt[:50]}...'"
        )

        # --- CORE CHANGE: Use `await` instead of `asyncio.run()` ---
        # This is the key to making the endpoint non-blocking.
        response_text = await cortex_text_interaction.direct_generate(
            db, prompt, session_id
        )
...
async def handle_mesh_community_serve_processor():
    ...
    try:
        ...
        if request.content_type and request.content_type.startswith(
            ...
            logger.info(
                f"{request_id}: Routing to ASR handler (will wait for ELP0 result)."
            )
            ...
            logger.info(
                f"{request_id}: Mesh ASR audio saved to temporary path: {temp_input_audio_path}"
            )
        ...
        # ==============================================================================
            ...
            if "messages" in request_data and isinstance(
                request_data.get("messages"), list
            ):
                has_image = "image_url" in str(request_data)
                mode_for_log = "vlm_chat_mesh" if has_image else "chat_mesh"
                logger.info(
                    f"{request_id}: Routing to ELP0 {mode_for_log} handler (waits for result)."
                )
            ...
            # --- ROUTE 2b: TEXT-TO-SPEECH ---
                ...
                logger.info(
                    f"{request_id}: Routing to ELP0 Text-to-Speech handler (waits for result)."
                )
                ...
                final_response_for_log = (
                    f"[Audio generated successfully in {mime_type} format]"
                )
            ...
            # --- ROUTE 2c: IMAGE GENERATION ---
                ...
                logger.info(
                    f"{request_id}: Routing to ELP0 Image Generation handler (waits for result)."
                )
                ...
                final_response_for_log = (
                    f"[{len(image_data_list)} image(s) generated successfully]"
                )
                ...
                raise ValueError(
                    "JSON payload does not match any supported service format (chat, tts, image_gen)."
                )

            if final_status_code == 200:
                logger.info(
                    f"{request_id}: Spawning background task for sanitized logging (Mode: {mode_for_log})."
                )
                asyncio.create_task(
        ...
        error_body = {
            "error": {
                "code": "bad_request_or_unsupported_format",
                "message": str(e),
                "supported_formats": [
        ...
        resp_data, _ = _create_openai_error_response(
            f"Internal server error: {type(e).__name__}", status_code=500
        )
...
def handle_cortex_config():
    ...
    if request.method == "GET":
        ...
        try:
            ...
            for key, value in new_settings.items():
                ...
                if key in constraints:
                    ...
                    try:
                        ...
                        return jsonify(
                            {
                                "error": f"Invalid type for '{key}'. Expected {constraint['type'].__name__}."
                            }
                        ), 400
                    ...
                    if "max" in constraint and value > constraint["max"]:
                        return jsonify(
                            {
                                "error": f"Value for '{key}' ({value}) exceeds maximum of {constraint['max']}."
                            }
                        ), 400
            ...
            logger.success(
                f"✅ {req_id}: Configuration successfully written to {env_file_path}"
            )
            return jsonify(
                {
            ...
            logger.warning(
                f"{req_id}: Invalid POST request to /ZephyCortexConfig: {ve}"
            )
...
async def handle_zephyrine_chat_response():
    ...
    try:
        ...
        session_id_for_logs = raw_request_data_dict.get(
            "session_id", f"zeph_req_{request_id}"
        )
        ...
        if stream_requested:
            ...
            return Response(
                json.dumps(resp_data_err),
                status=status_code_val,
                mimetype="application/json",
            )

        if not cortex_text_interaction:
            raise RuntimeError("cortex_text_interaction instance not available.")
        ...
        last_user_msg_obj = next(
            (msg for msg in reversed(messages_from_req) if msg.get("role") == "user"),
            None,
        )
        ...
        if not user_input_from_req and not image_b64_from_req:
            raise ValueError("No text or image content provided.")
        ...
        direct_response_text = await cortex_text_interaction.direct_generate(
            db, user_input_from_req, session_id_for_logs, image_b64=image_b64_from_req
        )

        # Format and return the non-streaming JSON response
        resp_data_ok = _format_openai_chat_response(
            direct_response_text, model_name=META_MODEL_NAME_NONSTREAM
        )
        ...
        logger.exception(
            f"{request_id}: 🔥🔥 UNHANDLED exception in Zeph async handler:"
        )
        ...
        resp_obj = Response(
            json.dumps(resp_data), status=status_code, mimetype="application/json"
        )
...
async def handle_openai_embeddings():
    ...
    if (
        not cortex_backbone_provider
        or not cortex_backbone_provider.embeddings
        or not cortex_backbone_provider.EMBEDDINGS_MODEL_NAME
    ):
        ...
        resp_data, status_code = _create_openai_error_response(
            "Embedding model not available.", err_type="server_error", status_code=500
        )
        response_payload = json.dumps(resp_data)
        return Response(
            response_payload, status=status_code, mimetype="application/json"
        )
    ...
    try:
        ...
        model_requested = request_data.get(
            "model"
        )  # Log requested model, but ignore it

        if model_requested:
            logger.warning(
                f"{request_id}: Request specified model '{model_requested}', but will use configured '{model_name_to_return}'."
            )
        ...
        if isinstance(input_data, list):
            if not all(isinstance(item, str) for item in input_data):
                logger.warning(
                    f"{request_id}: 'input' array must contain only strings."
                )
                resp_data, status_code = _create_openai_error_response(
                    "If 'input' is an array, all elements must be strings.",
                    err_type="invalid_request_error",
                    status_code=400,
                )
                response_payload = json.dumps(resp_data)
            ...
            logger.warning(
                f"{request_id}: 'input' must be a string or an array of strings."
            )
            resp_data, status_code = _create_openai_error_response(
        ...
        logger.debug(
            f"{request_id}: Embedding {len(texts_to_embed)} text(s) using {model_name_to_return}..."
        )
        ...
        if len(texts_to_embed) == 1:
            ...
            embedding_vector = await asyncio.to_thread(
                cortex_backbone_provider.embeddings.embed_query, texts_to_embed[0]
            )
            embeddings_list = [embedding_vector]
        else:
            # Use embed_documents for list of strings
            embeddings_list = await asyncio.to_thread(
                cortex_backbone_provider.embeddings.embed_documents, texts_to_embed
            )
        ...
        try:
            ...
            try:
                add_interaction(
                    error_db,
                    session_id=f"openai_emb_error_{request_id}",
                    mode="embedding",
                    input_type="error",
                    user_input=f"Embedding Error. Request: {str(request_data)[:1000]}",
                    llm_response=f"Handler Error: {e}"[:2000],
                )
            finally:
                error_db.close()
        except Exception as db_err:
        ...
        duration_req = (time.monotonic() - start_req) * 1000
        logger.info(
            f"🏁 OpenAI-Style Embedding Request {request_id} handled in {duration_req:.2f} ms. Status: {status_code}"
        )
...
async def handle_legacy_completions():
    ...
    logger.info(
        f"🚀 Async-Safe Legacy Completion Request ID: {request_id} on Endpoint: {endpoint_hit}"
    )
    ...
    try:
        ...
        logger.info(
            f"{request_id}: Awaiting non-blocking CortexThoughts.generate for legacy prompt..."
        )
        ...
        if "internal error" in response_text.lower() or "Error:" in response_text:
            ...
            resp_data, _ = _create_openai_error_response(
                response_text, status_code=status_code
            )
        else:
            status_code = 200
            logger.debug(
                f"{request_id}: CortexThoughts.generate completed successfully."
            )
            resp_data = _format_legacy_completion_response(
                response_text, model_name=META_MODEL_NAME_NONSTREAM
            )
        ...
        logger.exception(
            f"{request_id}: 🔥🔥 Unhandled exception in async legacy completions handler:"
        )
        resp_data, status_code = _create_openai_error_response(
...
async def handle_openai_chat_completion():
    ...
    try:
        ...
        session_id_for_logs = raw_request_data_dict.get(
            "session_id", f"openai_req_{request_id}"
        )

        if not cortex_text_interaction:
            raise RuntimeError(
                "Critical: cortex_text_interaction instance not available."
            )
        ...
        if not user_input_from_req and not image_b64_from_req:
            raise ValueError(
                "No text or image content was provided in the user message."
            )
        ...
        logger.info(
            f"{request_id}: Awaiting non-blocking direct_generate (ELP1) response..."
        )
        ...
        if stream_requested_by_client:
            logger.info(
                f"{request_id}: ELP1 generation complete. Starting CONTEXT-FREE pseudo-stream."
            )
            ...
            logger.info(
                f"{request_id}: Awaiting non-blocking direct_generate (ELP1) for non-streaming response."
            )
            ...
            if (
                "interrupted" in full_response_text.lower()
                or "Error:" in full_response_text
            ):
                final_response_status_code = (
                    503 if "interrupted" in full_response_text.lower() else 500
                )
                resp_data_err, _ = _create_openai_error_response(
                    full_response_text, status_code=final_response_status_code
                )
                resp_obj = Response(
                    json.dumps(resp_data_err),
                    status=final_response_status_code,
                    mimetype="application/json",
                )
            else:
                # 3. Format the complete response into the standard OpenAI JSON object.
                resp_data_ok = _format_openai_chat_response(
                    full_response_text, model_name=META_MODEL_NAME_NONSTREAM
                )
                resp_obj = jsonify(resp_data_ok)
                final_response_status_code = 200
        ...
        logger.exception(
            f"{request_id}: 🔥🔥 UNHANDLED exception in async completions handler:"
        )
        err_msg = f"Internal server error: {type(e).__name__}"
        resp_data, status_code = _create_openai_error_response(err_msg, status_code=500)
        resp_obj = Response(
...
async def handle_openai_moderations():
    ...
    try:
        ...
        model_requested = raw_request_data.get(
            "model"
        )  # Logged but we'll use our configured model name
        ...
        if isinstance(input_data, list):
            ...
            input_text_to_moderate = str(
                input_data[0]
            )  # Process first item if array for now
            if len(input_data) > 1:
                logger.warning(
        ...
        if not input_text_to_moderate.strip():
            ...
            logger.info(
                f"{request_id}: Input text is empty or whitespace. Returning as not flagged."
            )
            results_list = [
                {
                    "flagged": False,
        ...
        moderation_prompt_filled = PROMPT_MODERATION_CHECK.format(
            input_text_to_moderate=input_text_to_moderate
        )
        ...
        if cortex_text_interaction:
            cortex_text_interaction.current_session_id = moderation_llm_session_id
        ...
        logger.info(
            f"{request_id}: LLM moderation assessment raw response: '{llm_assessment_text}'"
        )
        ...
        if llm_assessment_text.startswith("FLAGGED:"):
            flagged = True
            try:
                violated_categories_str = llm_assessment_text.replace(
                    "FLAGGED:", ""
                ).strip()
                violated_list = [
                    cat.strip() for cat in violated_categories_str.split(",")
                ]
                for cat_key in violated_list:
                    if cat_key in categories:
                        ...
                        logger.warning(
                            f"{request_id}: LLM reported an unknown category '{cat_key}'"
                        )
            ...
            "CLEAN" not in llm_assessment_text.upper()
        ):  # If not "CLEAN" and not "FLAGGED:", it's ambiguous
            logger.warning(
        ...
        try:
            ...
            logger.error(
                f"{request_id}: Failed to log moderation result to DB: {db_log_err}"
            )
            if db:
                db.rollback()
        ...
        RuntimeError
    ) as rte:  # Catch errors from downstream calls like model unavailable
        logger.error(f"{request_id}: Runtime error during moderation: {rte}")
        resp_data, status_code = _create_openai_error_response(
            str(rte), err_type="server_error", status_code=500
        )
        resp = Response(
            json.dumps(resp_data), status=status_code, mimetype="application/json"
        )
        ...
        logger.exception(
            f"{request_id}: 🔥🔥 Unhandled exception in Moderation endpoint:"
        )
        error_message = (
            f"Internal server error in Moderation endpoint: {type(main_err).__name__}"
        )
        resp_data, status_code = _create_openai_error_response(
        ...
        try:
            ...
            logger.error(
                f"{request_id}: ❌ Failed log Moderation handler error: {db_err_log}"
            )
    ...
    if resp is None:
        logger.error(
            f"{request_id}: Moderation Handler logic flaw - response object 'resp' was not assigned!"
        )
        resp_data, _ = _create_openai_error_response(
            "Internal error: Handler failed to produce a response.", status_code=500
        )
...
async def handle_openai_models():
    ...
    model_list = [
        ...
        },
        {
            ...
            "owned_by": META_MODEL_OWNER,
            "permission": [],
            "root": META_MODEL_NAME_NONSTREAM,
            "parent": None,
        },
        {
        ...
        },
        {
            ...
            "owned_by": META_MODEL_OWNER,
            "permission": [],
            "root": TTS_MODEL_NAME_CLIENT_FACING,
            "parent": None,
        },
        {
        ...
        },
        {
            ...
            "owned_by": META_MODEL_OWNER,
            "permission": [],
            "root": AUDIO_TRANSLATION_MODEL_CLIENT_FACING,
            "parent": None,
        },
        {
    ...
    duration_req = (time.monotonic() - start_req) * 1000
    logger.info(
        f"🏁 /v1/models async request handled in {duration_req:.2f} ms. Status: {status_code}"
    )
    return response
...
async def handle_ollama_chat():
    ...
    try:
        ...
        stream_requested_by_client = raw_request_data_dict.get("stream", False)
        session_id_for_logs = raw_request_data_dict.get(
            "session_id", f"ollama_req_{request_id}"
        )
        ...
        total_duration_ns = int(
            (elp1_end_time - start_req_time_main_handler) * 1_000_000_000
        )
        ...
        if stream_requested_by_client:
            logger.info(
                f"{request_id}: ELP1 complete. Starting Ollama-style CONTEXT-FREE pseudo-stream."
            )
            ...
            logger.info(
                f"{request_id}: Awaiting direct_generate for non-streaming Ollama response."
            )
        ...
        logger.exception(
            f"{request_id}: 🔥🔥 UNHANDLED exception in async Ollama chat handler:"
        )
...
async def handle_ollama_tags():
    ...
    duration_req = (time.monotonic() - start_req) * 1000
    logger.info(
        f"🏁 /api/tags async request handled in {duration_req:.2f} ms. Status: {status_code}"
    )
    return response
...
async def handle_openai_asr_transcriptions():
    ...
    logger.info(
        f"🚀 OpenAI-Style ASR Request ID: {request_id} (ELP1 Pipeline with ELP0-Quality Failover)"
    )
    ...
    try:
        ...
        if not ENABLE_ASR:
            logger.warning(
                f"{request_id}: ASR endpoint called but ASR is disabled in config."
            )
            resp_data, status_code = _create_openai_error_response(
        ...
        if not cortex_text_interaction:
            raise RuntimeError(
                "CortexThoughts instance not configured for ASR post-processing."
            )
        ...
        if not audio_file_storage:
            raise ValueError("'file' field (audio data) is required.")
        if not model_requested or model_requested != ASR_MODEL_NAME_CLIENT_FACING:
            raise ValueError(
                f"Invalid 'model'. This endpoint supports '{ASR_MODEL_NAME_CLIENT_FACING}'."
            )

        language_for_asr_steps = (
            (language_param_for_log or WHISPER_DEFAULT_LANGUAGE).strip().lower()
        )
        if not language_for_asr_steps:
        ...
        temp_fd, temp_input_audio_path = tempfile.mkstemp(
            prefix="asr_orig_", suffix=file_extension, dir=temp_audio_dir
        )
        os.close(temp_fd)
        await asyncio.to_thread(audio_file_storage.save, temp_input_audio_path)
        logger.info(
            f"{request_id}: Input audio saved temporarily to: {temp_input_audio_path}"
        )
        ...
        if elp1_asr_err:
            raise RuntimeError(
                f"Low-Latency ASR step failed critically: {elp1_asr_err}"
            )

        raw_transcription = (
            elp1_asr_response.get("result", {}).get("text") or ""
        ).strip()
        ...
        if not is_transcription_valid:
            ...
            if hq_asr_err:
                raise RuntimeError(
                    f"High-Quality ASR failover step failed critically: {hq_asr_err}"
                )
            ...
            if (
                ...
                logger.warning(
                    f"{request_id}: High-quality ASR also produced no valid speech. Result will be empty."
                )
                raw_transcription = ""
        ...
        if response_format_req == "json":
            ...
            resp = Response(
                final_text_for_client, status=200, mimetype="text/plain; charset=utf-8"
            )
        ...
        if background_task_needed:
            ...
            asyncio.create_task(
                _run_background_asr_and_translation_analysis(
            ...
            temp_input_audio_path = (
                None  # Background task now owns the file for cleanup
            )
        else:
            logger.info(
                f"{request_id}: Skipping background task because high-quality failover was performed."
            )
        ...
        logger.warning(
            f"{request_id}: A handled error occurred in ASR endpoint: {type(e).__name__} - {e}"
        )
        ...
        resp_data, _ = _create_openai_error_response(
            str(e), err_type=error_type, status_code=status_code
        )
        resp = Response(
            json.dumps(resp_data), status=status_code, mimetype="application/json"
        )
        ...
        logger.exception(
            f"{request_id}: 🔥🔥 Unhandled exception in ASR endpoint (transcription):"
        )
        error_message = f"Internal server error in ASR endpoint: {type(e).__name__}"
        resp_data_err, status_code_err = _create_openai_error_response(
            error_message, status_code=500
        )
        resp = Response(
            json.dumps(resp_data_err),
            status=status_code_err,
            mimetype="application/json",
        )
        ...
        if temp_input_audio_path and os.path.exists(temp_input_audio_path):
            if background_task_needed:
                ...
                logger.info(
                    f"{request_id}: Cleaning up temp file in 'finally' after successful ELP1 failover process."
                )
            try:
                ...
                logger.warning(
                    f"{request_id}: Failed to delete temp file in main handler finally: {e_del_final}"
                )
    ...
    if resp is None:
        logger.error(
            f"{request_id}: ASR Handler logic flaw - response object 'resp' was not assigned!"
        )
        resp_data_err, _ = _create_openai_error_response(
            "Internal error: Handler did not produce a response.", status_code=500
        )
        resp = Response(
            json.dumps(resp_data_err), status=500, mimetype="application/json"
        )
...
async def handle_openai_audio_translations():
    ...
    logger.info(
        f"🚀 OpenAI-Style Audio Translation Request ID: {request_id} (Multi-Stage ELP1 + Background ELP0)"
    )
    ...
    diarized_transcription_for_client: Optional[str] = (
        None  # This goes into translation
    )
    ...
    try:
        ...
        if (
            not model_requested
            or model_requested != AUDIO_TRANSLATION_MODEL_CLIENT_FACING
        ):
            raise ValueError(
                f"Invalid 'model'. Expected '{AUDIO_TRANSLATION_MODEL_CLIENT_FACING}'."
            )
        ...
        temp_fd, temp_input_audio_path = tempfile.mkstemp(
            prefix="translate_orig_", suffix=file_extension, dir=temp_audio_dir
        )
        ...
        logger.info(
            f"{request_id}: ELP1 Step 1.1: Low-Latency ASR (Model: {WHISPER_LOW_LATENCY_MODEL_FILENAME})..."
        )
        asr_worker_script = os.path.join(SCRIPT_DIR, "audio_worker.py")
        ll_asr_cmd = [
            APP_PYTHON_EXECUTABLE,
        ...
        ll_asr_req_data = {
            "input_audio_path": temp_input_audio_path,
            "whisper_model_name": WHISPER_LOW_LATENCY_MODEL_FILENAME,
            "language": source_language_code_asr,
            "request_id": f"{request_id}-llasr",
        }

        ll_asr_resp, ll_asr_err = await asyncio.to_thread(
            _execute_audio_worker_with_priority,
            ll_asr_cmd,
            ll_asr_req_data,
            ELP1,
        ...
        if ll_asr_err or not (
            ll_asr_resp
            and isinstance(ll_asr_resp.get("result"), dict)
            and "text" in ll_asr_resp["result"]
        ):
            raise RuntimeError(
                f"Low-Latency ASR failed: {ll_asr_err or 'Invalid ASR worker response'}"
            )
        ...
        if raw_low_latency_transcription and raw_low_latency_transcription.strip():
            ...
            if cortex_text_interaction:
                cortex_text_interaction.current_session_id = correction_session_id
            llm_correction_output = await cortex_text_interaction.direct_generate(
                db, correction_prompt, correction_session_id, None, None
            )
            if llm_correction_output and not (
                isinstance(llm_correction_output, str)
                and "ERROR" in llm_correction_output.upper()
            ):
                corrected_transcription = llm_correction_output.strip()
                logger.info(
                    f"{request_id}: Auto-correction successful. Snippet: '{corrected_transcription[:100]}...'"
                )
            else:
                logger.warning(
        ...
        if corrected_transcription and corrected_transcription.strip():
            ...
            diarization_prompt = PROMPT_SPEAKER_DIARIZATION.format(
                transcribed_text=corrected_transcription
            )
            diarization_session_id = f"diarize_{request_id}"
            if cortex_text_interaction:
                cortex_text_interaction.current_session_id = diarization_session_id
            llm_diarization_output = await cortex_text_interaction.direct_generate(
                db, diarization_prompt, diarization_session_id, None, None
            )
            if llm_diarization_output and not (
                isinstance(llm_diarization_output, str)
                and "ERROR" in llm_diarization_output.upper()
            ):
        ...
        if not translation_model:
            raise RuntimeError(
                f"LLM role '{TRANSLATION_LLM_ROLE}' for translation unavailable."
            )

        src_lang_full = (
            langcodes.Language.make(language=source_language_code_asr).display_name()
            if source_language_code_asr != "auto"
            else "Unknown (auto-detect)"
        )
        tgt_lang_full = langcodes.Language.make(
            language=target_language_code
        ).display_name()

        trans_prompt_input = {
            "text_to_translate": diarized_text_for_translation,
            "target_language_full_name": tgt_lang_full,
            "target_language_code": target_language_code,
            "source_language_full_name": src_lang_full,
            "source_language_code": source_language_code_asr,
        }
        trans_chain = (
            ChatPromptTemplate.from_template(PROMPT_TRANSLATE_TEXT)
            | translation_model
            | StrOutputParser()
        )
        trans_timing_data = {
            "session_id": session_id_for_log,
            "mode": "translation_elp1",
        }
        ...
        if not quick_translated_text_for_client or (
            isinstance(quick_translated_text_for_client, str)
            and (
                "ERROR" in quick_translated_text_for_client.upper()
                or "Traceback" in quick_translated_text_for_client
            )
        ):
            raise RuntimeError(
                f"ELP1 LLM translation failed. Response: {quick_translated_text_for_client}"
            )
        ...
        logger.info(
            f"{request_id}: ELP1 Step 1.5: Synthesizing quick translated text to audio..."
        )
        ...
        if not final_tts_voice:
            ...
            final_tts_voice = lang_map.get(
                target_language_code, f"{target_language_code.upper()}-US"
            )
        ...
        tts_req_data = {
            "input": quick_translated_text_for_client,
            "voice": final_tts_voice,
            "response_format": output_audio_format,
            "request_id": f"{request_id}-elp1-tts",
        }

        tts_resp, tts_err = await asyncio.to_thread(
            _execute_audio_worker_with_priority,
            tts_worker_cmd,
            tts_req_data,
            ELP1,
        ...
        if tts_err or not (
            tts_resp
            and isinstance(tts_resp.get("result"), dict)
            and "audio_base64" in tts_resp["result"]
        ):
            raise RuntimeError(
                f"ELP1 TTS step failed: {tts_err or 'Invalid TTS worker response'}"
            )
        ...
        elp1_log_summary = (
            f"ELP1 Pipeline for Request {request_id}:\n"
        ...
        await asyncio.to_thread(
            ...
            llm_response=elp1_log_summary,
            classification="translation_elp1_successful",
            execution_time_ms=(time.monotonic() - start_req_time) * 1000,
        )
        await asyncio.to_thread(db.commit)
        ...
        logger.info(
            f"{request_id}: Spawning background task for high-quality ASR & deep translation (ELP0)..."
        )
        asyncio.create_task(
            _run_background_asr_and_translation_analysis(
                original_audio_path=temp_input_audio_path,
        ...
        temp_input_audio_path = (
            None  # Background task now owns the temp file for its ASR pass
        )
    ...
    # Ensure all db operations and file operations in except/finally are wrapped in asyncio.to_thread
        ...
        logger.error(
            f"{request_id}: Server configuration error for Audio Translation: {fnf_err}"
        )
        resp_data, status_code = _create_openai_error_response(
        ...
        resp_data, status_code = _create_openai_error_response(
            f"Audio Translation failed: {rt_err}",
            err_type="server_error",
            status_code=500,
        )
        resp = Response(
        ...
        resp_data, status_code = _create_openai_error_response(
            f"Translation task interrupted: {tie}",
            err_type="server_error",
        ...
        logger.exception(
            f"{request_id}: 🔥🔥 Unhandled exception in Audio Translation endpoint:"
        )
        error_message = (
            f"Internal server error in Audio Translation endpoint: {type(e).__name__}"
        )
        resp_data, status_code = _create_openai_error_response(
            error_message, status_code=500
        )
        ...
        try:
            ...
            logger.error(
                f"{request_id}: ❌ Failed log AudioTranslation handler error: {db_err_log}"
            )
    finally:
    ...
    if resp is None:
        logger.error(
            f"{request_id}: Audio Translation Handler logic flaw - response object 'resp' was not assigned!"
        )
        resp_data, _ = _create_openai_error_response(
            "Internal error: Handler failed to produce a response.", status_code=500
        )
...
async def handle_openai_tts():  # <<< CHANGED to async def
    ...
    try:
        ...
        response_format_requested = raw_request_data.get(
            "response_format", "mp3"
        ).lower()
        ...
        if not voice_requested or not isinstance(voice_requested, str):
            raise ValueError(
                "'voice' field (MeloTTS speaker ID, e.g., EN-US) is required."
            )
        ...
        if not os.path.exists(audio_worker_script_path):
            raise FileNotFoundError(
                f"Audio worker script missing at {audio_worker_script_path}"
            )
        ...
        logger.info(
            f"{request_id}: Executing audio worker with ELP1 priority in a background thread..."
        )
        ...
        if error_string_from_worker:
            ...
            audio_b64_data = audio_info["audio_base64"]
            actual_audio_format = audio_info.get("format", "mp3")
            response_mime_type = audio_info.get(
                "mime_type", f"audio/{actual_audio_format}"
            )
            ...
            raise RuntimeError(
                f"Audio worker returned invalid or incomplete response: {parsed_response_from_worker}"
            )
        ...
        resp_data, status_code = _create_openai_error_response(
            str(server_err), err_type="server_error", status_code=500
        )
        resp = Response(
            json.dumps(resp_data), status=status_code, mimetype="application/json"
        )
        ...
        error_message = (
            f"Internal server error in TTS endpoint: {type(main_handler_err).__name__}"
        )
        resp_data, status_code = _create_openai_error_response(
            error_message, status_code=500
        )
        ...
        try:
            ...
            logger.error(
                f"{request_id}: ❌ Failed log TTS handler error to DB: {db_err_log}"
            )
            if db:
                db.rollback()
    finally:
    ...
    if resp is None:
        logger.error(
            f"{request_id}: TTS Handler logic flaw - response object 'resp' was not assigned!"
        )
        resp_data, _ = _create_openai_error_response(
            "Internal error: Handler failed to produce a response object.",
            status_code=500,
        )
        resp = Response(json.dumps(resp_data), status=500, mimetype="application/json")
...
async def handle_openai_image_generations():  # Route is async
    ...
    if (
        db is None
    ):  # Fallback if g.db is not set (e.g. if before_request failed or not run)
        logger.error(
            f"{request_id}: DB session not found in g.db. Creating temporary session for this request."
        )
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"{request_id}: Failed to get/parse JSON body: {json_err}")
            try:
                request_data_snippet_for_log = (await request.get_data(as_text=True))[
                    :1000
                ]  # await for Quart
            except:
        ...
        if (
            cortex_text_interaction
        ):  # Ensure cortex_text_interaction global instance is available
            cortex_text_interaction.current_session_id = (
                session_id_for_log  # Set for helpers in cortex_text_interaction
            )
        else:
            logger.error(
            ...
            resp_data, status_code_val = _create_openai_error_response(
                "Server AI component (CortexThoughts) not ready.",
                err_type="server_error",
                status_code=503,
            )
            resp = Response(
        ...
        if n_images_requested_by_client is None:
            ...
            logger.info(
                f"{request_id}: 'n' not specified, defaulting to {n_images} for ELP1 image generation."
            )
        else:
            try:
                ...
                logger.warning(
                    f"{request_id}: Invalid 'n': '{n_images_requested_by_client}'. Defaulting to 1."
                )
                n_images = 1
        ...
        if not prompt_from_user or not isinstance(prompt_from_user, str):
            raise ValueError(
                "'prompt' field is required and must be a string for image generation."
            )
        if not model_requested or model_requested != IMAGE_GEN_MODEL_NAME_CLIENT_FACING:
            raise ValueError(
                f"Invalid 'model'. This endpoint supports '{IMAGE_GEN_MODEL_NAME_CLIENT_FACING}'."
            )
        ...
        logger.info(
            f"{request_id}: Refining user prompt for image generation (ELP1)..."
        )
        ...
        if wrapped_rag_result.get("status") == "success":
            ...
            if isinstance(rag_data_tuple, tuple) and len(rag_data_tuple) == 4:
                (
                    _url_ret_temp,
                    session_hist_retriever_for_refine,
                    _refl_ret_temp,
                    _ids_temp,
                ) = rag_data_tuple
            else:  # Should be caught by wrapper, but safeguard
                raise RuntimeError(
                    f"RAG wrapper returned unexpected data structure for image prompt: {rag_data_tuple}"
                )
        elif wrapped_rag_result.get("status") == "interrupted":
            raise TaskInterruptedException(
                wrapped_rag_result.get(
                    "error_message", "RAG for image prompt interrupted"
                )
            )
        else:  # Error
            raise RuntimeError(
        ...
        if session_hist_retriever_for_refine:
            retrieved_history_docs = await asyncio.to_thread(
                session_hist_retriever_for_refine.invoke, prompt_from_user
            )
        ...
        direct_hist_interactions_list = await asyncio.to_thread(
            get_global_recent_interactions, db_to_use, limit=3
        )
        recent_direct_history_str = cortex_text_interaction._format_direct_history(
            direct_hist_interactions_list
        )

        refined_prompt_for_generation = (
            await cortex_text_interaction._refine_direct_image_prompt_async(
                db=db_to_use,
                session_id=session_id_for_log,
                user_image_request=prompt_from_user,
                history_rag_str=history_rag_str,
        ...
        if (
            not refined_prompt_for_generation
            or refined_prompt_for_generation == prompt_from_user
        ):
            logger.info(
                f"{request_id}: Prompt refinement yielded no change or failed. Using original prompt."
            )
            refined_prompt_for_generation = prompt_from_user
        else:
            logger.info(
                f"{request_id}: Using refined prompt for image generation: '{refined_prompt_for_generation}'"
            )
        ...
        try:  # Log the prompt that will be used
            add_interaction(
                ...
                llm_response=refined_prompt_for_generation,
            )  # Refined prompt sent to worker
            db_to_use.commit()
        except Exception as db_log_err_prompt:
            logger.error(
                f"{request_id}: Failed to log refined image prompt: {db_log_err_prompt}"
            )
            if db_to_use:
                db_to_use.rollback()
        ...
        for i in range(n_images):
            ...
            if image_gen_err_msg:
                logger.error(
                    f"{request_id}: Image generation failed for attempt {i + 1}: {image_gen_err_msg}"
                )
                loop_error_message = image_gen_err_msg
                if interruption_error_marker in image_gen_err_msg:
                    raise TaskInterruptedException(image_gen_err_msg)
                error_during_loop = True
                break
            elif (
                list_of_one_image_dict
                and isinstance(list_of_one_image_dict, list)
                and list_of_one_image_dict
            ):
                all_generated_image_data_items.append(
                    list_of_one_image_dict[0]
                )  # Append the single image dict
                logger.info(f"{request_id}: Image {i + 1}/{n_images} data received.")
            else:
                logger.warning(
                    f"{request_id}: Image generation attempt {i + 1} returned no data and no error."
                )
                loop_error_message = (
                    "Image worker returned no data for an image attempt."
                )
                error_during_loop = True
                break
        ...
        if error_during_loop or not all_generated_image_data_items:
            final_err_msg = (
                loop_error_message or "Image generation failed to produce any results."
            )
            resp_data, status_code_val = _create_openai_error_response(
                final_err_msg, err_type="server_error", status_code=500
            )
            resp = Response(
                json.dumps(resp_data),
                status=status_code_val,
        ...
        for img_data_item in all_generated_image_data_items:
            ...
            if png_b64:
                if response_format_requested == "b64_json":
                    ...
                    logger.warning(
                        f"{request_id}: Returning data URI for 'url' format image request."
                    )
                    response_data_list_for_client.append(
                        {"url": f"data:image/png;base64,{png_b64}"}
                    )
            else:
                logger.error(
                    f"{request_id}: Worker image data item missing 'b64_json' (PNG). Item: {img_data_item}"
                )

        if not response_data_list_for_client:
            logger.error(
                f"{request_id}: No valid PNG b64_json data found after processing worker response(s)."
            )
            resp_data, status_code_val = _create_openai_error_response(
                "Failed to get valid image data from worker.",
                err_type="server_error",
                status_code=500,
            )
            resp = Response(
        ...
        openai_response_body = {
            "created": int(time.time()),
            "data": response_data_list_for_client,
        }
        ...
        resp = Response(
            json.dumps(resp_data), status=status_code_val, mimetype="application/json"
        )
        final_response_status_code = status_code_val
    except (
        TaskInterruptedException
    ) as tie:  # Catches interruptions from RAG or image gen
        logger.warning(
            f"🚦 {request_id}: Image Generation request (ELP1) INTERRUPTED: {tie}"
        )
        resp_data, status_code_val = _create_openai_error_response(
            f"Image generation task was interrupted: {tie}",
            err_type="server_error",
            code="task_interrupted",
            status_code=503,
        )
        ...
        logger.exception(
            f"{request_id}: 🔥🔥 Unhandled exception in Image Gen endpoint main try block:"
        )
        error_message = f"Internal server error in Image Gen endpoint: {type(e_main).__name__} - {str(e_main)}"
        resp_data, status_code_val = _create_openai_error_response(
            error_message, err_type="server_error", status_code=500
        )
        resp = Response(
            json.dumps(resp_data), status=status_code_val, mimetype="application/json"
        )
        ...
        try:
            ...
            logger.error(
                f"{request_id}: ❌ Failed log main Image Gen handler error to DB: {db_err_log_main}"
            )
            if db_to_use:
                db_to_use.rollback()
    finally:
        ...
        if (
            "db_temp_session" in locals() and db_temp_session
        ):  # Close temp session if created
    ...
    if resp is None:  # Should ideally not be reached if all paths assign to resp
        logger.error(
            f"{request_id}: Image Gen Handler logic flaw - response object 'resp' was not assigned!"
        )
        resp_data, status_code_val = _create_openai_error_response(
            "Internal error: Handler did not produce response.",
            err_type="server_error",
            status_code=500,
        )
        resp = Response(
        ...
        try:
            if db_to_use:
                add_interaction(
                    ...
                    llm_response="Critical: No resp obj created.",
                )
                db_to_use.commit()
        except:
            pass
...
async def handle_api_pull_dummy():
    ...
    logger.warning(
        "⚠️ Received async request for dummy endpoint: /api/pull Ollama impostor activated!"
    )
    ...
    def generate_dummy_pull():
        ...
        yield (
            json.dumps(
                {"status": "verifying sha256:...", "total": 100, "completed": 50}
            )
            + "\n"
        )
        time.sleep(0.5)
...
async def handle_api_push_dummy():
    ...
    logger.warning(
        "⚠️ Received async request for dummy endpoint: /api/push Ollama impostor activated!"
    )
...
async def handle_api_delete_dummy():
    ...
    logger.warning(
        "⚠️ Received async request for dummy endpoint: /api/delete Ollama impostor activated!"
    )
...
async def handle_api_create_dummy():
    ...
    logger.warning(
        "⚠️ Received async request for dummy endpoint: /api/create Ollama impostor activated!"
    )
...
async def handle_api_copy_dummy():
    ...
    logger.warning(
        "⚠️ Received async request for dummy endpoint: /api/copy Ollama impostor activated!"
    )
...
async def handle_api_blobs_dummy(digest: str):
    ...
    logger.warning(
        f"⚠️ Received async request for dummy endpoint: /api/blobs/{digest} Ollama impostor activated!"
    )
    if request.method == "HEAD":
...
async def handle_api_ps_dummy():
    ...
    logger.warning(
        "⚠️ Received async request for dummy endpoint: /api/ps (Not Implemented)"
    )
...
async def handle_openai_retrieve_model(model: str):
    ...
    duration_req = (time.monotonic() - start_req) * 1000
    logger.info(
        f"🏁 /v1/models/{model} async request handled in {duration_req:.2f} ms. Status: {status_code}"
    )
    return response
...
async def handle_create_pseudo_fine_tuning_job():
    ...
    logger.info(
        f"🚀 {request_id}: Received POST /v1/fine_tuning/jobs (Pseudo Fine-Tune via Data Ingestion)"
    )
    ...
    resp: Optional[Response] = None
    session_id_for_log: str = (
        f"pseudo_ft_req_{request_id}"  # Default session ID for this job's logs
    )
    raw_request_data: Optional[Dict[str, Any]] = None
    ...
    try:
        ...
        model_requested = raw_request_data.get(
            "model"
        )  # e.g., "gpt-3.5-turbo" - we log this

        if not training_file_ingestion_id or not isinstance(
            training_file_ingestion_id, str
        ):
            raise ValueError(
        ...
        if not model_requested:
            raise ValueError(
                "'model' (base model name to associate with this data) is required."
            )

        # Look up the uploaded file record
        uploaded_record = (
            db.query(UploadedFileRecord)
            .filter(UploadedFileRecord.ingestion_id == training_file_ingestion_id)
            .first()
        )
        ...
        if (
            uploaded_record.status == "processing"
            or uploaded_record.status == "queued_for_processing"
            or uploaded_record.status == "completed"
        ):
            logger.warning(
            ...
            response_body = {
                ...
                "hyperparameters": raw_request_data.get(
                    "hyperparameters", {"n_epochs": "continuous_self_reflection"}
                ),
                "trained_tokens": None,
        ...
        response_body = {
            ...
            "model": model_requested,
            "created_at": int(
                uploaded_record.created_at.timestamp()
        ...
        await asyncio.to_thread(
            ...
            llm_response=json.dumps(
                {
                    "job_id": job_id,
                    "status": response_body.get("status", "unknown_status"),
                }
            ),
        ...
        response_body, final_status_code = _create_openai_error_response(
            f"Server error during pseudo fine-tuning job creation: {str(e)}",
            err_type="server_error",
            status_code=500,
        )
        resp = Response(
        ...
        if db:
            await asyncio.to_thread(
                db.rollback
            )  # Rollback if error after DB interaction started
    finally:
        duration_req = (time.monotonic() - start_req_time) * 1000
        logger.info(
    ...
    if resp is None:
        logger.error(
            f"{request_id}: Pseudo FT Job handler logic flaw - response object 'resp' was not assigned!"
        )
        response_body, final_status_code = _create_openai_error_response(
...
async def handle_list_pseudo_fine_tuning_jobs():
    ...
    logger.info(
        f"🚀 {request_id}: Received GET /v1/fine_tuning/jobs (Pseudo Fine-Tune Info)"
    )
...
def handle_retrieve_fine_tuning_job(fine_tuning_job_id: str):
    ...
    if fine_tuning_job_id == "continuous_learning_main_process":
        ...
        response_body = {
            ...
            "hyperparameters": {
                "learning_mechanisms": [
                    "self_reflection",
                    "rag_vector_learning",
                    "background_generate_tot",
                ]
            },
        }
        logger.info(
            f"{request_id}: Responding with placeholder details for continuous learning process."
        )
        return jsonify(response_body), 200
    else:
        response_message = (
        ...
        logger.info(
            f"{request_id}: Job ID '{fine_tuning_job_id}' not found as a traditional job."
        )
        return jsonify(resp_data), 404


@system.route(
    "/v1/fine_tuning/jobs/<string:fine_tuning_job_id>/cancel", methods=["POST"]
)
def handle_cancel_fine_tuning_job(fine_tuning_job_id: str):
    request_id = f"req-ft-cancel-{uuid.uuid4()}"
    logger.info(
        f"🚀 {request_id}: Received POST /v1/fine_tuning/jobs/{fine_tuning_job_id}/cancel (Placeholder)"
    )
    ...
    if fine_tuning_job_id == "continuous_learning_main_process":
        ...
        logger.info(
            f"{request_id}: Responding that continuous learning process cannot be canceled."
        )
        return jsonify(response_body), 200
    else:
        resp_data, _ = _create_openai_error_response(
        ...
        logger.info(
            f"{request_id}: Job ID '{fine_tuning_job_id}' not found for cancellation."
        )
        return jsonify(resp_data), 404


@system.route(
    "/v1/fine_tuning/jobs/<string:fine_tuning_job_id>/events", methods=["GET"]
)
def handle_list_fine_tuning_job_events(fine_tuning_job_id: str):
    request_id = f"req-ft-events-{uuid.uuid4()}"
    logger.info(
        f"🚀 {request_id}: Received GET /v1/fine_tuning/jobs/{fine_tuning_job_id}/events (Placeholder)"
    )
    ...
    if fine_tuning_job_id == "continuous_learning_main_process":
            ...
            "data": [
                {
                    ...
                    "data": {
                        "step": None,
                        "metrics": {
                            "self_reflection_cycles": "ongoing",
                            "rag_updates": "continuous",
                        },
                    },
                    "type": "message",
                }
        ...
        logger.info(
            f"{request_id}: Responding with placeholder event for continuous learning."
        )
        return jsonify(response_body), 200
    else:
        ...
        logger.info(
            f"{request_id}: Job ID '{fine_tuning_job_id}' not found for events."
        )
        return jsonify(resp_data), 404
...
async def handle_upload_and_ingest_file():
    ...
    file_upload_ingestion_id = (
        f"file-upload-{uuid.uuid4()}"  # Unique ID for this specific upload
    )
    ...
    try:
        ...
        os.makedirs(
            FILE_INGESTION_TEMP_DIR, exist_ok=True
        )  # FILE_INGESTION_TEMP_DIR from CortexConfiguration.py

        # Create a unique temporary path for the uploaded file
        # Use a more descriptive prefix for temporary files
        temp_file_path = os.path.join(
            FILE_INGESTION_TEMP_DIR, f"{file_upload_ingestion_id}_{original_filename}"
        )
        await asyncio.to_thread(file_storage.save, temp_file_path)
        logger.info(
            f"{request_id}: File '{original_filename}' saved temporarily to '{temp_file_path}'."
        )
        ...
        if purpose == "fine-tune":
            logger.info(
                f"{request_id}: Purpose is 'fine-tune'. Automatically initiating background ingestion process for record ID {uploaded_record.id}..."
            )
        ...
        response_body = {
            ...
            "status": "processing_queued"
            if purpose == "fine-tune"
            else "uploaded",  # Reflect immediate action status
            "message": response_message,
        }
        final_status_code = 200
        ...
        if temp_file_path and os.path.exists(temp_file_path):
            try:
                ...
                logger.warning(
                    f"Failed to clean up partial file {temp_file_path}: {e_del}"
                )

    except Exception as e:
        logger.exception(f"{request_id}: 🔥🔥 Error processing uploaded file:")
        response_body, final_status_code = _create_openai_error_response(
            f"Server error during file upload: {str(e)}",
            err_type="server_error",
            status_code=500,
        )
        if db:
            await asyncio.to_thread(db.rollback)  # Rollback in case of DB error
    ...
    return Response(
        json.dumps(response_body), status=final_status_code, mimetype="application/json"
    )
...
async def handle_retrieve_file_ingestion_stub(file_id: str):
    ...
    logger.info(
        f"🚀 {request_id}: Received GET /v1/files/{file_id} (Ingestion Info Placeholder)"
    )
    ...
    if file_id.startswith("file-ingest-job-"):
        response_body = {
            ...
            "filename": "ingested_dataset_placeholder.jsonl",
            "purpose": "fine-tune-data-ingested-for-reflection",
            "status": "processed",
        ...
        resp_data, _ = _create_openai_error_response(
            f"Ingestion job ID '{file_id}' not found or not in expected format.",
            code="not_found",
            err_type="invalid_request_error",
        )
        return jsonify(resp_data), 404
...
async def handle_delete_file_ingestion_stub(file_id: str):
    ...
    logger.info(
        f"🚀 {request_id}: Received DELETE /v1/files/{file_id} (Ingestion Info Placeholder)"
    )
...
async def handle_retrieve_file_content_ingestion_stub(file_id: str):
    ...
    logger.info(
        f"🚀 {request_id}: Received GET /v1/files/{file_id}/content (Ingestion Info Placeholder)"
    )
    message = (
    ...
    resp_data, _ = _create_openai_error_response(
        message, code="content_not_available", err_type="invalid_request_error"
    )
    return jsonify(
        resp_data
    ), 404  # OpenAI returns 404 if content not retrievable for a file
...
async def handle_retrieve_file_metadata_stub(file_id: str):
    ...
    def db_lookup():
        ...
        try:
            ...
            if not record:
                record = (
                    db.query(FileIndex).filter(FileIndex.file_name == file_id).first()
                )
            return record
    ...
    if found_file_record:
        ...
        resp_data, _ = _create_openai_error_response(
            f"File '{file_id}' not found in local index.", code="file_not_found"
        )
        return jsonify(resp_data), 404
...
def handle_delete_file_stub(file_id: str):
    ...
    logger.info(
        f"{request_id}: Responding with placeholder for file deletion, explaining local file management."
    )
    return jsonify(response_body), 200
...
def handle_retrieve_file_content_stub(file_id: str):
    ...
    try:
        numeric_file_id = int(file_id)
        found_file_record = (
            db.query(FileIndex).filter(FileIndex.id == numeric_file_id).first()
        )
        search_method = f"database ID ({numeric_file_id})"
    except ValueError:
        found_file_record = (
            db.query(FileIndex).filter(FileIndex.file_path == file_id).first()
        )
        ...
        if not found_file_record:
            ...
            if found_file_record:
                search_method = f"file name ('{file_id}')"
    ...
    if found_file_record and found_file_record.indexed_content:
        ...
        mime_type = (
            found_file_record.mime_type if found_file_record.mime_type else "text/plain"
        )
        if "unknown" in mime_type.lower():
            mime_type = "text/plain"  # Fallback for "UnknownMIME"
        ...
        resp_data, _ = _create_openai_error_response(
            message=response_message,
            err_type="invalid_request_error",
            code="file_content_not_available",
        )
        logger.info(
            f"{request_id}: File '{file_id}' found but no indexed content available."
        )
        return jsonify(
            resp_data
        ), 404  # Or 200 with an error message in content if API expects that
    else:
        response_message = (
        ...
        logger.info(
            f"{request_id}: File '{file_id}' or its content not found in local index using {search_method}."
        )
        return jsonify(resp_data), 404
...
async def create_assistant_stub():
    ...
    return await asyncio.to_thread(
        _create_personal_assistant_stub_response,
        "/v1/assistants",
        "POST",
...
async def retrieve_assistant_stub(assistant_id: str):
    ...
    logger.info(
        f"🚀 {request_id}: Received async GET /v1/assistants/{assistant_id} (Placeholder)"
    )
    return await asyncio.to_thread(
        _create_personal_assistant_stub_response,
...
async def modify_assistant_stub(assistant_id: str):
    ...
    logger.info(
        f"🚀 {request_id}: Received async POST /v1/assistants/{assistant_id} (Placeholder)"
    )
    return await asyncio.to_thread(
        _create_personal_assistant_stub_response,
        f"/v1/assistants/{assistant_id}",
...
async def delete_assistant_stub(assistant_id: str):
    ...
    logger.info(
        f"🚀 {request_id}: Received async DELETE /v1/assistants/{assistant_id} (Placeholder)"
    )
    return await asyncio.to_thread(
        _create_personal_assistant_stub_response,
        f"/v1/assistants/{assistant_id}",
...
async def retrieve_thread_stub(thread_id: str):
    ...
    logger.info(
        f"🚀 {request_id}: Received async GET /v1/threads/{thread_id} (Placeholder)"
    )
    return await asyncio.to_thread(
        _create_personal_assistant_stub_response,
...
async def modify_thread_stub(thread_id: str):
    ...
    logger.info(
        f"🚀 {request_id}: Received async POST /v1/threads/{thread_id} (Placeholder)"
    )
    return await asyncio.to_thread(
        _create_personal_assistant_stub_response,
        f"/v1/threads/{thread_id}",
...
async def delete_thread_stub(thread_id: str):
    ...
    logger.info(
        f"🚀 {request_id}: Received async DELETE /v1/threads/{thread_id} (Placeholder)"
    )
    return await asyncio.to_thread(
        _create_personal_assistant_stub_response,
        f"/v1/threads/{thread_id}",
...
async def create_message_stub(thread_id: str):
    ...
    logger.info(
        f"🚀 {request_id}: Received async POST /v1/threads/{thread_id}/messages (High-Fidelity Stub)"
    )
...
async def list_messages_stub(thread_id: str):
    ...
    logger.info(
        f"🚀 {request_id}: Received async GET /v1/threads/{thread_id}/messages (High-Fidelity Stub)"
    )
                ...
                "content": [
                    {
                        "type": "text",
                        "text": {
                            "value": "A simulated user message.",
                            "annotations": [],
                        },
                    }
                ],
...
async def create_run_stub(thread_id: str):
    ...
    logger.info(
        f"🚀 {request_id}: Received async POST /v1/threads/{thread_id}/runs (High-Fidelity Stub)"
    )
...
async def retrieve_run_stub(thread_id: str, run_id: str):
    ...
    logger.info(
        f"🚀 {request_id}: Received async GET /v1/threads/{thread_id}/runs/{run_id} (High-Fidelity Stub)"
    )
...
@system.route(
    "/v1/threads/<string:thread_id>/runs/<string:run_id>/cancel", methods=["POST"]
)
...
@system.route(
    "/v1/threads/<string:thread_id>/runs/<string:run_id>/steps", methods=["GET"]
)
...
@system.route(
    "/v1/threads/<string:thread_id>/runs/<string:run_id>/submit_tool_outputs",
    methods=["POST"],
)
...
async def startup_tasks():
    ...
    if (
        ...
        and stella_icarus_daemon_manager.is_enabled
    ):
        logger.info(
            "AdelaideAlbertCortex: Starting all StellaIcarus Ada Daemon services..."
        )
    ...
    if ENABLE_FILE_INDEXER:
        logger.info(
            "AdelaideAlbertCortex: startup_tasks: Attempting to initialize global FileIndex vector store..."
        )
        if cortex_backbone_provider and cortex_backbone_provider.embeddings:
            ...
            await init_file_vs_from_indexer(
                cortex_backbone_provider
            )  # This is initialize_global_file_index_vectorstore
            ...
            logger.error(
                "AdelaideAlbertCortex: startup_tasks: CRITICAL - CortexEngine or embeddings None. Cannot init FileIndex VS."
            )
    else:
        logger.info(
            "AdelaideAlbertCortex: startup_tasks: File Indexer and its Vector Store are DISABLED by config."
        )

    if ENABLE_SELF_REFLECTION:
        logger.info(
            "AdelaideAlbertCortex: startup_tasks: Attempting to initialize global Reflection vector store..."
        )
        if cortex_backbone_provider and cortex_backbone_provider.embeddings:
            ...
            logger.error(
                "AdelaideAlbertCortex: startup_tasks: CRITICAL - CortexEngine or embeddings None. Cannot init Reflection VS."
            )
    else:
        logger.info(
    ...
    logger.info(
        f"AdelaideAlbertCortex: >>> Exiting startup_tasks (async). Total Duration: {task_duration:.2f}s <<<"
    )
...
async def _initialize_prebuffer_lut():
    ...
    try:
        ...
        if not db_session:
            raise RuntimeError(
                "Failed to create a database session for LUT initialization."
            )

        # --- Step 1: Query the database for an existing entry ---
        logger.debug(
            f"{log_prefix}: Checking database for existing PREBUFFER_LUT content..."
        )
        existing_lut_entry = (
        ...
        if existing_lut_entry and existing_lut_entry.llm_response:
            ...
            logger.info(
                f"{log_prefix}: Found existing LUT content in database. Loading into memory."
            )
            _PREBUFFERED_THINK_CONTENT = existing_lut_entry.llm_response
        else:
            # --- Step 3c: If not found, generate and save it ---
            logger.warning(
                f"{log_prefix}: No PREBUFFER_LUT content found. Generating a new one..."
            )
            if not cortex_text_interaction:
                raise RuntimeError(
                    "Cannot generate LUT content, CortexThoughts instance is not available."
                )
            ...
            if not generated_monologue or "Error:" in generated_monologue:
                raise RuntimeError(
                    f"Failed to generate LUT content from LLM. Response: {generated_monologue}"
                )

            _PREBUFFERED_THINK_CONTENT = generated_monologue.strip()
            logger.success(
                f"{log_prefix}: New LUT content generated: '{_PREBUFFERED_THINK_CONTENT[:70]}...'"
            )
            ...
            db_session.commit()
            logger.info(
                f"{log_prefix}: New LUT content has been saved to the database for future startups."
            )
...
if __name__ == "__main__":
    ...
    logger.error(
        "This program (AdelaideAlbertCortex) is designed to be run with an ASGI/WSGI server like Hypercorn. Running it like this is not supported."
    )
    ...
    logger.info(
        ">>> AdelaideAlbertCortex: MODULE IMPORTED BY SERVER (Hypercorn worker process) <<<"
    )
    logger.info(
        "----------------------------------------------------------------------"
    )
    ...
    if (
        ...
        print(
            "AdelaideAlbertCortex: CRITICAL FAILURE - Core AI components not initialized. Exiting.",
            file=sys.stderr,
            flush=True,
        )
        sys.exit(1)
        ...
        logger.success(
            "AdelaideAlbertCortex: ✅ Core AI components appear initialized globally."
        )
    ...
    try:
        logger.info(
            "AdelaideAlbertCortex: >>> CALLING init_db() NOW. This must complete successfully for the application. <<<"
        )
        # init_db() is imported from database.py
        # It's responsible for setting up the engine, SessionLocal, and migrations.
        init_db()
        db_initialized_successfully = (
            True  # If init_db() returns without exception, assume success
        )
        logger.success(
            "AdelaideAlbertCortex: ✅ init_db() call completed (reported no critical errors)."
        )
        ...
        logger.critical(
            f"AdelaideAlbertCortex: 🔥🔥 init_db() FAILED CRITICALLY DURING APP STARTUP: {e_init_db_call}"
        )
        logger.exception(
            "AdelaideAlbertCortex: Traceback for init_db() failure at app level:"
        )
        print(
            f"AdelaideAlbertCortex: CRITICAL FAILURE IN init_db(): {e_init_db_call}. Cannot continue. Exiting.",
            file=sys.stderr,
            flush=True,
        )
    ...
    try:
        from database import (
            SessionLocal as AppSessionLocalCheck,  # Re-import to get current state
        )
        ...
        if AppSessionLocalCheck is None:
            ...
            if hasattr(AppSessionLocalCheck, "kw") and AppSessionLocalCheck.kw.get(
                "bind"
            ):
                logger.success(
                    "AdelaideAlbertCortex: ✅ SessionLocal appears configured and bound to an engine."
                )
            else:
                logger.error(
        ...
        logger.critical(
            f"AdelaideAlbertCortex: FATAL - Unexpected error checking SessionLocal: {e_sl_check}. EXITING."
        )
        sys.exit(1)

    # --- Run Asynchronous Startup Tasks (like Vector Store Initialization) ---
    logger.info(
        "AdelaideAlbertCortex: 🚀 Preparing to run asynchronous startup_tasks (e.g., Vector Store initializations)..."
    )
    ...
    try:
        logger.debug(
            "AdelaideAlbertCortex: Setting up asyncio event loop for startup_tasks..."
        )
        try:
            ...
            if loop.is_closed():
                logger.warning(
                    "AdelaideAlbertCortex: Default asyncio event loop was closed. Creating new one for startup_tasks."
                )
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
        except RuntimeError:  # No current event loop on this thread
            logger.info(
                "AdelaideAlbertCortex: No current asyncio event loop for startup_tasks. Creating new one."
            )
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
        ...
        loop.run_until_complete(
            startup_tasks()
        )  # startup_tasks() is defined earlier in AdelaideAlbertCortex
        ...
        print(
            f"AdelaideAlbertCortex: CRITICAL FAILURE IN startup_tasks(): {su_err}. Cannot continue. Exiting.",
            file=sys.stderr,
            flush=True,
        )
        sys.exit(1)
    ...
    if db_initialized_successfully and startup_tasks_completed_successfully:
        ...
        logger.info(
            "AdelaideAlbertCortex: Initializing and Starting Interaction Indexer service..."
        )
        if (
            "initialize_global_interaction_vectorstore" in globals()
            and "cortex_backbone_provider" in globals()
        ):
            initialize_global_interaction_vectorstore(cortex_backbone_provider)

        if (
            "InteractionIndexer" in globals()
            and "cortex_backbone_provider" in globals()
        ):
            _interaction_indexer_stop_event = threading.Event()
            interaction_indexer_thread = InteractionIndexer(
                _interaction_indexer_stop_event, cortex_backbone_provider
            )
        ...
        if "ENABLE_FILE_INDEXER" in globals() and ENABLE_FILE_INDEXER:
            ...
            if "start_file_indexer" in globals() and callable(
                globals()["start_file_indexer"]
            ):
                start_file_indexer()  # This function should exist in AdelaideAlbertCortex
            else:
                logger.error(
                    "AdelaideAlbertCortex: 'start_file_indexer' function not found. File Indexer NOT started."
                )
        else:
            logger.info(
                "AdelaideAlbertCortex: File Indexer is DISABLED by config or variable not found. Not starting."
            )
        ...
        if "ENABLE_SELF_REFLECTION" in globals() and ENABLE_SELF_REFLECTION:
            ...
            if "start_self_reflector" in globals() and callable(
                globals()["start_self_reflector"]
            ):
                start_self_reflector()  # This function should exist in AdelaideAlbertCortex
            else:
                logger.error(
                    "AdelaideAlbertCortex: 'start_self_reflector' function not found. Self Reflector NOT started."
                )
        else:
            logger.info(
                "AdelaideAlbertCortex: Self Reflector is DISABLED by config or variable not found. Not starting."
            )
    else:
        logger.error(
        ...
        print(
            "AdelaideAlbertCortex: CRITICAL - Reached end of startup with initialization flags false. Exiting.",
            file=sys.stderr,
            flush=True,
        )
        sys.exit(1)
    ...
    logger.info("--------------------------------------------------------------------")
    logger.info(
        "AdelaideAlbertCortex: ✅ Zephyrine mainEngineFrame_MacroController_EngineSharedResources module-level initializations complete."
    )
    logger.info(
        f"   Application (app) is now considered ready by this worker process (PID: {os.getpid()})."
    )
