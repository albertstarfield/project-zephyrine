...
def fast_file_discovery(repo_root: str) -> List[str]:
    ...
    gitignore_patterns_re = [
        re.compile(p) for p in load_gitignore_patterns_for_re(repo_root)
    ]
    ...
    for root, dirs, files in os.walk(repo_root, topdown=True):
        dir_count += 1
        print(
            f"\r   -> [Walker] Scanned {dir_count} directories. Current: {os.path.relpath(root, repo_root)}",
            end="",
            flush=True,
        )
        ...
        dirs[:] = [
            ...
            if not any(
                p.fullmatch(
                    os.path.join(os.path.relpath(root, repo_root), d)
                    .replace(os.sep, "/")
                    .lstrip("./")
                )
                for p in gitignore_patterns_re
            )
        ]
        ...
        for file in files:
            ...
            relative_path = (
                os.path.join(rel_dir, file).replace(os.sep, "/")
                if rel_dir != "."
                else file
            )
            if not any(
                p_re.fullmatch(relative_path.lstrip("./"))
                for p_re in gitignore_patterns_re
            ):
    ...
    for i, line in enumerate(chunk.splitlines()):
        ...
        if "math." in line or "cmath." in line:
            update_notes(
                ...
                line_num,
                line,
                "A mathematical formula was detected.",
    ...
    if chunk.strip():
        ...
        if "LLM API Error" not in llm_analysis:
            update_notes(
                ...
                file_path,
                (chunk_num * CONFIG["MAX_CHUNK_LINES"]) + 1,
                chunk[:200] + "...",
                llm_analysis,
            )
        else:
            update_notes(
                ...
                (chunk_num * CONFIG["MAX_CHUNK_LINES"]) + 1,
                chunk[:200] + "...",
                "LLM analysis failed for this chunk.",
                llm_analysis,
            )
...
def analyze_image_file(file_path: str):
    ...
    try:
        ...
        update_notes(
            "IMAGE REVIEW", file_path, 1, os.path.basename(file_path), vlm_analysis
        )
    except Exception as e:
        update_notes(
            "IMAGE REVIEW",
...
def perform_cynical_audit():
    ...
    cortex_code = full_codebase_map.get(
        "systemCore/mainEngineFrame_MacroController_EngineSharedResources/AdelaideAlbertCortex.py",
        "",
    )
    if (
        "direct_generate" in cortex_code
        and "background_generate" in cortex_code
        and "watchdog" in cortex_code
    ):
        update_notes(
            "LOGICAL FLAW",
    ...
    if (
        "background_generate_task_semaphore" in cortex_code
        and "asyncio.create_task" in cortex_code
    ):
        update_notes(
            "POTENTIAL ISSUE",
            "AdelaideAlbertCortex.py",
...
def map_finding_type_to_dal(finding_type: str) -> str:
    mapping = {
        "CRITICAL BUG": "DAL A (Fatal)",
        "LOGICAL FLAW": "DAL A (Fatal)",
        "HIGH RISK": "DAL B (Hazardous)",
        "POTENTIAL ISSUE": "DAL C (Major)",
        "AUTOMATED VALIDATION ANALYSIS": "DAL D (Minor)",
        "MATHEMATICAL VALIDATION": "DAL C (Major)",
        "IMPROVEMENT": "DAL D (Minor)",
        "IMAGE REVIEW": "DAL E (No Safety Effect)",
    }
...
def generate_report():
    ...
    for connections in file_connections.values():
        nodes.update(connections)
    for node in nodes:
        mermaid_graph += f"    {node.replace('.py', '')}[{node}];\n"
    for source, targets in file_connections.items():
        for target in targets:
            mermaid_graph += (
                f"    {source.replace('.py', '')} --> {target.replace('.py', '')};\n"
            )
...
"""
    ...
    if image_findings:
        ...
        for finding in image_findings:
            report_content += f"| `{finding['file']}` | {finding['note']} |\n"
    cynical_findings = [f for f in audit_findings if f["type"] in ["LOGICAL FLAW"]]
    rule_llm_findings = [
        f for f in audit_findings if f["type"] not in ["IMAGE REVIEW", "LOGICAL FLAW"]
    ]
    if cynical_findings:
        ...
        for finding in cynical_findings:
            ...
            report_content += (
                f"| **{finding['type']}** | `{finding['file']}` | {note_suggestion} |\n"
            )
    severity_order = [
        "CRITICAL BUG",
        "HIGH RISK",
        ...
        if severity in findings_by_type:
            ...
            for finding in findings_by_type[severity]:
                ...
                if finding["suggestion"]:
                    note_suggestion += (
                        f"<br><br>**Suggestion:** {finding['suggestion']}"
                    )
    ...
    try:
        ...
        print(
            f"   ⚠️  Feature list not found at {CONFIG['FEATURE_LIST_PATH']}. Skipping feature audit."
        )
    ...
    if feature_list_content:
        ...
        for line in feature_list_content.splitlines():
            ...
            if match:
                ...
                parsed_features.append(
                    {"status": status_str, "description": desc.strip()}
                )
        ...
        for feature in parsed_features:
            ...
            linked_findings = [
                ...
                if any(
                    kw in f"{f['file']} {f['note']} {f['code']}".lower()
                    for kw in keywords
                    if len(kw) > 3
                )
            ]
            ...
            appendix_md += (
                f"| {description_md} | {status} | {scrutiny_notes} | {risks_md} |\n"
            )
...
def generate_pdf_report(md_path: str, pdf_path: str):
    ...
    try:
        ...
        asyncio.run(
            md_to_pdf(
                pdf_path, md_file_path=md_path, stylesheet_string=github_style_css
            )
        )
...
def main():
    ...
    for i, file_path in enumerate(files_to_audit):
        ...
        total_lines = len(lines)
        num_chunks = (
            (total_lines + CONFIG["MAX_CHUNK_LINES"] - 1) // CONFIG["MAX_CHUNK_LINES"]
            if total_lines > 0
            else 1
        )
        if not content_for_chunking.strip():
            print(
                f"   ({i + 1}/{len(files_to_audit)}) Auditing Code: {rel_path} [Empty File]"
            )
            continue
        ...
        if "db_session_opened_on_line" in current_file_context:
            update_notes(
                "POTENTIAL ISSUE",
                file_path,
                current_file_context["db_session_opened_on_line"],
...
if __name__ == "__main__":
    ...
    CONFIG["REPORT_FILE"] = CONFIG["REPORT_TEMPLATE"].replace(
