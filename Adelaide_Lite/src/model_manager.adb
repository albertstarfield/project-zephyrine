...
package body Model_Manager is
   ...
   function Get_Context (Kind : Model_Type) return Llama_Interface.Llama_Context is
   begin
      return Models (Kind).Context;
   end Get_Context;

   function Get_Model (Kind : Model_Type) return Llama_Interface.Llama_Model is
   begin
   ...
   begin
      ...
      Models (Qwen_0_8B).Path := To_Unbounded_String 
        ("../llama.cpp/models/qwen2.5-0.8b-instruct-q8_0.gguf");
      Models (Qwen_4B).Path := To_Unbounded_String 
        ("../llama.cpp/models/qwen2.5-3b-instruct-q8_0.gguf");
   ...
   begin
      Generate (Qwen_4B, Wrap_ChatML (Persona, Prompt), Int_Res, 
                Images, Session_ID, 8192, Stream, (Stream /= null), Level);
      Result := Int_Res;
   end Hybrid_Generate;
   ...
   begin
      ...
      if not S then
         Model_Gate.Release (Kind);
         Result := To_Unbounded_String ("ERROR");
         return;
      end if;
         ...
         begin
            ...
            if Len > 0 then
               ...
               begin
                  Append (Result, S_P);
                  if Stream /= null then
                     Push_Chunk (Stream, Session_ID, S_P);
                  end if;
               end;
            end if;
            declare
