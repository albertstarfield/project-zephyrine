...
package body Model_Manager is
   ...
   protected body Model_Gate is
      ...
      procedure Release (Kind : Model_Type) is
         Total_ELP1 : Natural := 0;
      begin
         Busy (Kind) := False;
         for K in Model_Type loop
            Total_ELP1 := Total_ELP1 + Acquire_ELP1 (K)'Count;
         end loop;
         if Total_ELP1 = 0 then Abort_Flag := False; end if;
      end Release;
   ...
   begin
      ...
      M_Params.N_Gpu_Layers := -1; --  Correct C type handled via binding
      Models (Kind).Model := Llama_Model_Load_From_File (Path_C, M_Params);
      Free (Path_C);

      if Models (Kind).Model /= Null_Model then
         C_Params.N_Ctx := Interfaces.C.unsigned (Requested_Ctx);
   ...
   function Wrap_ChatML (S_Msg : String; U_Msg : String) return String is
   begin
      return "<|im_start|>system" & ASCII.LF & S_Msg & "<|im_end|>" & ASCII.LF &
             "<|im_start|>user" & ASCII.LF & U_Msg & "<|im_end|>" & ASCII.LF &
   ...
   begin
      while I <= Text'Last loop
         ...
         else Append (Result, Text (I)); I := I + 1; end if;
      end loop;
   ...
   begin
      if Session_ID'Length > 0 and then Session_ID (Session_ID'First) = '/' then
         ...
         Set_Field (C_Obj, "model", String'("adelaide-hybrid"));
         Set_Field (C_Obj, "done", False);
         declare
            Msg : constant JSON_Value := Create_Object;
         begin
            Set_Field (Msg, "role", String'("assistant"));
            Set_Field (Msg, "content", Str_Piece);
            Set_Field (C_Obj, "message", Msg);
         end;
         Stream.Push (Write (C_Obj) & ASCII.LF);
      end if;
   end Push_Chunk;
   ...
   begin
      ...
      if not Success then Model_Gate.Release (Kind); Result := To_Unbounded_String ("ERROR"); return; end if;

      Models (Kind).In_Use := True;
      Vocab := Llama_Model_Get_Vocab (Models (Kind).Model);
      N_Toks := Llama_Tokenize (Vocab, P_C, int (Prompt'Length), Tokens (1)'Address, 32768, True, True);
      Free (P_C);
      ...
      for I in 1 .. 2048 loop
         ...
         declare
            T : constant Llama_Token := Llama_Sampler_Sample (Sampler, Models (Kind).Context, -1);
            P : array (1 .. 256) of aliased Character;
            L : int;
         begin
            if Llama_Vocab_Is_Eog (Vocab, T) then exit; end if;
            L := Llama_Token_To_Piece (Vocab, T, P (1)'Address, 256, 0, True);
            if L > 0 then
               declare
                  S : constant String := String (P (1 .. Integer (L)));
               begin
                  Append (Result, S);
                  if Stream /= null then Push_Chunk (Stream, Session_ID, S); end if;
               end;
            end if;
            declare
               function Llama_Batch_Get_One (T : System.Address; N : int) return Llama_Batch;
               pragma Import (C, Llama_Batch_Get_One, "llama_batch_get_one");
               B : constant Llama_Batch := Llama_Batch_Get_One (T'Address, 1);
            begin
         ...
         begin
            ...
            if N > 0 then
               declare
                  function Llama_Batch_Get_One (T : System.Address; N : int) return Llama_Batch;
                  pragma Import (C, Llama_Batch_Get_One, "llama_batch_get_one");
                  B : constant Llama_Batch := Llama_Batch_Get_One (T (1)'Address, N);
               begin
                  ...
                  if Llama_Decode (Models (Qwen_Embedding).Context, B) = 0 then
                     declare
                        ...
                        D : constant int := Llama_Model_N_Embd (Models (Qwen_Embedding).Model);
   ...
   function Generator_Callback (P : String) return String is (declare R : Unbounded_String; begin Generate (Qwen_4B, P, R, GNATCOLL.JSON.Empty_Array, "", 4096, null, False, ELP1); return To_String (R); end);
