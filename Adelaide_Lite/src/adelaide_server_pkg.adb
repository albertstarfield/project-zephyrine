...
package body Adelaide_Server_Pkg is
   ...
   begin
      ...
      Model_Manager.Hybrid_Generate
        (To_String (Local_Prompt), Result, Stream => Queue, Session_ID => "async-stream");
      Queue.Close;
   ...
   begin
      ...
      if URI = "/v1/models" or else URI = "/api/tags" then
         ...
         declare
            Payload : Unbounded_String := (if Raw_S /= "" then To_Unbounded_String (Raw_S) else Raw_B);
            Model_Name : Unbounded_String := To_Unbounded_String ("adelaide-hybrid");
         begin
            if Length (Payload) > 0 then
               declare
                  Parser_Result : constant GNATCOLL.JSON.Read_Result := GNATCOLL.JSON.Read (To_String (Payload));
               begin
                  if Parser_Result.Success then
                     ...
                     begin
                        if GNATCOLL.JSON.Has_Field (Val, "name") then
                           Model_Name := To_Unbounded_String (String'(GNATCOLL.JSON.Get (Val, "name")));
                        elsif GNATCOLL.JSON.Has_Field (Val, "model") then
                           Model_Name := To_Unbounded_String (String'(GNATCOLL.JSON.Get (Val, "model")));
                        end if;
                     end;
                  end if;
               end;
      ...
      elsif URI = "/api/chat" or else URI = "/v1/chat/completions" or else URI = "/api/generate" then
         declare
            Payload : Unbounded_String := (if Raw_S /= "" then To_Unbounded_String (Raw_S) else Raw_B);
               ...
               begin
                  if Parser_Result.Success then
                     ...
                     if GNATCOLL.JSON.Has_Field (Val, "model") then
                        Req_Model := To_Unbounded_String (String'(GNATCOLL.JSON.Get (Val, "model")));
                     end if;
                     ...
                     if GNATCOLL.JSON.Has_Field (Val, "messages") then
                        ...
                        begin
                           if GNATCOLL.JSON.Has_Field (Last, "content") then
                              begin
                                 Prompt := To_Unbounded_String (String'(GNATCOLL.JSON.Get (Last, "content")));
                              exception
                                 when others => null;
                              end;
                           end if;
                        ...
                        Prompt := To_Unbounded_String (String'(GNATCOLL.JSON.Get (Val, "prompt")));
                     end if;
                  end if;
               end;
            end if;
            ...
            if Is_Streaming then
               ...
               begin
                  ...
                  T.Start (To_String (Prompt), To_String (Req_Model), 
                           (if URI = "/v1/chat/completions" then Streaming_Queue.OpenAI else Streaming_Queue.Ollama), Q);
                  return AWS.Response.Stream (Content_Type => (if URI = "/v1/chat/completions" then "text/event-stream" else "application/x-ndjson"), Handle => S);
               end;
            else
               Model_Manager.Hybrid_Generate (To_String (Prompt), Result, Stream => null, Session_ID => "web-api");
               if URI = "/api/generate" then
                  GNATCOLL.JSON.Set_Field (Resp, "model", To_String (Req_Model));
                  GNATCOLL.JSON.Set_Field (Resp, "response", To_String (Result));
                  ...
                  begin
                     ...
                     GNATCOLL.JSON.Set_Field (Msg_Out, "content", To_String (Result));
                     GNATCOLL.JSON.Set_Field (Choice, "message", Msg_Out);
                     GNATCOLL.JSON.Append (Choices, Choice);
                     GNATCOLL.JSON.Set_Field (Resp, "id", "chatcmpl-adelaide-" & TS_Str);
                     GNATCOLL.JSON.Set_Field (Resp, "object", "chat.completion");
                     ...
                     GNATCOLL.JSON.Set_Field (Resp, "done", True);
                     GNATCOLL.JSON.Set_Field (Usage, "prompt_tokens", Integer'(0));
                     GNATCOLL.JSON.Set_Field (Usage, "completion_tokens", Integer'(0));
                     GNATCOLL.JSON.Set_Field (Usage, "total_tokens", Integer'(0));
         ...
         begin
                  ...
                  if Parser_Result.Success then
                     ...
                     begin
                        if GNATCOLL.JSON.Has_Field (Val, "prompt") then
                           ...
                           begin
                              Prompt := To_Unbounded_String (String'(GNATCOLL.JSON.Get (Val, "input")));
                           exception
                              when others => null;
                           end;
                        end if;
            ...
            begin
               ...
               if Len = 0 then
                  for I in 1 .. 128 loop
                     GNATCOLL.JSON.Append (Emb_Arr, GNATCOLL.JSON.Create (Long_Float (0.1)));
                  end loop;
               else
                  for I in 1 .. Len loop
