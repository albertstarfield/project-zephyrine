...
package body Supertonic_Interface is
   ...
   begin
      if Current_TTS /= System.Null_Address then
         Free (Current_TTS);
      end if;
   ...
   procedure Load_Voice_Style (Path : String) is
      ...
      -- In Ada, simulating const char** is tricky, so we'll pass an array of one
      type Chars_Ptr_Array is array (0 .. 0) of aliased chars_ptr;
      Paths_Array : aliased Chars_Ptr_Array := (0 => C_Path);
   begin
      if Current_Style /= System.Null_Address then
   ...
   begin
      if Current_TTS = System.Null_Address or Current_Style = System.Null_Address then
      ...
      declare
         -- Convert floats to PCM bytes. 1 float = 4 bytes.
   ...
   begin
      if Current_Style /= System.Null_Address then
         Free_Style (Current_Style);
         Current_Style := System.Null_Address;
      end if;
      if Current_TTS /= System.Null_Address then
         Free (Current_TTS);
         Current_TTS := System.Null_Address;
      end if;
   end Shutdown;
