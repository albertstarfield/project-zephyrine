...
def install_dependencies_linux():
    ...
    if (
        subprocess.call(
            ["which", "apt-get"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        == 0
    ):
        ...
        subprocess.call(
            ["which", "dnf"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        == 0
    ):
        package_manager = "dnf"
        ...
        subprocess.call(
            ["which", "yum"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        == 0
    ):
        package_manager = "yum"
        ...
        subprocess.call(
            ["which", "zypper"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        == 0
    ):
        ...
        subprocess.call(
            ["which", "swupd"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        == 0
    ):
...
def install_dependencies_macos():
    ...
    if os.path.exists("/Applications/Xcode-beta.app") or os.path.exists(
...
def install_dependencies_windows():
    ...
    try:
        ...
        print(
            "Console may stay here for a while until 4-6 GB downloads some necessary dependencies! Downloading and Auto Installing Visual Studio"
        )
        subprocess.run(
            [
                "powershell",
        ...
        nmake_paths = (
            subprocess.run(
                [
                    "powershell",
                    "(Get-ChildItem -Path 'C:\\BuildTools\\VC\\Tools\\MSVC\\' -Filter '16.*.*' -Directory | Select-Object -ExpandProperty FullName | ForEach-Object { Join-Path -Path $_ -ChildPath 'bin\\Hostx64\\x64' })",
                ],
                capture_output=True,
                text=True,
                check=True,
...
def clean_installed_folder():
    print(
        "Cleaning Installed Folder to lower the chance of interfering with the installation process"
    )
...
def clone_submodule_light(
    path, url, commit
):  # lighter than clone_submodule espescially if you need to import Ada compiler that rarely have premade binaries for almost all computers
    print(f"Cloning submodule: {path} from {url}")
...
def import_submodules_manually():
    clone_submodule(
        os.path.join(rootdir, "systemCore", "vendor", "llama.cpp"),
        "https://github.com/ggerganov/llama.cpp",
        "master",
    )
    clone_submodule(
        os.path.join(rootdir, "systemCore", "vendor", "ggllm.cpp"),
        "https://github.com/cmp-nct/ggllm.cpp",
        "master",
    )
    clone_submodule(
        os.path.join(rootdir, "systemCore", "vendor", "ggml"),
        "https://github.com/ggerganov/ggml",
        "master",
    )
    clone_submodule(
        os.path.join(rootdir, "systemCore", "vendor", "llama-gguf.cpp"),
        "https://github.com/ggerganov/llama.cpp",
        "master",
    )
    clone_submodule(
        os.path.join(rootdir, "systemCore", "vendor", "whisper.cpp"),
        "https://github.com/ggerganov/whisper.cpp",
        "master",
    )
    clone_submodule(
        os.path.join(rootdir, "systemCore", "vendor", "gemma.cpp"),
    ...
    clone_submodule_light(
        LLVMAdaAOT, "https://github.com/AdaCore/gnat-llvm.git", "main"
    )
    clone_submodule_light(LLVMinterfaceAdaAOT, "git://gcc.gnu.org/git/gcc.git", "main")
    clone_submodule(
        AdaAlirePkg, "https://github.com/alire-project/alire.git", "master"
    )  # for alire you need to recurse submodule see the repo for more detail https://github.com/alire-project/alire
...
def build_llama():
    ...
    subprocess.run(
        ["git", "checkout", "93356bd"], check=True
    )  # return to ggml era not the dependencies breaking gguf model mode
    ...
    if platform_system in ["Linux", "Darwin"]:
        subprocess.run(
            ["cp", "bin/main", os.path.join(rootdir, "systemCore", "bin", "chat")],
            check=True,
        )
...
def build_ggml_base(binary_name):
    ...
    opencl = detect_opencl()
    metal = (
        "no_metal"  # Placeholder, update this if you have a metal detection function
    )
    ...
    try:
        subprocess.run(["cmake", ".."] + cmake_args, check=True)
        subprocess.run(
            ["make", "-j", str(get_alloc_threads()), binary_name], check=True
        )
        os.chdir(rootdir)
        if platform_system == "Linux":
            subprocess.run(
                ["cp", f"bin/{binary_name}", f"{rootdir}/usr/bin/chat"], check=True
            )
        elif platform_system == "Darwin":
            subprocess.run(
                ["cp", f"bin/{binary_name}", f"{rootdir}/usr/bin/chat"], check=True
...
def build_adaAOTcompiler():
    ...
    def get_latest_gcc_version():
        ...
        latest_version = sorted(versions, key=lambda s: list(map(int, s.split("."))))[
            -1
        ]
    ...
    gcc_url = (
        f"https://ftp.gnu.org/gnu/gcc/gcc-{latest_version}/gcc-{latest_version}.tar.xz"
    )
    ...
    def get_latest_version(base_url, component):
        ...
        return latest_version, f"{component}-{latest_version}.tar.gz" if any(
            "gz" in s
            for s in version_pattern.findall(f"{component}-{latest_version}.tar.gz")
        ) else f"{component}-{latest_version}.tar.bz2"
    ...
    def check_gnatmake():
        try:
            ...
            if result.returncode != 0:
                raise Exception(
                    "gnatmake command not found or not functioning properly."
                )
        except FileNotFoundError:
    ...
    if platform_system == "Darwin":
        subprocess.run(
            ["ln", "-s", "gcc/gcc/ada", "llvm-interface/gnat_src"], check=True
        )
    if platform_system == "Linux":
        subprocess.run(
            ["ln", "-s", "gcc/gcc/ada", "llvm-interface/gnat_src"], check=True
        ...
        subprocess.run(
            ["mv", "llvm-interface/gcc", "llvm-interface/gnat_src"], check=True
        )
    ...
    print(
        "ada AOT compile break! remove this when development of Ada AOT breakpoint, What's after this isn't implemented yet -Albert"
    )
    exit()
...
def buildLLMBackend():
    ...
    bin_dir = os.path.join(
        rootdir, "systemCore", "bin", targetFolderPlatform, targetFolderArch
    )
    ...
    shutil.copy(
        os.path.join(
            rootdir, "systemCore", "vendor", "llama.cpp", "build", "bin", "main"
        ),
        os.path.join(llama_dir, "LLMBackend-llama"),
    )
    print(
        "Copying any Acceleration and Debugging Dependencies for LLaMa GGML v2 v3 Legacy"
    )
    shutil.copytree(
        os.path.join(rootdir, "systemCore", "vendor", "llama.cpp", "build", "bin"),
        llama_dir,
        dirs_exist_ok=True,
    )
    ...
    shutil.copy(
        ...
        os.path.join(llama_gguf_dir, "LLMBackend-llama-gguf"),
    )
    shutil.copy(
        os.path.join(
            rootdir,
        ...
        os.path.join(llama_gguf_dir, "finetune"),
    )
    shutil.copy(
        os.path.join(
            rootdir,
        ...
        os.path.join(llama_gguf_dir, "export-lora"),
    )

    print(
        "Copying any Acceleration and Debugging Dependencies for LLaMa GGUF Neo Model"
    )
    shutil.copytree(
        os.path.join(rootdir, "systemCore", "vendor", "llama-gguf.cpp", "build", "bin"),
        llama_gguf_dir,
        dirs_exist_ok=True,
    )
    ...
    shutil.copy(
        os.path.join(
            rootdir, "systemCore", "vendor", "ggllm.cpp", "build", "bin", "main"
        ),
        os.path.join(falcon_dir, "LLMBackend-falcon"),
    )
    print("Copying any Acceleration and Debugging Dependencies for Falcon")
    shutil.copytree(
        os.path.join(rootdir, "systemCore", "vendor", "ggllm.cpp", "build", "bin"),
        falcon_dir,
        dirs_exist_ok=True,
    )
    ...
    shutil.copytree(
        os.path.join(rootdir, "systemCore", "vendor", "gemma.cpp", "build"),
        google_gemma_dir,
        dirs_exist_ok=True,
    )
    shutil.copy(
        os.path.join(rootdir, "systemCore", "vendor", "gemma.cpp", "build", "gemma"),
        os.path.join(google_gemma_dir, "LLMBackend-gemma"),
    )
    ...
    # compile the ./systemCore/adelaidemainEngineFrame_MacroController_EngineSharedResources/adaelaidemainEngineFrame_MacroController_EngineSharedResources.ada to ./systemCore/adelaidemainEngineFrame_MacroController_EngineSharedResources binary

    # Build AdelaidemainEngineFrame_MacroController_EngineSharedResources Ada Program for C++ LLM Management
    ...
    try:
        if platform.system() in ["Linux", "Darwin"]:
            ...
            if conda_prefix and n_prefix:
                subprocess.run(
                    ["sudo", "chmod", "-R", "777", conda_prefix, n_prefix], check=True
                )
            else:
            ...
            if conda_prefix and n_prefix:
                subprocess.run(
                    ["icacls", conda_prefix, "/grant", "Everyone:F", "/T"], check=True
                )
                subprocess.run(
                    ["icacls", n_prefix, "/grant", "Everyone:F", "/T"], check=True
                )
            else:
                print("CONDA_PREFIX or N_PREFIX not set.")
        else:
...
def main():
    ...
    if (
        not os.path.isfile(os.path.join(rootdir, "installed.flag"))
        or force_rebuild == "1"
    ):
        ...
        print(
            "Rebuilding Electron Program For specific Computer Configuration with OpenSSL FIPS"
        )
